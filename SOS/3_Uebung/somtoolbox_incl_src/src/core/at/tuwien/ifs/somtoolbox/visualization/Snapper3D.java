...
import at.tuwien.ifs.somtoolbox.structures.ComponentLine3DDistance;
...
public class Snapper3D extends Snapper{
    ...
    /*
    public static double[][] convertPointArrayToDoubleArrayArray(Point2D[] centres){
        double[][] doubleCentres = new double[centres.length][2];
        for (int i = 0; i < centres.length; i++) {
            doubleCentres[i][1] = centres[i].getX();
            doubleCentres[i][1] = centres[i].getY();
        }
        return doubleCentres;
    }
*/
 
//    public Snapper2D(AbstractMetric distanceFunction, LineDistanceFunction lineDistanceFunction, int xSize, int ySize) {
    ...
    public Point3d[] doSnapping(Point3d[] line) throws SOMToolboxException {
     ...
     //       ar[i] = new Point3d(centres[i][0], centres[i][1], centres[i][2]);
      //  }
        ...
        // Hashtable<Point3d, int[]> outgoingLineTable = new Hashtable<Point3d, int[]>(); // find counts for parallel lines
...
//        int xSize = new Double(gridSize).intValue();
  //      int ySize = new Double(gridSize).intValue();
    ...
    //    if (grid == null) {
            ...
            for (int x = 0; x < xSize; x++) {
                for (int y = 0; y < ySize; y++) {
                    ...
                    for (int z = 0; z < zSize; z++) {
                        // System.out.println(x + " / " + y + " / " + z + " | " + index);// + (x + y + z) + "| " + (x*y*z) + " | index: " + (x * xSize
                        // + y * ySize + z));
            ...
            /*
            gridSumsXY = new double[grid.length];
            gridDiffsXY = new double[grid.length];
            gridSumsYZ = new double[grid.length];
            gridDiffsYZ = new double[grid.length];
            gridSumsXZ = new double[grid.length];
            gridDiffsXZ = new double[grid.length];
            for (int i = 0; i < gridSumsXY.length; i++) {
                gridSumsXY[i] = grid[i].x + grid[i].y; // + grid[i].y;
                gridSumsYZ[i] = grid[i].y + grid[i].z; // + grid[i].y;
                gridSumsXZ[i] = grid[i].x + grid[i].z; // + grid[i].y;
                gridDiffsXY[i] = grid[i].x - grid[i].y; // - grid[i].y;
                gridDiffsYZ[i] = grid[i].y - grid[i].z; // - grid[i].y;
                gridDiffsXZ[i] = grid[i].x - grid[i].z; // + grid[i].y;
            }
            */
      //  }
     ...
     * TODO: Consider disallowing 135° / 315° as too sharp turns. private Point2D[] snap(Point2D[] line, int xSize, int ySize) { // Snapping process //
     * 1. For each bin centre, find the 4 neighbouring Unit locations, thus resulting in bins * 4 points // 2. For each point: consider the point as
     * fixed, and find a line that is correctly snapped (only 45°) angles and as near as possible to the // original line // 3. From the resulting
     * bins * 4 lines, chose the one closest to the original line ArrayList<Point2D[]> allSnappedLines = new ArrayList<Point2D[]>(); for (int i = 0;
     * i < line.length; i++) { Point2D[] neighbouringPoints = getNeighbouringUnits(line[i]); for (int j = 0; j < neighbouringPoints.length; j++) { //
     * find the snapped points forward and backwards from the current bin point // this means we will have lines e.g. as follows (for 6 bins, and i ==
     * 3) // lineSegmentForward = (0/0 0/0 0/0 0/0 x5/y5 x6/y6) // lineSegmentBackward = (x1/y1 x2/y2, x3/y3 0/0 0/0 0/0) Point2D[] lineSegmentForward =
     * snapPoint(neighbouringPoints[j], line, i + 1, +1, xSize, ySize, centres[0].length); Point2D[] lineSegmentBackward =
     * snapPoint(neighbouringPoints[j], line, i - 1, -1, xSize, ySize, centres[0].length); // then merge them to one line, and set the fixed point
     * Point2D[] mergedLine = new Point2D[lineSegmentForward.length]; for (int k = 0; k < lineSegmentBackward.length; k++) { mergedLine[k] = new
     * Point2D.Double(lineSegmentForward[k].getX() + lineSegmentBackward[k].getX(), lineSegmentForward[k].getY() + lineSegmentBackward[k].getY()); }
     * mergedLine[i] = neighbouringPoints[j]; // now if that point is the same as the last one, don't add that one // for some strange reason this is
     * required additionally to the condition in the recursion if ((i > 1 && (!(line[i].getX() == line[i - 1].getX() && line[i].getY() == line[i -
     * 1].getY())))) { allSnappedLines.add(mergedLine); } } } // find the closest snapped line double minDist = Double.MAX_VALUE; Point2D[]
     * minDistLine = null; for (int i = 0; i < allSnappedLines.size(); i++) { Point2D[] currentLine = (Point2D[]) allSnappedLines.get(i); double dist =
     * LineDistance.lineDistance(line, currentLine, lineDistanceFunction); if (dist < minDist) { minDist = dist; minDistLine = currentLine; } } return
     * minDistLine; }
     */
    /**
     * Returns a snapped line of the given line. Snapping the metro lines means to find a line as similar as possible to the given line, which has all
    ...
    private Point3d[] snap(Point3d[] line) {
        ...
        for (int i = 0; i < line.length; i++) {
            ...
            for (int j = 0; j < neighbouringPoints.length; j++) {
                ...
                Point3d[] lineSegmentForward = snapPoint(neighbouringPoints[j], line, i + 1, +1, line.length);
                Point3d[] lineSegmentBackward = snapPoint(neighbouringPoints[j], line, i - 1, -1, line.length);
                ...
                for (int k = 0; k < lineSegmentBackward.length; k++) {
                    ...
                    mergedLine[k] = new Point3d(
                            (lineSegmentForward[k].x + lineSegmentBackward[k].x) , 
                            (lineSegmentForward[k].y + lineSegmentBackward[k].y) , 
                            (lineSegmentForward[k].z + lineSegmentBackward[k].z) );
                }
                mergedLine[i] = neighbouringPoints[j];
//                System.out.println("candidate: " + i + " " + ArrayUtils.toString(mergedLine[i]));
   ...
   //     System.out.println("found " + allSnappedLines.size() + " solutions.");
        ...
        for (int i = 0; i < allSnappedLines.size(); i++) {
            Point3d[] currentLine = (Point3d[]) allSnappedLines.get(i);
     ...
     * 
     * @previousPoint the point to start from
...
/*
            if (
                       (grid[i].x == startPoint.x 
                               || gridSumsXY[i] == startPointCoordinatesSumXY 
                               || gridSumsXY[i] == startPointCoordinatesSumXY) // &&  grid[i].x == startPoint.z)// && grid[i].x == startPoint.y)
                    && (grid[i].y == startPoint.y 
                            || gridSumsYZ[i] == startPointCoordinatesSumYZ 
                            || gridDiffsYZ[i] == startPointCoordinatesDifferenceYZ) // &&  grid[i].y == startPoint.z) // && grid[i].x == grid[i].x)
                //    && (grid[i].z == startPoint.z 
                   //         || gridSumsXZ[i] == startPointCoordinatesSumXZ
                     //       || gridDiffsXZ[i] == startPointCoordinatesDifferenceXZ))
                            )
  ...
  */          
            ...
            if ( 
                    (grid[i].x == startPoint.x || grid[i].y == startPoint.y 
                            || Math.abs(grid[i].x - startPoint.x) == Math.abs(grid[i].y - startPoint.y))
                    && 
                    (grid[i].x == startPoint.x || grid[i].z == startPoint.z 
                            || Math.abs(grid[i].x - startPoint.x) == Math.abs(grid[i].z - startPoint.z))
                    && 
                    (grid[i].z == startPoint.z || grid[i].y == startPoint.y 
                            || Math.abs(grid[i].z - startPoint.z) == Math.abs(grid[i].y - startPoint.y))
                    ){

                double currentDistance = grid[i].distance(line[currentPosition]);
                if (currentDistance < minDistance) {
                    closestPoint = (Point3d) grid[i].clone();
                    minDistance = currentDistance;
                }
            }
        }

        // compare this startpoint to the last one and check for identity and don't consider the closest but the point itself for further processing
        // if so
        if (currentPosition > 1 
                && line[currentPosition].x == line[currentPosition - direction].x
    ...
    public Point3d[] getNeighbouringUnits(Point3d p) {
        ...
        Point3d leftUpperLow = new Point3d((double) new Double(p.x).intValue(), (double) new Double(p.y).intValue(),
                (double) new Double(p.z).intValue());
        ...
        return new Point3d[] { leftUpperLow, leftUpperHigh, leftLowerLow, leftLowerHigh, rightUpperLow, rightUpperHigh, rightLowerLow, rightLowerHigh };
