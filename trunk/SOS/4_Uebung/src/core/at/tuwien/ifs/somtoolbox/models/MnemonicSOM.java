...
/**
 * This class implements so-called Mnemonic SOMs, i.e. two-dimensional SOMs which are not rectangular or quadratic, but have a grid which is not fully
 * occupied with units.<br>
 * More details can be found in:<br>
 * <i>Mnemonic SOMs: Recognisable Shapes for Self-Organizing Maps</i>. Proceedings of the 5th Workshop On Self-Organizing Maps Paris (WSOM 2005), pp.
 * 131-138, September 5-8 2005, Paris, France. [<a href="http://www.ifs.tuwien.ac.at/~mayer/publications/pdf/may_wsom05.pdf" target="_blank">Download
 * as PDF</a>].
 * 
 * @author Rudolf Mayer
 * @version $Id: MnemonicSOM.java 2889 2009-12-12 00:31:37Z mayer $
 */
public class MnemonicSOM extends GrowingSOM  implements SOMToolboxApp {
    ...
    public static final Parameter[] OPTIONS = new Parameter[] { OptionFactory.getSwitchHtmlOutput(false), OptionFactory.getOptLabeling(false),
            OptionFactory.getOptNumberLabels(false), OptionFactory.getOptDimension(true), OptionFactory.getOptMapDescriptionFile(true),
            OptionFactory.getOptUnitDescriptionFile(true), OptionFactory.getOptNumberWinners(false), OptionFactory.getSwitchSkipDataWinnerMapping(),
    ...
    private MnemonicSOM(int id, Unit su, SOMInputReader ir) throws SOMToolboxException {
        ...
        super(id, su, ir, new MnemonicGrowingLayer(id, su, ir.getXSize(), ir.getYSize(), ir.getZSize(), ir.getMetricName(), ir.getDim(),
                ir.getVectors(), 7));
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            gsom = new MnemonicSOM(new MnemonicSOMLibFormatInputReader(null, unitDescFileName, mapDescFileName, dimension));
        } catch (Exception e) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(e.getMessage() + " Aborting.");
            e.printStackTrace();
            System.exit(-1);
        ...
        gsom.setSharedInputObjects(new SharedSOMVisualisationData(null, null, null, null, fileProps.vectorFileName(true),
        ...
        try {
            SOMLibMapOutputter.write(gsom, fileProps.outputDirectory(), fileProps.namePrefix(false), true, somProps, fileProps);
        } catch (IOException e) {
        ...
        if (!skipDataWinnerMapping) {
            try {
                SOMLibMapOutputter.writeDataWinnerMappingFile(gsom, data, numWinners, fileProps.outputDirectory(), fileProps.namePrefix(false), true);
            } catch (IOException e) {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(
                        "Could not open or write to output file " + fileProps.namePrefix(false) + ": " + e.getMessage());
                System.exit(-1);
