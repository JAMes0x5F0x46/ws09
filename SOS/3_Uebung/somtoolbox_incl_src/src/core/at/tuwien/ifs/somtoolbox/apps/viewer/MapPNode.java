...
/**
 * The graphical representation of a map in the {@link SOMViewer} application. This class makes use of the <a href="http://www.cs.umd.edu/hcil/jazz/"
 * target="_blank">Piccolo framework</a> and is the top {@link PNode} for all map-level visualisations. The class holds several other PNodes as
 * children:
 * <ul>
 ...
 * @version $Id: MapPNode.java 2874 2009-12-11 16:03:27Z frank $
 */
public class MapPNode extends PNode {
     ...
     */
    public MapPNode(JFrame parentFrame, String weightVectorFileName, String unitDescriptionFileName, String mapDescriptionFileName,
            CommonSOMViewerStateData state) throws FileNotFoundException, SOMLibFileFormatException {
        this(parentFrame, new MnemonicSOMLibFormatInputReader(weightVectorFileName, unitDescriptionFileName, mapDescriptionFileName), state, true);
    }
     ...
     * @param growingLayer The layer for whicht the mappnode is generated
     * @param state
     */
    ...
    private void init(JFrame parentFrame, SOMLibFormatInputReader inputReader, CommonSOMViewerStateData state, GrowingLayer growingLayer,
        ...
        if (fileName != null && !fileName.trim().equals("")) {
            try {
                InputCorrections inputCorrections = new InputCorrections(fileName, growingLayer, state.inputDataObjects.getInputData());
        ...
        try {
            ...
            for (int j = 0; j < growingLayer.getYSize(); j++) {
                for (int i = 0; i < growingLayer.getXSize(); i++) {
                    if (growingLayer.getUnit(i, j) != null) { // check needed for mnemonic SOMs (might not have all units != null)
                        Unit unit = growingLayer.getUnit(i, j);
                        Point[][] locations = null;
                        if (inputObjects.getDataWinnerMapping() != null) {
                            locations = initInputLocations(unit);
                        }
                        units[i][j] = new GeneralUnitPNode(unit, state, classInfo, dataInfo, locations, UNIT_WIDTH, UNIT_HEIGHT);
        ...
        if (classInfo != null) {
            state.classPiechartsVisible = true;
        ...
        if (inizializeVis) {
            Visualizations.initVisualizations(inputObjects, inputReader, state.defaultPaletteIndex, state.defaultPalette, state.palettes, this);
        }
        ...
        if (state.inputDataObjects.getLinkageMap() != null) {
            ...
            for (String beginName : linkageMap.keySet()) {
                String endName = (String) linkageMap.get(beginName);
        ...
        addInputEventListener(new ZoomChangeListener());
    }
    ...
    private Point[][] initInputLocations(Unit unit) {
        ...
        for (int index = 0; index < size; index++) {
            ...
            try {
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox").warning(e1.getMessage() + " Could not draw exact point on unit!");
                // e1.printStackTrace();
                continue;
            }
            ...
            for (int uindex = 1; uindex <= 3; uindex++) {
                ...
                // Find x-axis and y-axis pull force of the winner. The pull force of 2nd unit is higher than the 3rd and so on
                ...
                // Now clearly both units lie on the same position on y-axis and thus there is no need to compute displacement for y-axis
            ...
            for (int rindex = 0; rindex < index; rindex++) {
                double distance = Point.distanceSq(locdxy[rindex][0], locdxy[rindex][1], locdxy[index][0], locdxy[index][1]);
                if (distance < InputPNode.MIN_DISTANCE_SQ) {
                    ...
                    // And on the other sie this input is forcing that input to leave a free space for him by moving towards center.
        ...
        for (int i = 0; i < size; i++) {
            ...
            locations[1][i] = new Point((int) ((UNIT_WIDTH / 2) + locdxyShifted[i][0]), (int) ((UNIT_WIDTH / 2) + locdxyShifted[i][1]));
        }
        return locations;
    }
    ...
    public BackgroundImageVisualizer getVisualizationOfType(Class<?> classType) {
        for (int i = 0; i < visualizations.length; i++) {
            if (visualizations[i].getClass().equals(classType)) {
                return (BackgroundImageVisualizer) visualizations[i];
            }
        }
        return null;
    }
    ...
    public ThematicClassMapVisualizer getThematicClassMapVisualizer() {
        return (ThematicClassMapVisualizer) getVisualizationOfType(ThematicClassMapVisualizer.class);
    }
     ...
     * 
     * @param hist double[][] with histogram data, e.g. unit counts
     * @param vis_index visualization variant index (0 = flat, 1 = smoothed)
     */
    public void showHistogramOverlayVisualization(double[][] hist, int vis_index) {
        ...
        int palind = state.currentPalette;
        overlayVis.setPalette(palind, state.palettes[palind]);
        try {
            overlayVisualizationImage = new PImage(overlayVis.getVisualization(overlayVis_index, gsom, getScaledBackgroundImageWidth(overlayVis),
                    getScaledBackgroundImageHeight(overlayVis)));
        } catch (SOMToolboxException e) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(e.getMessage());
            return;
        }
    ...
    /**
     * Reloads the given palette after it has been edited with the {@link PaletteEditor}, i.e. basically invalidates the cache for all visualisations
     * and then calls {@link MapPNode#changePalette(int)}
     */
    public boolean reloadPaletteAfterEditing(int palette) throws SOMToolboxException {
        for (int i = 0; i < visualizations.length; i++) {
            if (visualizations[i] instanceof AbstractMatrixVisualizer) {
                ((AbstractMatrixVisualizer) visualizations[i]).invalidateCache(palette);
            }
        }
        return changePalette(palette);
    }

    public boolean changePalette(int palette) throws SOMToolboxException {
        state.currentPalette = palette;
        boolean doneSomething = false;

        if (overlayVisualizationImage != null && overlayVisualizationImage.isDescendentOf(this)) {
            // -> overlayVis is currently active
            overlayVis.setPalette(palette, state.palettes[palette]);
            this.removeChild(overlayVisualizationImage);
        ...
        if (currentVisualizationImage != null && currentVisualizationImage.isDescendentOf(this)) {
            if (currentVisualization instanceof AbstractMatrixVisualizer) {
                ((AbstractMatrixVisualizer) currentVisualization).setPalette(palette, state.palettes[palette]);
            }
            BufferedImage img = getVisualisation();
            this.removeChild(currentVisualizationImage);
            currentVisualizationImage = new PImage(img);
    ...
    public void setClusteringElements(SortedMap<Integer, ClusterElementsStorage> elements) {
        ...
        if (currentClusteringElements != null) {
            HashSet<PNode> tmp = new HashSet<PNode>(); // use set to avoid duplicates (labels appearing on multiple layers)
    ...
    public ArrayList<PNode>[] getClusterLabelsByLevel() {
        ...
        return (ArrayList<PNode>[]) labels.toArray(new ArrayList[labels.size()]);
    }
    ...
    /**
     * Method to be used for setting the initial visualisation on startup - does not actually create the image, just sets the
     * {@link #currentVisualization} and {@link #currentVisualizationVariant}.
     */
    public boolean setInitialVisualizationOnStartup(BackgroundImageVisualizer vis, int variant) throws SOMToolboxException {
        ...
        if (neededInputs != null && neededInputs.length > 0) {
            for (int i = 0; i < neededInputs.length; i++) {
                SOMVisualisationData inputObject = inputObjects.getObject(neededInputs[i]);
                // System.out.println("in need of: " + inputObject.getFileName() + ", "+ inputObject.getType() + ", " + inputObject.getName());
                inputObject.loadFromFile(state.getFileChooser(), parentFrame);
                if (currentVisualization.needsAdditionalFiles() == null || currentVisualization.needsAdditionalFiles().length == 0) {
    ...
    /**
     * Gets the visualisation variant from the current {@link #currentVisualization} obtaining the values from {@link #currentVisualizationVariant},
     * {@link #getScaledBackgroundImageWidth()} and {@link #getScaledBackgroundImageHeight()}.
     */
    private BufferedImage getVisualisation() throws SOMToolboxException {
        return currentVisualization.getVisualization(currentVisualizationVariant, gsom, getScaledBackgroundImageWidth(),
    ...
    protected void paint(PPaintContext paintContext) {
        ...
        // System.out.println("SCALE: " + paintContext.getScale());
        // double currentScale = paintContext.getScale();
        // // int currentDetailLevel;
        // String detailLevel = "unknown";
        // if (currentScale < state.scaleLimitDetailNo) { // no information
        // // currentDetailLevel = GeneralUnitPNode.DETAIL_LEVEL_NO;
        // detailLevel = "none";
        // } else if ((currentScale >= state.scaleLimitDetailNo) && currentScale < state.scaleLimitDetailLow) {
        // // little information
        // // currentDetailLevel = GeneralUnitPNode.DETAIL_LEVEL_LOW;
        // detailLevel = "low";
        // } else if ((currentScale >= state.scaleLimitDetailLow) && currentScale < state.scaleLimitDetailMedium) { // more labels
        // // currentDetailLevel = GeneralUnitPNode.DETAIL_LEVEL_MEDIUM;
        // detailLevel = "medium";
        // } else if (currentScale >= state.scaleLimitDetailMedium) { // detailed information
        // // currentDetailLevel = GeneralUnitPNode.DETAIL_LEVEL_HIGH;
        // detailLevel = "high";
        // }
        // if (state.getMapDetailPanel().isVisible()) {
        // state.getMapDetailPanel().updatePanel(detailLevel);
        // }
    }
            ...
            for (int j = 0; j < gsom.getLayer().getYSize(); j++) {
                for (int i = 0; i < gsom.getLayer().getXSize(); i++) {
                    if (gsom.getLayer().getUnit(i, j) != null) { // check needed for mnemonic SOMs
                        ...
                        // now let the new class pie charts be displayed (though the chosen solution seems like a hack..)
                        units[i][j].reInitUnitDetails();
    ...
    public void setBackgroundImage(BufferedImage background) {
        ...
        backgroundImage = new PImage(background.getScaledInstance(getBackgroundImageWidth(), getBackgroundImageHeight(), Image.SCALE_SMOOTH));
    ...
    public void reInitUnitPNodes(int detailLevel) {
        for (int i = 0; i < units.length; i++) {
            for (int j = 0; j < units[0].length; j++) {
                units[i][j].reInitUnitDetails(detailLevel);
            }
        }
    }

    public class ZoomChangeListener implements PInputEventListener {
        public void processEvent(PInputEvent aEvent, int type) {
            if (aEvent.isMouseWheelEvent()) { // zooming event - update the zoom display
                // double currentScale = paintContext.getScale();
                ...
                if (currentScale < state.scaleLimits[1]) { // no information
                    detailLevel = "none";
                } else if ((currentScale >= state.scaleLimits[1]) && currentScale < state.scaleLimits[2]) {
                    detailLevel = "low";
    ...
    public GeneralUnitPNode getGeneralUnitPNodeAtPos(float x, float y) {
        return getGeneralUnitPNodeAtPos(x, y);
    }
