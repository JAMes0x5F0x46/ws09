 ...
 * @version $Id: QuantizationErrorVisualizer.java 2874 2009-12-11 16:03:27Z frank $
 */
        ...
        try {
            for (int y = 0; y < gsom.getLayer().getYSize(); y++) {
                for (int x = 0; x < gsom.getLayer().getXSize(); x++) {
                    ...
                    if (u.getNumberOfMappedInputs() > 0) {
                        ci = (int) Math.round(((qe.getUnitQualities("qe")[u.getXPos()][u.getYPos()] - minQE) / (maxQE - minQE))
        ...
        try {
            for (int y = 0; y < gsom.getLayer().getYSize(); y++) {
                for (int x = 0; x < gsom.getLayer().getXSize(); x++) {
                    ...
                    if (u.getNumberOfMappedInputs() > 0) {
                        ci = (int) Math.round(((qe.getUnitQualities("mqe")[u.getXPos()][u.getYPos()] - minMQE) / (maxMQE - minMQE))
                                * (double) (palette.length - 1));
                        g.setPaint(palette[ci]);
                    } else {
                        g.setPaint(Color.WHITE);
