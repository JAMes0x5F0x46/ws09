 ...
 * @version $Id: SentenceParser.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SentenceParser {
    ...
    public void find_parse_Document(String name) {
        try {
            File file = new File(prefix + "/" + java.net.URLDecoder.decode(name));
            filenames.add(name);
            ...
            if (file.exists()) {
                ...
                System.out.println("sentences in document " + file + ": " + (this.parsedDocuments[numDocs - 1].size() - 1));
...
class DocumentParser {
    ...
    public void storeText() {
        try {
            ...
            while (line != null) {
                if (!line.equals("")) {
                    if ((line.indexOf(".", 0) != -1) || (line.indexOf(";", 0) != -1) || (line.indexOf("?", 0) != -1) || (line.indexOf("!", 0) != -1)) {
                        sentence = sParser.findSentence(sParser.delete_tags(sentence + " " + line), this.document);
                    } else {
                        sentence = sentence + " " + line;
                    }
