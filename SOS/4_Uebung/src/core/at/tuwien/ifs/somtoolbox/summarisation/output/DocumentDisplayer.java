 ...
 * @version $Id: DocumentDisplayer.java 2936 2009-12-15 00:40:02Z mayer $
 */
public class DocumentDisplayer {
    ...
    public DocumentDisplayer(CommonSOMViewerStateData st, JScrollPane pane, ArrayList<String>[] parsedDoc, Object[] items) {
        this.state = st;
        this.inputvectors = (SOMLibSparseInputData) state.growingSOM.getSharedInputObjects().getData(SOMVisualisationData.INPUT_VECTOR);
     ...
     * displays results
     */
    public void showResult(int docID, ArrayList<String> resultdoc, DoubleArrayList resultscores, boolean scores, int highl, boolean filename) {
        try {
            // display title
            if (filename) {
            ...
            for (int i = 0; i < resultdoc.size(); i++) {
                ...
                if (scores) {
                    ...
                    doc.insertString(doc.getLength(), StringUtils.format3FractionDigits.format(resultscores.get(i)) + newline,
    ...
    public void showAllResults(ArrayList<String> result, DoubleArrayList resultscores, ArrayList<String> resultItemnames, boolean scores, int highl,
            boolean filename) {
        try {
            if (result.size() == 0) {
                ...
                for (int i = 0; i < result.size(); i++) {
                    ...
                    if (scores) {
                        doc.insertString(doc.getLength(), "scores: ", doc.getStyle("regular"));
                        doc.insertString(doc.getLength(), StringUtils.format3FractionDigits.format(resultscores.get(i)), doc.getStyle("underline"));
                        if (!filename) {
                            doc.insertString(doc.getLength(), newline, doc.getStyle("regular"));
                        }
                    }
     ...
     */
    public void showOriginal(ArrayList<String> result, DoubleArrayList[] allscores, boolean scores, int highl, boolean filename) {
     ...
     * @param filename
     */
    public void showOriginal(int docID, ArrayList<String> result, DoubleArrayList allscores, boolean scores, int highl, boolean filename) {
        try {
            // display title
            if (filename) {
                doc.insertString(doc.getLength(), this.parsedDocuments[docID].get(0) + " ", doc.getStyle("bold"));
            ...
            for (int i = 1; i < fulltext.size(); i++) {
                ...
                if (scores) {
                    doc.insertString(doc.getLength(), "scores: ", doc.getStyle("regular"));
                    doc.insertString(doc.getLength(), StringUtils.format3FractionDigits.format(allscores.get(i - 1)) + newline,
                            doc.getStyle("underline"));
                }
            }
            doc.insertString(doc.getLength(), newline, doc.getStyle("regular"));
            ...
            for (int i = 0; i < text.size(); i++) {
                ...
                if (sentence.indexOf(searchString) != -1 && !str.equals("")) {
                    ...
                    while (sentence.indexOf(searchString, beginning) != -1) {
                        ...
                        doc.insertString(doc.getLength(), (text.get(j)).substring(beginning, searchindex), doc.getStyle("regular"));
                        doc.insertString(doc.getLength(), (text.get(j)).substring(searchindex, searchindex + searchString.length()),
                                doc.getStyle("highlight"));
                        beginning = searchindex + searchString.length();
                    }
                    doc.insertString(doc.getLength(), (text.get(j)).substring(beginning) + newline, doc.getStyle("regular"));
                } else {
            ...
            while (st.hasMoreElements()) {
                ...
                if (sent_index <= sent.length()) {
                    if (score <= border) {
                        ...
                        doc.insertString(doc.getLength(), sent.substring(sent_index, begin + token.length() + 1), doc.getStyle("highlight_1"));
                        sent_index = begin + token.length() + 1;
                    } else {
                        tempborder = border * 2;
                        if (score <= tempborder) {
                            int begin = sent.indexOf(token, sent_index);
                            doc.insertString(doc.getLength(), sent.substring(sent_index, begin + token.length() + 1), doc.getStyle("highlight_2"));
                            sent_index = begin + token.length() + 1;
                        } else {
                            tempborder = border * 3;
                            if (score <= tempborder) {
                                int begin = sent.indexOf(token, sent_index);
                                doc.insertString(doc.getLength(), sent.substring(sent_index, begin + token.length() + 1), doc.getStyle("highlight_3"));
                                sent_index = begin + token.length() + 1;
                            } else {
                                tempborder = border * 4;
                                if (score <= tempborder) {
                                    ...
                                    doc.insertString(doc.getLength(), sent.substring(sent_index, begin + token.length() + 1),
                                            doc.getStyle("highlight_4"));
                                    sent_index = begin + token.length() + 1;
                                } else {
                                    int begin = sent.indexOf(token, sent_index);
                                    doc.insertString(doc.getLength(), sent.substring(sent_index, begin + token.length() + 1),
                                            doc.getStyle("highlight_5"));
                                    sent_index = begin + token.length() + 1;
                                }
                            }
                        }
    ...
    /**
     * highlights sentence with a color according to its score
     * 
     * @param docID
     * @param sent
     * @param score
