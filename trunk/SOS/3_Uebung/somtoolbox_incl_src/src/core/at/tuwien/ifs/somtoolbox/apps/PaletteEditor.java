 ...
 * @author Rudolf Mayer
 * @author Jakob Frank
 * @version $Id: PaletteEditor.java 2874 2009-12-11 16:03:27Z frank $
 */
public class PaletteEditor extends JFrame {
    ...
    public PaletteEditor(SOMViewer somViewer, CommonSOMViewerStateData state) {
        ...
        setPalette(state.palettes[state.currentPalette]);
        cmbPalettes.setSelectedIndex(state.currentPalette);
    }
    ...
    private JPanel getEmbeddedPaletteSelector() {
        ...
        cmbPalettes = new JComboBox(state.palettes);
        cmbPalettes.setSelectedIndex(state.currentPalette);

        cmbPalettes.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                setPalette(state.palettes[cmbPalettes.getSelectedIndex()]);
        ...
        if (btnClose == null) {
            ...
            btnClose.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    System.out.println("Palette: " + getPaletteDrawingPanel().getSize().width + "x" + getPaletteDrawingPanel().getSize().height);
                    System.out.println("Colors: " + getColorPanel().getSize().width + "x" + getColorPanel().getSize().height);
        ...
        if (btnNew == null) {
            ...
            btnNew.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (mode == SOMVIEWER) {
                        if (state != null) {
                            Palette[] pn = new Palette[state.palettes.length + 1];
                            for (int i = 0; i < state.palettes.length; i++) {
                                pn[i] = state.palettes[i];
                            }
                            pn[pn.length - 1] = p;
                            state.palettes = pn;
                            cmbPalettes.setModel(new JComboBox(state.palettes).getModel());
                ...
                public void actionPerformed(ActionEvent e) {
                    if (mode == SOMVIEWER) {
                        if (cmbPalettes != null && state != null) {
                            state.palettes[cmbPalettes.getSelectedIndex()] = getPalette();
                            state.currentPalette = cmbPalettes.getSelectedIndex();
                            if (somViewer != null) {
     ...
     * 
     * @param args <c>args[0]</c> can be a xml-Palette-file {@link Palette#savePaletteToXML(File)} which will be loaded at startup.
     */
        ...
        button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (btnAddColor.isSelected()) {
                    ...
                    JDialog jd = JColorChooser.createDialog(PaletteEditor.this, "Choose a color", true, jcc, new ActionListener() {
                        public void actionPerformed(ActionEvent e) {
     ...
     * 
     * @param p The Palette, which MUST NOT be <c>null</c>
     */
            ...
            if (p.getGradient() != null) { // gradient palette
                ...
                for (int i = 0; i < cg.getNumberOfPoints(); i++) {
                    ...
                    if (i > 0 && i < cg.getNumberOfPoints() - 1) {
                        ...
                        colorPanel.add(createGradientSpinner(i, cg.getGradientPoint(i), cg.getGradientPoint(i - 1), cg.getGradientPoint(i + 1)),
                                spinnerConstr);
                    } else {
                        JLabel l = new JLabel("" + StringUtils.format3FractionDigitsWithZeros.format(cg.getGradientPoint(i)));
