 ...
 * @version $Id: ControlFrame.java 2874 2009-12-11 16:03:27Z frank $
 */
public class ControlFrame extends JFrame implements MouseListener {
    ...
    public ControlFrame(LinkedHashMap<String, PlaybackThread> lhm, ActivityGridModel mod, JButton btn_start_on_panel) {
        ...
        try {
            ...
            int iResultX = Math.min(gr_activity.getSize().width + iAdditionalX + insDlg.left + insDlg.right, screenSize.width);
            int iResultY = Math.min(gr_activity.getSize().height + iAdditionalY + insDlg.top + insDlg.bottom, screenSize.height - 50);
    ...
    private void jbInit() throws Exception {
        ...
        this.getContentPane().add(btn_pause,
                new GridBagConstraints(0, 1, 1, 2, 0.0, 0.0, GridBagConstraints.SOUTHWEST, GridBagConstraints.NONE, new Insets(0, 10, 10, 10), 0, 0));
        this.getContentPane().add(btn_stop,
                new GridBagConstraints(1, 1, 1, 2, 0.0, 0.0, GridBagConstraints.SOUTHWEST, GridBagConstraints.NONE, new Insets(0, 10, 10, 10), 0, 0));
        this.getContentPane().add(lb_Playtime1,
                new GridBagConstraints(2, 1, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 10), 0, 0));
        this.getContentPane().add(lb_songs1,
                new GridBagConstraints(2, 2, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.getContentPane().add(lb_Playtime2,
                new GridBagConstraints(3, 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.getContentPane().add(lb_songs2,
                new GridBagConstraints(3, 2, 1, 1, 0.0, 0.0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
        this.getContentPane().add(jScrollPane1,
                new GridBagConstraints(0, 0, 4, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 10, 0), 0, 0));
    ...
    public void btn_pause_actionPerformed(ActionEvent actionEvent) {
        if (paused) {
            ...
            while (iter.hasNext()) {
                ((PlaybackThread) iter.next().getValue()).resume_playback();
            }
            tuThread.end_pause();
            btn_pause.setText("Pause");
            btn_pause.setIcon(ii_pause);
            ...
            Iterator<Entry<String, PlaybackThread>> iter = lhmThreads.entrySet().iterator();
            while (iter.hasNext()) {
                ((PlaybackThread) iter.next().getValue()).pause_playback();
            }
    ...
    public void updateTime(long lPausesDuration) {
        ...
        lb_Playtime2.setText("" + Commons.sdfHHmmss.format(new Date(currentTime - Commons.lStarttime - lPausesDuration)));
    ...
    public void setVisible(boolean parm1) {
        if (parm1) {
            ...
            Iterator<Entry<String, PlaybackThread>> iter = lhmThreads.entrySet().iterator();
            while (iter.hasNext()) {
                ((PlaybackThread) iter.next().getValue()).start();
            ...
            Iterator<Entry<String, PlaybackThread>> iter = lhmThreads.entrySet().iterator();
            while (iter.hasNext()) {
                ((PlaybackThread) iter.next().getValue()).stop_playback();
            }
            lhmThreads.clear();
