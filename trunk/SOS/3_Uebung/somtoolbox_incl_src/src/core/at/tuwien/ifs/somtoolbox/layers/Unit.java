...
/**
 * Represents a unit on a map. It has a position in terms of x and y coordinates and an n-dimensional weight vector. Data can be mapped onto a
 * <code>Unit</code>. Labels can be assigned to a <code>Unit</code> to describe the mapped data. A <code>Unit</code> can also have an assigned map for
 * use in hierarchical models.<br>
 * TODO: The type of <code>mappedSOM</code> should be made more general than it is now. FIXME: similar to what should be done with
 * {@link GrowingLayer}, we should make a Unit3D version out of this for 3D SOMs, to keep the memory fingerprint small, and avoid overloading many
 * methods with 2D and 3D params, which makes it very hard to read
 * 
 * @author Michael Dittenbach
 * @version $Id: Unit.java 2874 2009-12-11 16:03:27Z frank $
 */
public class Unit extends InputContainer {
    ...
    /**
     * Constructs a <code>Unit</code> on <code>Layer</code> specified by argument <code>layer</code> at position <code>x</code>/<code>y</code> with a
     * given weight vector <code>vec</code>.
     * 
    ...
    /**
     * Constructs a <code>Unit</code> on <code>Layer</code> specified by argument <code>layer</code> at position <code>x</code>/<code>y</code> with a
     * randomly initialized weight vector of dimension <code>d</code>. Argument <code>norm</code> determines whether the weight vector should be
     * normalized or not. TODO: This might be change in the future due to unflexibility regarding hard coded normalization methods.
     * 
     * @param l the layer that contains this <code>Unit</code>.
     * @param x the horizontal position on the <code>layer</code>.
     * @param y the vertical position on the <code>layer</code>.
    ...
    /**
     * Convenience method to add an input datum specified by its name and distance. The quantization error is recalculated if argument
    ...
    /**
     * Restores the labels of a unit based on the information provided by the arguments. The value of argument <code>nrUnitLabels</code> must be equal
     * to the dimensionalities of the arrays specified in the other arguments. If this is not the case, no labels will be restored.
     * 
    ...
    /**
     * Restores the mapped input data of a unit based on the information provided by the arguments. The value of argument <code>nrVecsMapped</code>
     * must be equal to the dimensionalities of the arrays specified in the other arguments. If this is not the case, no input data will be restored.
     * The quantization error will also be recalculated.
     * 
     * @param nrVecsMapped the number of input data.
     * @param mappedVecs an array of strings containing the data identifiers.
     * @param mappedVecsDist an array of double values containing the distances between the weight vector and the respective input data.
     */
    ...
    public String printUnitDetails(InputData inputData, TemplateVector tv) {
        StringBuffer sb = new StringBuffer("Unit details for ").append(getXPos()).append("/").append(getYPos()).append(", ").append(
        ...
        if (tv != null) {
            ...
            for (int i = 0; i < tv.dim(); i++) {
                String label = tv.getLabel(i).length() >= numSpace ? tv.getLabel(i).substring(0, numSpace - 1) : tv.getLabel(i);
        ...
        if (featureWeights != null) {
            sb.append(headerFeatureWeight).append(StringUtils.getSpaces(firstColumnSpace - headerFeatureWeight.length())).append(
        ...
        if (getNumberOfMappedInputs() > 0) {
            for (String label : getMappedInputNames()) {
                ...
                if (inputData != null && inputData.getInputDatum(label) != null) {
                    sb.append(StringUtils.toStringWithPrecision(inputData.getInputDatum(label).getVector().toArray(), numDigits)).append("\n");
                }
            }
        }
        return sb.toString();
