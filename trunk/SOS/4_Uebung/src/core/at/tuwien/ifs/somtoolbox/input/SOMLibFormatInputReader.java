 ...
 * @author Rudolf Mayer
 * @version $Id: SOMLibFormatInputReader.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMLibFormatInputReader extends AbstractSOMInputReader {
     ...
     */
    public SOMLibFormatInputReader(String weightVectorFileName, String unitDescriptionFileName, String mapDescriptionFileName)
            throws FileNotFoundException, SOMLibFileFormatException {
        // test if we can read all files, to prevent waiting a few minutes on reading the weight-vector file, if then the unit file can't be read
    ...
    public String getFilePath() {
        String res = "";
        if (weightVectorFileName != null) {
            res = weightVectorFileName.substring(0, weightVectorFileName.lastIndexOf(System.getProperty("file.separator")) + 1);
        } else if (unitDescriptionFileName != null) {
            res = unitDescriptionFileName.substring(0, unitDescriptionFileName.lastIndexOf(System.getProperty("file.separator")) + 1);
        } else {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").severe("You should not have come here. Something went awfully wrong. Aborting.");
        }
        return res;
    }
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.startsWith("$TYPE")) {
                    ...
                    if (lineElements.length > 1) {
                        ...
                        throw new SOMLibFileFormatException("Map description file format corrupt: no x-dimension specified in line '" + line + "'.");
                    }
                    ...
                    if (lineElements.length > 1) {
                        ...
                        throw new SOMLibFileFormatException("Map description file format corrupt: no y-dimension specified in line '" + line + "'.");
                    }
                    ...
                    if (lineElements.length > 1) {
                        ...
                        throw new SOMLibFileFormatException("Map description file format corrupt: no z-dimension specified in line '" + line + "'.");
                    }
                    ...
                    if (lineElements.length > 1) {
                        ...
                        throw new SOMLibFileFormatException("Map description file format corrupt: no vector dimension specified in line '" + line
                                + "'.");
                    }
                    ...
                    if (lineElements.length > 1) {
                        ...
                        throw new SOMLibFileFormatException("Map description file format corrupt: no metric specified in line '" + line + "'.");
                    }
    ...
    protected void readUnitDescriptionFile(String fileName) throws FileNotFoundException, SOMLibFileFormatException {
            ...
            while (!((line = br.readLine()).startsWith("$POS"))) {
                ...
                if (line == null) {
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        throw new SOMLibFileFormatException("Unit description file corrupt. Unknown FILE_FORMAT_VERSION");
                    }
                } else if (line.startsWith("$XDIM")) {
                    String[] lineElements = line.split(StringUtils.REGEX_SPACE_OR_TAB);
                    if (lineElements.length > 1) {
        ...
        try {
            StdErrProgressWriter progressWriter = new StdErrProgressWriter(xSize * ySize * zSize, "Reading info for unit ", 10);
            // line = br.readLine();
            while (line != null) {
                if (line.startsWith("$POS_X")) { // new unit
                    ...
                    if (line.startsWith("$POS_Y")) {
                        ...
                        if (lineElements.length > 1) {
                            ...
                            throw new SOMLibFileFormatException("Unit description file format corrupt, no $POS_Y specified in '" + line + "'.");
                        }
                    } else {
                        throw new SOMLibFileFormatException("Unit description file format corrupt. $POS_Y must follow immediately after $POS_X.");
                    }
                    line = br.readLine();
                    ...
                    if ((nrVecMapped > 0 && mappedVecsDistLength != nrVecMapped)) {
                        throw new SOMLibFileFormatException("Number of items in $MAPPED_VECS_DIST (" + mappedVecsDistLength
                                + ") doesn't equal $NR_VEC_MAPPED (" + nrVecMapped + ") in " + unitID);
                    }
    ...
    private void readWeightVectorFile(String fileName) throws FileNotFoundException, SOMLibFileFormatException {
        ...
        try {
            ...
            StdErrProgressWriter progressWriter = new StdErrProgressWriter(numUnits, "Reading weight vector of unit ", 10);
            for (int k = 0; k < zSize; k++) {
                for (int j = 0; j < ySize; j++) {
                    for (int i = 0; i < xSize; i++) {
                        if (line == null) {
                        ...
                        line = br.readLine();
                        progressWriter.progress();// "Reading weight vector of unit " + i + "/" + j + "/" + k + ", ", ((j * xSize) + i + 1));
                    }
                }
            }
            // if ((xSize!=props.xSize()) || (ySize!=props.ySize())) {
            // Logger.getLogger("at.tuwien.ifs.somtoolbox").info("Map size given in properties file differs from actual map size. Ignoring values in
            // properties file.");
            // }
            br.close();
        } catch (IOException e) {
