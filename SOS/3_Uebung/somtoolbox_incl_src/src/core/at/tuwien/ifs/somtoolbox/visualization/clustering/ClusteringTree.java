 ...
 * @version $Id: ClusteringTree.java 2874 2009-12-11 16:03:27Z frank $
 */
public class ClusteringTree extends PNode implements Serializable {
    ...
    /**
     * Recreates the ClusterElementsStorageNode for layer l. Replaces allClusteringElements with a new HashMap containing all sticky layers from the
     * previous version and the newly created layer l.
     * 
     * @param l the number of clusters
            ...
            for (ClusterNode oneCluster : clusterStorage) {
                ...
                if (tmpLabels != null && tmpLabels.length > 0) {
                    ...
                    if (bigLabel == null) {
                        ...
                        int i;
                        if ((oneCluster.getWithValue() != state.labelsWithValues) || (oneCluster.getFactorValue() != state.clusterByValue)) {
    ...
    public void dumpAllClusteringElements() {
        ...
        for (Iterator<Integer> it = allClusteringElements.keySet().iterator(); it.hasNext();) {
            Integer i = it.next();
            System.out.print(i + ": ");
            for (Iterator<PNode> itSNs = allClusteringElements.get(i).clusterLabels.iterator(); itSNs.hasNext();) {
                PText label = (PText) (itSNs.next()).getChild(0);
    ...
    /**
     * Resizes all labels so that labels of a lower layer are 1/3 smaller than the one above. The maximum font size/border width is set by the
     * constructor.
     */
    ...
    /**
     * Searches the clusters that are less or equal lvl and stores their children (ClusterNodes) which are > lvl. Used to get a clustering into lvl+1
     * clusters.
     * 
     * @param lvl
     ...
     */
    private void getAllChildrenUntil(int lvl, ClusterNode start, ArrayList<ClusterNode> store) {
        if (start.getLevel() > lvl) {
            ...
            getAllChildrenUntil(lvl, start.getChild1(), store);
            getAllChildrenUntil(lvl, start.getChild2(), store);
        }
    }
    ...
    public ClusterNode findClusterOf(GeneralUnitPNode unitPNode, int level) {
        ...
        for (ClusterNode clusterNode : store) {
            if (clusterNode.containsNodes(unitPNode)) {
                return clusterNode;
            }
        }
        return null;
    ...
    public void addEditLabelEventListenerToAll() {
        for (Iterator<ClusterElementsStorage> it = allClusteringElements.values().iterator(); it.hasNext();) {
            ArrayList<PNode> labelsArray = it.next().clusterLabels;
    ...
    /**
     * Changes the colors of the tree according to the currently chosen palette. In each step the palette is split in 2 halves and each child cluster
     * gets one half. The color of a cluster shown on the screen is the color "in the middle" of its palette. This means in the worst case there are
     * 2^n colors needed to paint n clusters, but this also means that close clusters will have more similar colors than outliers.
     */
    ...
    public Color[] getPalette() {
        CommonSOMViewerStateData state = CommonSOMViewerStateData.getInstance();
        return state.palettes[state.currentPalette].getColors();
    }
    ...
    public int[][] getClusterAssignment(int level, int xSize, int ySize) {
        ...
        for (ClusterNode curcluster : list) {
            ...
            for (int i = 0; i < unitNodes.length; i++) {
                int x = unitNodes[i].getUnit().getXPos();
                int y = unitNodes[i].getUnit().getYPos();
