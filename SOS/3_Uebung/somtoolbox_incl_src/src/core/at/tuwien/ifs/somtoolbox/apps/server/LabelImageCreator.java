 ...
 * 
 * @author Rudolf Mayer
 * @version $Id: LabelImageCreator.java 2874 2009-12-11 16:03:27Z frank $
 */
public class LabelImageCreator {
    ...
    public static void main(String[] args) throws ParserConfigurationException, SAXException, IOException, SOMToolboxException {
        ...
        JSAPResult config = OptionFactory.parseResults(args, OptionFactory.getOptLabelFileName(true), OptionFactory.getOptOutputDirectory(true));
     ...
     */
    public void writeImages(String labelFileName, String outputDirectory) throws ParserConfigurationException, SAXException, IOException,
            SOMToolboxException {
        ...
        for (int i = 0; i < levels; i++) {
            ...
            int xSize = 44;
            int ySize = 32;
            BufferedImage image = new BufferedImage((int) (xSize * CELL_SIZE_VIEWER * d), (int) (ySize * CELL_SIZE_VIEWER * d),
            ...
            for (int j = end; j >= start; j--) {
                ...
                for (int k = 0; k < list.size(); k++) {
                    LabelDescription label = (LabelDescription) list.get(k);
                    if (label.isVisible()) {
                        ...
                        for (int index = 0; index < split.length; index++) {
                            g.drawString(split[index], label.getX(scale), label.getYBaseline(g.getFontMetrics(), scale, index));
                        }
                    } else {
            ...
            ExportUtils.saveImageToFile(outputDirectory + "/label-zoom" + (i + 1) + ".png", image);
        }
    }
