...
/**
 * Merges two or more SOMLib Input files, i.e. vector and template files. Template vectors can be off different dimensionality, and may contain
 * different features, but some features may also be overlapping. Different merge strategies are available: union of all features sets, intersection
 * of features sets, and strategies in between, retaining a feature if it appears in at least x sets. This class is a bit similar to
 * {@link VectorFileMerger}, but more generic in the number of inputs it can take, and in the merging strategies, though it doesn't support the
 * weighting strategies implemented in {@link VectorFileMerger}.
 * 
 * @author Rudolf Mayer
 * @version $Id: SOMLibInputMerger.java 2874 2009-12-11 16:03:27Z frank $
 */
...
public class SOMLibInputMerger  implements SOMToolboxApp {
    ...
    public static void main(String[] args) throws IOException, SOMLibFileFormatException {
        JSAPResult config = OptionFactory.parseResults(args, OptionFactory.getOptMergeMode(), OptionFactory.getOptInputDirectory(false),
                OptionFactory.getOptOutputFileName(true), OptionFactory.getOptSOMLibInputs(false));

        String[] inputTvs = FileUtils.findAllSOMLibFiles(config, "inputs", "inputDir", ".tv", ".vec");
        String[] inputVecs = inputTvs;
        mergeVectors(inputTvs, inputVecs, config.getString("output"), config.getString("mode"), null);
    }

    public static void mergeVectors(final String[] inputTvs, final String[] inputVecs, String outFile, String modeString, int[] size)
            throws IOException, SOMLibFileFormatException {
        int percentage = -1;
        mergeMode mode = mergeMode.All;
        if (mode != null) {
            if (modeString.equals(mergeMode.Intersection.toString())) {
                mode = mergeMode.Intersection;
            } else if (modeString.equals(mergeMode.Union.toString())) {
                mode = mergeMode.Union;
            } else if (modeString.equals(mergeMode.All.toString())) {
            } else {
                try {
    ...
    private static void merge(final String[] inputTvs, final String[] inputVecs, String outFile, int percentage, mergeMode mode, int[] size)
            throws IOException, SOMLibFileFormatException {
        System.out.println("\n\nStarting feature merging, mode: " + mode + (mode == mergeMode.MinOccurence ? ", min occurrence: " + percentage : "")
                + "\n\n");
        ...
        // e.g. if those were generated from transforming a SOM's weight-vectors to an input vector file we need to re-calculate the number of vectors
        totalVectorCount = 0;
        for (String inputVec : inputVecs) {
            final Map<String, String> headers = FileUtils.readSOMLibFileHeaders(FileUtils.openFile("Input Vector File", inputVec
                    + InputData.inputFileNameSuffix));
        ...
        mergedTV.writeToFile(outFile + ".tv");
        ...
        AbstractSOMLibSparseInputData.writeHeaderToFile(writer, totalVectorCount, mergedFeatures.size());
        ...
        for (int i = 0; i < inputVecs.length; i++) {
            ...
            for (int j = 0; j < inputData.numVectors(); j++) {
                ...
                AbstractSOMLibSparseInputData.writeInputDatumToFile(writer, inputDatum.getLabel(), newVector);
                writer.flush();
        ...
        classInfo.writeToFile(outFile + ".cls");
        ...
        try {
            new SOMProperties(size[0], size[1], SOMLibSparseInputData.DEFAULT_RANDOM_SEED, 20, 0, SOMProperties.defaultLearnRate, -1d, 1d,
                    "MissingValueMetricWrapper", false).writeToFile(outFile, ".", true);
