 ...
 * 
 * @author Thomas Lidy
 * @version $Id: SmoothedCountHistograms.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SmoothedCountHistograms extends AbstractMatrixVisualizer {
    ...
    public BufferedImage createVisualization(int index, GrowingSOM gsom, int width, int height) throws SOMToolboxException {
        ...
        int ci = 0;
        double paletteLim = (double) (palette.length - 1);
        ...
        if (index == 0) /* flat histogram color image */
        {
            ...
            for (int x = 0; x < xSize; x++) {
                for (int y = 0; y < ySize; y++) {
                    ...
                    smallimg.setRGB(x, y, palette[ci].getRGB());
                }
            }
        ...
        } else /* index == 1 ... sdh-like image */
        {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ci = (int) Math.round(bcs.interpolate((double) x + 0.5, (double) y + 0.5) * paletteLim);
                    // limit ci value between 0 & max value
                    ci = Math.min(palette.length - 1, Math.max(0, ci));

                    // check for mnemonic SOM: if a unit is empty and has a low value, or the unit has no neighbours
                    // if ((ci < 3 && gsom.getLayer().getUnit(x/unitWidth, y/unitHeight) == null) || !gsom.getLayer().hasNeighbours(x/unitWidth,
                    // y/unitHeight)) {
                    ...
                    res.setRGB(x, y, palette[ci].getRGB());
                }
            }
