...
/**
 * @author Doris Baum
 * @version $Id: QuiverPNode.java 2874 2009-12-11 16:03:27Z frank $
 */
public class QuiverPNode extends PNode implements UnitSelectionListener {
    ...
    public void computeArrows() {
        ...
        try {
            ...
            for (int i = 0; i < allShifts.size(); i++) {
                ...
                currentArrow = new ArrowPNode((currentShift.getX1() + 0.5) * map.getUnitWidth(), (currentShift.getY1() + 0.5) * map.getUnitHeight(),
                        secMapXOffset + ((currentShift.getX2() + 0.5) * map2.getUnitWidth()), secMapYOffset
    ...
    public void updateArrowTypeVisibility() {
        for (int i = 0; i < this.getChildrenCount(); i++) {
            ...
            if ((stableArrows && (curArrow.getType() == ArrowPNode.STABLE)) || (adjacentArrows && (curArrow.getType() == ArrowPNode.ADJACENT))
                    || (outlierArrows && (curArrow.getType() == ArrowPNode.OUTLIER)) || (clusterArrows && (curArrow.getType() == ArrowPNode.CLUSTER))) {
