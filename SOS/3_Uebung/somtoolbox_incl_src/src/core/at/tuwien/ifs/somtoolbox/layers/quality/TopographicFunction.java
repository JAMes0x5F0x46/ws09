 ...
 * More Infos: T. Villmann, R. Der, M. Herrmann, and T.M. Martinez. Topology preservation in self- organizing feature maps: Exact definition and
 * measurement.In IEEE Transactions on Neural Networks 8, pages 256-266, 1997.
 * 
 * @author Gerd Platzgummer
 * @version $Id: TopographicFunction.java 2874 2009-12-11 16:03:27Z frank $
 */
                ...
                if (adj[spalte][zeile] != 1) {
                    ...
                    while (!erreichbaren.isEmpty()) {
                        ...
                        for (int i = 0; i < erreichbaren.size(); i++) {
                            int minposcand = ((Integer) erreichbaren.get(i)).intValue();
                            int mincand = -1;
                        ...
                        if (minpos != -1) {// muss immer true sein
                            ...
                            if (minpos == spalte) {// wenn der Naechste schon der Zielknoten ist
                                ...
                                for (int i = 0; i < N; i++) {
                                    if ((i != minpos) && (i != zeile)) { // adj[i][minpos] fuer i==minpos ist immer Integer.MAX_VALUE, daher
                                        // unwichtig
