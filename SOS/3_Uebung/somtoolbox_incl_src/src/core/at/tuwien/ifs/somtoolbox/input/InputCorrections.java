...
public class InputCorrections {
    ...
    public class InputCorrection {
        ...
        public InputCorrection(Unit sourceUnit, Unit targetUnit, String label, double originalDistance, CreationType creationType) {
        ...
        public String getPrintString() {
            return label + " " + sourceUnit.getXPos() + "/" + sourceUnit.getYPos() + " " + targetUnit.getXPos() + "/" + targetUnit.getYPos();
        }
    ...
    public void readFromFile(String fileName, Layer layer, InputData data) throws SOMToolboxException {
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                double newDistance = data != null ? layer.getMetric().distance(target.getWeightVector(), data.getInputDatum(label).getVector()) : 0;

                corrections.add(new InputCorrection(source, target, label, source.getMappedInputDistance(label), CreationType.MANUAL));
    ...
    public InputCorrection get(int index) {
        return (InputCorrection) corrections.get(index);
    }
    ...
    public InputCorrection addComputedInputCorrection(Unit source, Unit target, String label, InputCorrections manualCorrections)
            throws SOMToolboxException {
        double originalDistance = manualCorrections.get(label) != null ? manualCorrections.get(label).getOriginalDistance()
                : source.getMappedInputDistance(label);
    ...
    public InputCorrection addManualInputCorrection(Unit source, Unit target, String label) throws SOMToolboxException {
        ...
        if (correction != null) {
            ...
            correction = new InputCorrection(source, target, label, source.getMappedInputDistance(label), CreationType.MANUAL);
            corrections.add(correction);
        }
        return correction;
    }
