 ...
 * @version $Id: PlaybackThread.java 2874 2009-12-11 16:03:27Z frank $
 */
public class PlaybackThread extends Thread {
    ...
    protected byte[] datalineBuffer = new byte[1 * 1024 * Constants.DATALINE_FORMAT.getFrameSize()]; // the data line buffer
    ...
    public PlaybackThread(String id, PlaybackThreadDataRecord record, SourceDataLine line, boolean repeat, float probalityToDecode,
            boolean updateStats, String decodedOutputDir) {
    ...
    /**
     * Convenience constructur that takes a {@link PlaybackThreadDataRecord} and pulls all data from it to create a new thread.<br>
    ...
    public PlaybackThread(String id, PlaybackThreadDataRecord record, SourceDataLine line, String decodedOutputDir) {
        ...
        try {
            ...
            for (int j = 0; j < record.listOfSongs.length; j++) {
                if (record.listOfSongs[j] != null) {
                    ...
                    for (int i = 0; i < record.listOfSongs[j].size(); i++) {
                        String currentSong = (String) record.listOfSongs[j].get(i);
                        file = new File(currentSong);
                        if (file.exists()) {
                            tempVector.add(file);
                        } else {
                            Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").warning(id + ": " + "File not found '" + currentSong + "'. Ignoring");
                        }
                    }
                }
                ...
                if (tempVector.size() >= 1) {
                    ...
                    Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").warning(id + ": " + "Channel " + j + ": No song.");
                    muteChannel(j);
                    // throw new Exception();
                }
            }
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").warning(id + ": " + "Could not create NodeThread '" + id + "'. " + ex.getMessage());
            ex.printStackTrace();
    ...
    public void decodingFailed(int channel, boolean stats) {
        // Try again
        try {
            Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").warning(id + ": " + "Decoding failed. Trying next song...");
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").warning(id + ": " + "Getting next song failed... Trying next song");
            ex.printStackTrace();
    ...
    public AudioInputStream getNextSong(int channel, boolean stats) throws IOException, UnsupportedAudioFileException {
        ...
        if (tempFile.getName().endsWith(".wav")) {
            ...
            if (PlaybackThread.decodedFiles.containsKey(tempFile)) {
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").fine(id + ": " + "found in HashMap, playing decoded version..");
            } else {
                // Is perhaps a decoded version already there?
                File tryDecodedFile = new File(DecoderThread.getDecodedFileName(tempFile, decodedOutputDir, Constants.DECODED_SUFFIX));
                if (tryDecodedFile.exists()) {
                    // use already decoded version (was not in Hashmap)
                    Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").fine(id + ": " + "Accidently found: " + tryDecodedFile + ". I'll use it.");
                    ...
                    if (PlaybackThread.rand.nextFloat() < probalityToDecode) {
                        // Decode
                        Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").info(id + ": " + "Decoding: " + tempFile);

                        // Change table model
                        if (positions[channel] != null) {
                            for (int i = 0; i < playbackListeners.size(); i++) {
                                PlaybackListener playbackListener = (playbackListeners.get(i));
                                playbackListener.setDecodingAt(positions[channel].x, positions[channel].y, tempFile.getName());
                            }
                        }
                        ...
                        DecoderThread decoderThread = new DecoderThread(this, tempFile, channel, stats, decodedOutputDir, Constants.DECODED_SUFFIX);
                        decoderThread.start();
                    } else {
                        // Play the mp3 file
    ...
    public AudioInputStream prepareAudioInputStream(File file, int channel, boolean stats) throws IOException, UnsupportedAudioFileException {
        ...
        Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").info(id + ": " + "Next song for channel " + channel + ": " + file);
        ...
        if (ready) {
            ...
            try {
                if (!empty[0]) {
                    ...
                    Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").fine(id + ": " + "Left channel empty, filling with 0");
                }
                if (!empty[1]) {
                    ...
                    Logger.getLogger("at.tuwien.ifs.somtoolbox.multichannel").fine(id + ": " + "Right channel empty, filling with 0");
                }
