...
/**
 * This class implements the Growing Hierarchical Self-Organizing Map. It is basically a wrapper for the {@link at.tuwien.ifs.somtoolbox.models.GrowingSOM}
 * and mainly handles command line execution and parameters. It implements the {@link at.tuwien.ifs.somtoolbox.models.NetworkModel} interface wich is
 * currently not used, but may be used in the future. It is also not clear, if this class will be removed and replaced by the <code>GrowingSOM</code>,
 * becaus it already contains the hierarchical functionality, only the training procedure would have to be updated.
 * 
 * @author Michael Dittenbach
 * @version $Id: GHSOM.java 2889 2009-12-12 00:31:37Z mayer $
 */
public class GHSOM extends AbstractNetworkModel implements SOMToolboxApp {
    ...
    public static final Parameter[] OPTIONS = new Parameter[] { OptionFactory.getSwitchHtmlOutput(false), OptionFactory.getOptLabeling(false),
            OptionFactory.getOptNumberLabels(false), OptionFactory.getOptNumberWinners(false), OptionFactory.getSwitchSkipDataWinnerMapping(),
    ...
    /**
     * Constructs a new <code>GHSOM</code> with <code>dim</code>-dimensional weight vectors. Argument <code>norm</code> determines whether the
    ...
    public GHSOM(int dim, boolean norm, GHSOMProperties props, InputData data) {
        layer0 = new GrowingLayer(1, 1, "at.tuwien.ifs.somtoolbox.layers.metrics.L2Metric", dim, norm, props.pca(), 7, data);
        try {
    ...
    /**
     * Constructs an already trained <code>GHSOM</code> with a <code>SOMInputReader</code> provided by argument <code>ir</code>.
     * 
    ...
    private ExpandedUnits getExpandedUnits(GrowingLayer layer, QualityMeasure qm, String qmName, double fraction, double totalQuality) {
            ...
            for (int j = 0; j < layer.getYSize(); j++) {
                for (int i = 0; i < layer.getXSize(); i++) {
                    Logger.getLogger("at.tuwien.ifs.somtoolbox").finest(
                            "unit (" + i + "/" + j + "): " + quality[i][j] + " > " + fraction + " * " + totalQuality + " (="
                                    + (fraction * totalQuality) + ")");
                    if ((quality[i][j] > (fraction * totalQuality)) && (layer.getUnit(i, j).getNumberOfMappedInputs() > 0)) {
    ...
    /**
     * Trains the GHSOM with the input data and training parameters specified in the properties provided by argument <code>props</code>.
     * 
     * @param data input data to train the map with.
     * @param props the training properties.
     */
    private void train(InputData data, GHSOMProperties props) {
        ...
        while (unitQueue.getNumElements() > 0) {
            ...
            GrowingSOM newGSOM = new GrowingSOM(++id, currentUnit, data.dim(), data.isNormalizedToUnitLength(), props, data);
