 ...
 * @version $Id: PseudoSilhouetteValue.java 2874 2009-12-11 16:03:27Z frank $
 */
public class PseudoSilhouetteValue extends AbstractQualityMeasure {
    ...
    public PseudoSilhouetteValue(Layer layer, InputData data) {
        ...
        StdErrProgressWriter progressWriter = new StdErrProgressWriter(layer.getXSize() * layer.getYSize(), "Calculating PseudoSilhouetteValue for unit ", 50);

        for (int y = 0; y < layer.getYSize(); y++) {
            for (int x = 0; x < layer.getXSize(); x++) {
                progressWriter.progress(y * layer.getYSize() + x);
                // logger.fine("handling unit: " + (x + 1) + " / " + (y + 1) + " of " + (layer.getXSize() * layer.getYSize()));
                Unit u = null;
                try {
                    u = layer.getUnit(x, y);
                ...
                if (u != null) {
                    ...
                    for (int i = 0; i < u.getNumberOfMappedInputs(); i++) {
                        ...
                        for (int j = 0; j < u.getNumberOfMappedInputs(); j++) {
                            ...
                            try {
                                double withinDistance = new L1Metric().distance(thisDatum.getVector().toArray(), thatDatum.getVector().toArray());
                        ...
                        logger.finer("\t minDist - avgDistanceWithinCluster / max: " + minDist + " - " + avgDistanceWithinCluster + " / " + max);
                    ...
                    for (int e = 0; e < tmpUnitSilhouetteValues.length; e++) {
                        ...
                        logger.finer("\t" + tmpUnitSilhouetteValues[e]);
                    }
