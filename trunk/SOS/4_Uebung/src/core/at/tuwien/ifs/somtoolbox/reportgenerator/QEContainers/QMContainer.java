 ...
 * @version $Id: QMContainer.java 2956 2009-12-16 11:19:34Z mayer $
 */
public class QMContainer {
     ...
     * Units (TRY: Enlarge the search radius to a certain neighbourhood radius (1))
     * 
     * @param type 1 = Max-Max, 2 = Min-Min, 3 = Max-Min, 4 = Min-Max Compares according to Min /Max Values return a Array [0] gives the percentage of
     *            similarity, [1] gives the absolute difference in strength between max/min 1/2, [2] the region of intersection(1-9)
     */
     ...
     * This is made upon an assumption, we say a unit is highly dense when the average value of the first 15% of units with highest values is equal or
     * above that value that
     */
    ...
    /**
     * returns a ArrayList with all classes for witch unit u resembles a Cluster Edge Unit for those classes. this need a clss file to b presents,
    ...
    /**
     * * Returns true if 8-fold neighbourhood shows any Units with no Inputs. Need a class file to b presents, returns null otherwise<br/>
    ...
    public Boolean hasNeighboringEmptyUnits(Unit u) {
        ...
        for (int x_new = -1; x_new <= 1; x_new++) {
            for (int y_new = -1; y_new <= 1; y_new++) {
                ...
                c[0] = u.getXPos() + x_new;
                c[1] = u.getYPos() + y_new;
                if (c[0] >= 0 && c[0] <= this.sGrid.XLength - 1 && c[1] >= 0 && c[1] <= this.sGrid.YLength - 1 && c[0] != u.getXPos()
                        && c[1] != u.getYPos()) {
                    SemanticNode s = this.getNode(c);
                    if (s != null) {
