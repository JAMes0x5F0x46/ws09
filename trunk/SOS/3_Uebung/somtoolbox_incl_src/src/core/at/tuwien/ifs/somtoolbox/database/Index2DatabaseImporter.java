 ...
 * 
 * @author Rudolf Mayer
 * @version $Id: Index2DatabaseImporter.java 2874 2009-12-11 16:03:27Z frank $
 */
public class Index2DatabaseImporter {
    public static final String[] templateFields = new String[] { "number", "label", "documentFrequency", "collectionTermFrequency",
    ...
    public static final String[] documentTermTableFields = new String[] { "documentNumber", "termNumber", "rawTermFrequency", "weight",
            "weightNormalised" };
    ...
    public Index2DatabaseImporter(String databaseServerAddress, String databaseName, String user, String password, String databaseTableNamePrefix)
    ...
    public void writeTemplateVector(IntMatrix tfMatrix, Vector<String> selectedTerms, HashMap<Integer, Object> allTerms) throws SQLException {
     ...
     */
    public void writeInputVector(IntMatrix tfMatrix, Vector<String> selectedTerms, HashMap<Integer, Object> allTerms, HashMap<Integer, Object> labelMap) throws SQLException {
        ...
        if (reduced) {
            StdErrProgressWriter progressDf = new StdErrProgressWriter(documentFrequencies.length, "Calculating df values ", 10);
            System.out.println("\n\nreduced, calcucalting hashmap\n");
            terms_hm_reverse = VectorTools.reverseHashMap(allTerms);
            for (int i = 0; i < selectedTerms.size(); i++) {
                progressDf.progress();
                int terminmatrix = ((Integer) terms_hm_reverse.get(selectedTerms.get(i))).intValue();
        ...
        for (int i = 0; i < tfMatrix.rows(); i++) {
            dbConnector.doInsert(dbConnector.getDocumentTableName(), documentTableFields, new Object[] { new Integer(i),
                    labelMap.get(new Integer(i)).toString() });
            ...
            if (reduced) {
                for (int j = 0; j < selectedTerms.size(); j++) {
                    int terminmatrix = ((Integer) terms_hm_reverse.get(selectedTerms.get(j))).intValue();
                    tfs[j] = tfMatrix.get(i, terminmatrix);
                    double fraq = (double) documentNumber / (double) documentFrequencies[terminmatrix];
                    weights[j] = (double) tfs[j] * Math.log(fraq);
                }
            } else {
            ...
            for (int termIndex = 0; termIndex < tfs.length; termIndex++) {
                // values[termIndex] = new Object[] { new Integer(i), new Integer(termIndex), new Double(tfs[termIndex]),
                ...
                // new Object[] { new Integer(i), new Integer(termIndex), new Double(tfs[termIndex]), new Double(weights[termIndex]),
                // new Double(weightsNormalised[termIndex]) });
                executeDocumentTermInsert(i, termIndex, tfs[termIndex], weights[termIndex], weightsNormalised[termIndex]);
            }
            ...
            // System.gc();
            // System.out.println("Ran garbage collection. Freed: " + StringUtils.readableBytes(rt.freeMemory() - mem) + ". Total in use: "
    ...
    public void writeInputVector(DoubleMatrix inputVectorMatrix, HashMap<Integer, Object> labelMap) throws SQLException {
        ...
        for (int i = 0; i < inputVectorMatrix.rows(); i++) {
            ...
            for (int termIndex = 0; termIndex < st.length; termIndex++) {
                dbConnector.doInsert(dbConnector.getDocumentTermsTableName(), documentTermTableFields, new Object[] { new Integer(i),
                        new Integer(termIndex), new Double(st[termIndex]) });
            }
