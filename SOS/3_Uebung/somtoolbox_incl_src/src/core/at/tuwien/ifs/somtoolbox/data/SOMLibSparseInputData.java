...
/**
 * Implements {@link InputData} based on a SOMLib <a href="http://olymp.ifs.tuwien.ac.at/somtoolbox/doc/somlibFileFormat.html#input_vectors">Input
 * Vector File</a>.
 * 
 * @author Michael Dittenbach
 * @version $Id: SOMLibSparseInputData.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMLibSparseInputData extends AbstractSOMLibSparseInputData {
    private static final String INPUT_VECTOR_FILE_FORMAT_CORRUPT_MESSAGE = "Input vector file format corrupt in vector number ";
     ...
     */
    protected SOMLibSparseInputData(DoubleMatrix2D data, String[] dataNames, boolean norm, Random rand, TemplateVector tv,
            SOMLibClassInformation clsInfo) {
        initFromExistingData(data, dataNames, norm, rand, tv, clsInfo);
    }

    private void initFromExistingData(DoubleMatrix2D data, String[] dataNames, boolean norm, Random rand, TemplateVector tv,
            SOMLibClassInformation clsInfo) {
        this.data = data;
    ...
    /**
     * Uses default values for sparsity (<code>true</code>), normalisation (<code>true</code>), chacheblocks (<code>1</code>) and seed (<code>7</code>
     * ).
     */
    ...
    public SOMLibSparseInputData(String vectorFileName, String templateFileName) {
        this(vectorFileName, templateFileName, DEFAULT_SPARSE, DEFAULT_NORMALISED, DEFAULT_NUM_CACHE_BLOCKS, DEFAULT_RANDOM_SEED);
    }
    ...
    public SOMLibSparseInputData(String vectorFileName, String templateFileName, boolean sparse, boolean norm, int numCacheBlocks, long seed) {
        ...
        if (dim != templateVector.dim()) {
            String errorMessage = "Dimensionalities in input vector file and template vector file differ (" + dim + " != " + templateVector.dim()
                    + ". Aborting.";
    ...
    public SOMLibSparseInputData(String vectorFileName, String templateFileName, String classInfoFileName) throws SOMToolboxException {
        this(vectorFileName, templateFileName, classInfoFileName, DEFAULT_SPARSE, DEFAULT_NORMALISED, DEFAULT_NUM_CACHE_BLOCKS, DEFAULT_RANDOM_SEED);
    }

    public SOMLibSparseInputData(String vectorFileName, String templateFileName, String classInfoFileName, boolean sparse, boolean norm,
            int numCacheBlocks, long seed) throws SOMToolboxException {
     ...
     * href="http://olymp.ifs.tuwien.ac.at/somtoolbox/doc/somlibFileFormat.html#input_vectors">Input Vector File</a> specification. Additionally
     * calculates the {@link AbstractSOMLibSparseInputData#meanVector} and creates the {@link AbstractSOMLibSparseInputData#nameCache} for faster
     * index search.
     * 
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (lineElements.length < 2) {
                    String msg = "Header in input vector file corrupt in line #" + lineNumber + ": less than two elements!";
                ...
                if (header.equals("$TYPE")) {
                    ...
                    if (content_type.equals("audio")) {
                        Logger.getLogger("at.tuwien.ifs.somtoolbox").info("Content type = audio. Setting sparsity to false.");
                        sparse = false;
                    }
            ...
            while (line != null) {
                if (!line.equals("")) {
                    ...
                    if (index >= numVectors) {
                        String errorMessage = "Input vector file format corrupt. Incorrect number of vectors: header says " + numVectors
                                + ", but already reading vector " + (index + 1) + ". Aborting.";
                        Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(errorMessage);
                    ...
                    if (lineElements.length != dim + 1) {
                        String msg = (INPUT_VECTOR_FILE_FORMAT_CORRUPT_MESSAGE + (index + 1)) + ": dimension specified is " + dim + ", found "
                        ...
                        try {
                            ...
                            String msg = (INPUT_VECTOR_FILE_FORMAT_CORRUPT_MESSAGE + (index + 1)) + " (line #" + lineNumber + "): " + e.getMessage()
                                    + ". Aborting.";
                            Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(msg);
                            throw new IOException(msg);
                        }
            ...
            if (index != numVectors) {
                String errorMessage = "Input vector file format corrupt. Incorrect number of vectors: header says " + numVectors + ", but read "
                        + index + ". Aborting.";
                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(errorMessage);
                throw new IOException(errorMessage);
            } else { // file is sane
    ...
    protected static BufferedReader openFile(String vectorFileName) {
        BufferedReader br;
        try {
            br = FileUtils.openFile("Input vector file", vectorFileName);
    ...
    protected void addInstance(int index, String label) {
        // avoid heading or trailing spaces --> can create problems with DB driven vectors that do not store those spaces
    ...
    public InputData subset(String[] names) {
        ...
        SOMLibSparseInputData res = new SOMLibSparseInputData(newData, names, isNormalized, rand, templateVector, classInfo);
