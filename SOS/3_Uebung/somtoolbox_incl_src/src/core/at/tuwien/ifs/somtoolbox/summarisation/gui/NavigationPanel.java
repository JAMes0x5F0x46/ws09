 ...
 * @author Rudolf Mayer
 * @version $Id: NavigationPanel.java 2932 2009-12-14 23:39:14Z mayer $
 */
public class NavigationPanel extends JPanel {
    ...
    public NavigationPanel(SummariserGUI sum, CommonSOMViewerStateData state, Object[] itemNames) {
        ...
        GridBagConstraintsIFS gc = new GridBagConstraintsIFS(GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL);
    ...
    void updateResults() {
        ...
        if (sdsActive) {
            ...
            documentdispl.clearResults();
            if (getSumClicked()) {
                documentdispl.showResult(selectedDoc, resulth.getResultDoc(selectedDoc), resulth.getResultScores(selectedDoc), isScores(),
                        getTypeHighlight(), false);
            } else {
                documentdispl.showOriginal(selectedDoc, resulth.getResultDoc(selectedDoc), resulth.getDocumentScores(selectedDoc), isScores(),
                        getTypeHighlight(), false);
            }
        } else {
            ...
            if (getSumClicked()) {
                if (mdsPanel.getMDSType() == MultiDocumentSummarisationPanel.MULTI_SDS) {
                    for (int i = 0; i < getFileNames().size(); i++) {
                        documentdispl.showResult(i, resulth.getResultDoc(i), resulth.getResultScores(i), isScores(), getTypeHighlight(),
                                isFileNames());
                    }
                    ...
                    documentdispl.showAllResults(resulth.getMultiResultDocs(), resulth.getMultiResultScores(), resulth.getMultiResultFilenames(),
            ...
            // else update in original text
                ...
                if (mdsPanel.getMDSType() == MultiDocumentSummarisationPanel.MULTI_SDS) {
                    for (int i = 0; i < getFileNames().size(); i++) {
                        ...
                        documentdispl.showOriginal(i, resulth.getResultDoc(i), resulth.getDocumentScores(i), isScores(), getTypeHighlight(),
                                isFileNames());
                    }
                } else if (mdsPanel.getMDSType() == MultiDocumentSummarisationPanel.MULTIDOC_SUM
                        || mdsPanel.getMDSType() == MultiDocumentSummarisationPanel.SIM_METHOD) {
                    // resulth.showOriginal(-1, score,highlight, filename_bool, true);
                    documentdispl.showOriginal(resulth.getMultiResultDocs(), resulth.getDocumentScores(), isScores(), getTypeHighlight(),
                            isFileNames());
                }
            }
        }
