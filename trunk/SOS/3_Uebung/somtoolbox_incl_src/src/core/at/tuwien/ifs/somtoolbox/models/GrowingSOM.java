...
/**
 * This class implements the Growing Self-Organizing Map. It is basically a wrapper for the {@link at.tuwien.ifs.somtoolbox.layers.GrowingLayer} and mainly
 * handles command line execution and parameters. It implements the {@link at.tuwien.ifs.somtoolbox.models.NetworkModel} interface wich is currently not
 * used, but may be used in the future.
 * 
 * @author Michael Dittenbach
 * @version $Id: GrowingSOM.java 2889 2009-12-12 00:31:37Z mayer $
 */
public class GrowingSOM extends AbstractNetworkModel implements SOMToolboxApp {

    public static final Parameter[] OPTIONS = new Parameter[] { OptionFactory.getSwitchHtmlOutput(false), OptionFactory.getOptLabeling(false),
            OptionFactory.getOptNumberLabels(false), OptionFactory.getOptWeightVectorFileInit(false), OptionFactory.getOptMapDescriptionFile(false),
            OptionFactory.getSwitchSkipDataWinnerMapping(), OptionFactory.getOptNumberWinners(false), OptionFactory.getOptProperties(true),
            OptionFactory.getOptUseMultiCPU(false) };
    ...
    public static void main(String[] args) {
        ...
        if (weightFileName == null) {
            som = new GrowingSOM(data.dim(), data.isNormalizedToUnitLength(), somProps, data);
        } else {
            try {
        ...
        som.setSharedInputObjects(new SharedSOMVisualisationData(null, null, null, null, fileProps.vectorFileName(true),
        ...
        try {
            SOMLibMapOutputter.write(som, fileProps.outputDirectory(), fileProps.namePrefix(false), true, somProps, fileProps);
        ...
        if (!skipDataWinnerMapping) {
            ...
            try {
                SOMLibMapOutputter.writeDataWinnerMappingFile(som, data, numWinners, fileProps.outputDirectory(), fileProps.namePrefix(false), true);
            } catch (IOException e) {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(
                        "Could not open or write to output file " + fileProps.namePrefix(false) + ": " + e.getMessage());
                System.exit(-1);
    ...
    /**
     * Constructs a new <code>GrowingSOM</code> with <code>dim</code>-dimensional weight vectors. Argument <code>norm</code> determines whether the
     * randlomy initialized weight vectors should be normalized to unit length or not.
     * 
     ...
     */
    public GrowingSOM(int dim, boolean norm, SOMProperties props, InputData data) {
        layer = new GrowingLayer(props.xSize(), props.ySize(), props.zSize(), props.metricName(), dim, norm, props.pca(), props.randomSeed(), data);

    }

    /**
     * Constructs a new <code>GrowingSOM</code> with <code>dim</code>-dimensional weight vectors. Argument <code>norm</code> determines whether the
     * randlomy initialized weight vectors should be normalized to unit length or not. In hierarchical network models consisting of multiple maps such
     * as the {@link GHSOM}, a unique identifier is assigned by argument <code>id</code> and the superordinate unit is provided by argument
     * <code>su</code>.
     * 
    ...
    public GrowingSOM(int id, Unit su, int dim, boolean norm, SOMProperties props, InputData data) {
        layer = new GrowingLayer(id, su, props.xSize(), props.ySize(), props.zSize(), props.metricName(), dim, norm, props.pca(), props.randomSeed(),
                data);
    }

    /**
     * Private constructor used recursively in hierarchical network models consisting of multiple maps. A unique identifier is assigned by argument
     * <code>id</code> and the superordinate unit is provided by argument <code>su</code>.
     * 
     * @param id a unique identifier used in hierarchies of maps (e.g. the <code>GHSOM</code>).
     * @param su the superordinate unit of the map.
    ...
    protected GrowingSOM(int id, Unit su, SOMInputReader ir) {
        ...
        try {
            ...
            layer = new GrowingLayer(id, su, ir.getXSize(), ir.getYSize(), ir.getZSize(), ir.getMetricName(), ir.getDim(), ir.getVectors(), 7);
        ...
        try {
            for (int j = 0; j < layer.getYSize(); j++) {
                for (int i = 0; i < layer.getXSize(); i++) {
                    ...
                    if (layer.getUnit(i, j, 0) == null) { // if this unit is empty, i.e. not part of the mnemonic map --> we skip it
                        ...
                        layer.getUnit(i, j, 0).restoreMappings(ir.getNrVecMapped(i, j), ir.getMappedVecs(i, j), ir.getMappedVecsDist(i, j));
                        layer.getUnit(i, j, 0).restoreLabels(ir.getNrUnitLabels(i, j), ir.getUnitLabels(i, j), ir.getUnitLabelsQe(i, j),
                                ir.getUnitLabelsWgt(i, j));
                        layer.getUnit(i, j, 0).restoreKaskiLabels(ir.getNrKaskiLabels(i, j), ir.getKaskiUnitLabels(i, j),
                                ir.getKaskiUnitLabelsWgt(i, j));
                        layer.getUnit(i, j, 0).restoreKaskiGateLabels(ir.getNrKaskiGateLabels(i, j), ir.getKaskiGateUnitLabels(i, j, 0));
                        if (ir.getNrSomsMapped(i, j) > 0) { // if expanded then create new growingsom
                            ...
                            if (ir.getWeightVectorFileName() != null) {
                                subWeightFileName = ir.getFilePath() + ir.getUrlMappedSoms(i, j)[0] + SOMLibFormatInputReader.weightFileNameSuffix;
                            }
                            String subUnitFileName = null;
                            if (ir.getUnitDescriptionFileName() != null) {
                                subUnitFileName = ir.getFilePath() + ir.getUrlMappedSoms(i, j)[0] + SOMLibFormatInputReader.unitFileNameSuffix;
                            }
                            String subMapFileName = null;
                            if (ir.getMapDescriptionFileName() != null) {
                                subMapFileName = ir.getFilePath() + ir.getUrlMappedSoms(i, j)[0] + SOMLibFormatInputReader.mapFileNameSuffix;
                            }
                            try {
                                layer.getUnit(i, j, 0).setMappedSOM(
                                        new GrowingSOM(++id, layer.getUnit(i, j, 0), new SOMLibFormatInputReader(subWeightFileName, subUnitFileName,
                                                subMapFileName)));
                            } catch (Exception e) {
                                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(e.getMessage() + " Aborting.");
                                System.exit(-1);
    ...
    protected GrowingSOM(int id, Unit su, SOMInputReader ir, GrowingLayer layer) {
        ...
        int totalUnitNum = layer.getXSize() * layer.getYSize();
    ...
    /**
     * Constructs an already trained <code>GrowingSOM</code> with a <code>SOMInputReader</code> provided by argument <code>ir</code>.
     * 
     * @param ir an object implementing the <code>SOMinputReader</code> interface to load an already trained model.
     */
    ...
    /**
     * Trains the map with the input data and training parameters specified in the properties provided by argument <code>props</code>. If the value of
     * property <code>tau</code> is 1, a fix-sized layer is trained, otherwise the layer grows until a certain quality criterion determined by
     * <code>tau</code> and the mean quantization error specified by argument <code>mqe0</code> of the data (which is automatically calculated) is
     * reached. This method is usually used for GHSOM training.
     * 
    ...
    public QualityMeasure train(InputData data, GHSOMProperties props, double targetQualityValue, String qualityMeasureName) {
        ...
        return layer.train(data, props.learnrate(), props.sigma(), iterationsToTrain, 0, props.tau(), targetQualityValue, qualityMeasureName, props);
    }

    /**
     * Trains the map with the input data and training parameters specified in the properties provided by argument <code>props</code>. If the value of
     * property <code>tau</code> is 1, a fix-sized layer is trained, otherwise the layer grows until a certain quality criterion determined by
     * <code>tau</code> and the mean quantization error of the data (which is automatically calculated) is reached.
     * 
     * @param data input data to train the map with.
     * @param props the training properties
     */
    public void train(InputData data, SOMProperties props) {
        ...
        layer.train(data, props.learnrate(), props.sigma(), iterationsToTrain, 0, props.tau(), props.growthQualityMeasureName(), props);
    }
    ...
    private class IntermediateSOMDumper implements TrainingInterruptionListener {
        ...
        public void interruptionOccurred(int currentIteration, int numIterations) {
            ...
            try {
                SOMLibMapOutputter.writeWeightVectorFile(GrowingSOM.this, fileProperties.outputDirectory(), filename, true, "$CURRENT_ITERATION="
                        + currentIteration, "$NUM_ITERATIONS=" + numIterations);
            } catch (IOException e) {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe("Could not open or write to output file " + filename + ": " + e.getMessage());
            }
