 ...
 * More Infos: H.U. Bauer and K.R. Pawelzik. Quantifying the neighborhood preservation of self- organizing feature maps. In IEEE Transactions on
 * Neural Networks 3, pages 570-579, 1992.
 * 
 * @author Gerd Platzgummer
 * @version $Id: TopographicProduct.java 2874 2009-12-11 16:03:27Z frank $
 */
public class TopographicProduct extends AbstractQualityMeasure {
            ...
            for (int x = 0; x < xSize; x++) {
                for (int y = 0; y < ySize; y++) {
                    ...
                    for (int z = 0; z < xSize * ySize; z++) {
                        ...
                        mapDist[x][y][z].setRank((double) currank);
                    }
                }
            }
        } catch (Exception ex) {
                ...
                for (int y = 0; y < ySize; y++) {
                    ...
                    for (int x2 = 0; x2 < xSize; x2++) {
                        for (int y2 = 0; y2 < ySize; y2++) {
                            double distance = Double.MAX_VALUE;
                            if ((x != x2) || (y != y2))
                                distance = metric.distance(((GrowingLayer) layer).getUnit(x, y).getWeightVector(), ((GrowingLayer) layer).getUnit(x2,
                                        y2).getWeightVector());
                    ...
                    for (int z = 0; z < xSize * ySize; z++) {
                        ...
                        protoDist[x][y][z].setRank((double) currank);
                    }
                }
            }
        } catch (Exception ex) {
    ...
    private double calculateTPMap() {
        ...
        tpmap = tpmap * 1 / ((double) kmax * ((double) kmax + 1.0));
        return tpmap;
    }
    ...
    private double calcQ1(int x, int y, int k) {
        try {
            ...
            Unit kNextUnit = layer.getUnit(kNextDistanceTag.getX(), kNextDistanceTag.getY());// Koordinaten zu k n�chster Unit ermitteln
    ...
    private double calcQ2(int x, int y, int k) {
        try {
            ...
            double q2Divisor = Math.sqrt((kNextDistanceTag.getX() - x) * (kNextDistanceTag.getX() - x) + (kNextDistanceTag.getY() - y)
                    * (kNextDistanceTag.getY() - y));
    ...
    public double[][] getUnitQualities(String name) throws QualityMeasureNotFoundException {
        if (name.startsWith("TP_Unit|")) {
            ...
            // Berechnung wird hier gestartet; Caching: wenn gleiches k schon ein Ergebnis vorhanden, dann dieses zur�ckgeben, sonst Neuberechnung
            if (_K != (double) k) {
                resetResults();
                rankInputSpace();
                rankOutputSpace();
