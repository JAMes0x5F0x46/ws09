...
/**
 * @author Rober Neumayer
 * @version $Id: SOMSilhouetteValue.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMSilhouetteValue extends AbstractQualityMeasure {
    ...
    public SOMSilhouetteValue(Layer layer, InputData data) {
        ...
        StdErrProgressWriter progressWriter = new StdErrProgressWriter(layer.getXSize() * layer.getYSize(), "Calculating SOMSilhouetteValue for unit ", 50);

        for (int y = 0; y < layer.getYSize(); y++) {
            for (int x = 0; x < layer.getXSize(); x++) {
                progressWriter.progress(y * layer.getYSize() + x);
                // System.out.println("handling unit: " + (x + 1) + " / " + (y + 1) + " of " + (layer.getXSize() * layer.getYSize()));
                Unit u = null;
                try {
                    u = layer.getUnit(x, y);
                ...
                if (u != null) {
                        ...
                        for (int xx = 0; xx < layer.getXSize(); xx++) {
                            ...
                            if (!(xx == x && yy == y) && closestUnit.getNumberOfMappedInputs() > 0) {
                                ...
                                try {
                                    ...
                                    double distance = new L1Metric().distance(u.getWeightVector(), closestUnit.getWeightVector());
                    ...
                    for (int i = 0; i < u.getNumberOfMappedInputs(); i++) {
                        ...
                        try {
                            minDist = new L1Metric().distance(thisDatum.getVector().toArray(), closestUnit.getWeightVector());
                        } catch (MetricException e) {
                            e.printStackTrace();
                        }
                        ...
                        if (debug) {
                            System.out.println("\t minDist - avgDistanceWithinCluster / max: " + minDist + " - " + avgDistanceWithinCluster + " / "
                                    + max);
                        }
                    ...
                    for (int e = 0; e < tmpUnitSilhouetteValues.length; e++) {
                        unitSumSilhouette += tmpUnitSilhouetteValues[e];
                        if (debug) {
                            System.out.println("\t" + tmpUnitSilhouetteValues[e]);
                        }
                    }
                    this.unitSilhouetteValues[x][y] = unitSumSilhouette / new Double(tmpUnitSilhouetteValues.length).doubleValue();
                }
            }
        }
        ...
        if (debug) {
            System.out.println("#units - NaNCount: " + (new Double(layer.getXSize() * layer.getYSize()).doubleValue() - NaNCount));
        }
        // get closest unit
        // get lowest distance in that cluster
        this.mapSilhouetteValue = sumSilhouette / (new Double(layer.getXSize() * layer.getYSize()).doubleValue() - NaNCount);
