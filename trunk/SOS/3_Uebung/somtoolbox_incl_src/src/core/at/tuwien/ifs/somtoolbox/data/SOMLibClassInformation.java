 ...
 * <b>$CLASS_NAMES</b> mandatory: a space-seperated list of class names; the count has to be the same as in $NUM_CLASSES. <br>
 * <b>$XDIM</b> integer, mandatory: number of units in x-direction. Fixed to <i>2</i>. <br>
 * <b>$YDIM</b> integer, mandatory: dimensionality class information vector, equals the number of input vectors ({@link InputData#numVectors()}). <br>
 * <b>labelName_n&nbsp;classIndex_n</b> the $YDIM number of mappings from the input vector label name to the class label index [0...($NUM_CLASSES-1)]. <br>
 * <p>
 ...
 * </p>
 * <p>
 * Finally, the simplest form of the file is to have lines with just the class label; then, this class is assigned to the input datum with the index
 * of the line number.<br>
 * The number of classes, and the indices of those classes, are computer automatically.
 * </p>
 * 
 ...
 * @author Thomas Lidy
 * @author Rudolf Mayer
 * @version $Id: SOMLibClassInformation.java 2874 2009-12-11 16:03:27Z frank $
 */
...
public class SOMLibClassInformation {
     ...
     * {@link #readSOMLibClassInformationFile()}) and the tab seperated file ({@link #readTabSepClassInformationFile()}).
     * 
    ...
    public SOMLibClassInformation(String classInformationFileName) throws SOMToolboxException {
        ...
        try {
            ...
            try {
                ...
                try {
                    logger.info("Reading tab/space separated class file, trying to read simple format...");
                    readSimple();
    ...
    private void throwClassInfoReadingError(String classInformationFileName, IOException e) throws SOMLibFileFormatException {
        throw new SOMLibFileFormatException("Problems reading class information file " + classInformationFileName + ": ' " + e.getMessage()
                + "'. Aborting.");
    }

    /**
     * Reads a class information file containing no header, and tab-seperated String entries for the input vector and class labels.
     * 
     * @throws SOMToolboxException
     * @throws IOException
     */
    private void readTabSepClassInformationFile() throws SOMToolboxException, IOException {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (lineElements.length != 2) {
                br.close();
                throw new SOMLibFileFormatException("Number of elements per line must be exactly 2! Error in line " + index);
            }
    ...
    public void processItems(boolean sort) {
        ...
        for (int i = 0; i < dataNamesTemp.size(); i++) {
            ...
            if (classid < 0) {
                System.out.printf("(%d) Did not find classindex for \"%s\", which is the class of \"%s\"%n", i, classname, label);
                continue;
            }
            dataNames[i] = label;
    ...
    public void addItem(String label, String classname) {
        // Rudi: not sure what the below code attempted to do, but it breaks things in some cases, as too little steps are done
    ...
    private void readSOMLibClassInformationFile() throws IOException, SOMToolboxException {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (line.startsWith("$TYPE")) {
                ...
                if (lineElements.length > 1) {
                    ...
                    throw new SOMLibFileFormatException("Class information file format corrupt in $NUM_CLASSES line. Aborting.");
                }
                ...
                if (lineElements.length > 1) {
                    ...
                    if (lineElements.length > numClasses + 1) {
                        throw new SOMLibFileFormatException("Class information file format corrupt in $CLASS_NAMES line; expecting to find "
                                + numClasses + " classes, but found " + (lineElements.length - 1) + ". Aborting.");
                    }
                    for (int c = 0; c < numClasses; c++) {
                        classNames[c] = lineElements[c + 1];
                    }
                } else {
                    throw new SOMLibFileFormatException("Class information file format corrupt in $CLASS_NAMES line. Aborting.");
                }
                ...
                if (lineElements.length > 1) {
                    ...
                    throw new SOMLibFileFormatException("Class information file format corrupt in $YDIM line. Aborting.");
                }
                ...
                if (lineElements.length > 1) {
                    ...
                    for (int i = 1; i < lineElements.length; i++) {
                        ...
                        if (rgb.length == 3) {
                            try {
                                ...
                                throw new SOMLibFileFormatException("Class information file format corrupt in $CLASS_COLOURS - colour '"
                    ...
                    if (colourIndex + 1 < numClasses()) { // if we have too few classes
                        logger.info("Class info file contained too few class colours (" + colourIndex + ", # of classes: " + numClasses()
                                + "), filling up with default classes.");
                    ...
                    throw new SOMLibFileFormatException("Class information file format corrupt in $CLASS_COLOURS line. Aborting.");
                }
            }
        }
        ...
        while (line != null) {
            ...
            if (lineElements.length != columns) {
                ...
                try {
                    ...
                    throw new SOMLibFileFormatException("Class id number format corrupt in element number " + index + ": '" + lineElements[1]
                            + "'. Aborting.");
                }
            }
        ...
        if (index != numData) {
            throw new SOMLibFileFormatException("Class information file format corrupt. Incorrect number of data items. Aborting.\n"
    ...
    public String getClassName(String vectorName) throws SOMLibFileFormatException {
        ...
        if (object instanceof Integer) {
            ...
            throw new SOMLibFileFormatException("Class information file corrupt. Error pairing input vectors with class names for vector: "
                    + vectorName);
        }
    }

    public boolean hasClassAssignmentForName(String vectorName) {
        return dataHash.containsValue(vectorName);
    }
    ...
    public void writeToFile(String fileName) throws IOException, SOMLibFileFormatException {
        ...
        writer.write("$TYPE class_information");
        writer.newLine();
        writer.write("$NUM_CLASSES " + numClasses);
        writer.newLine();
        ...
        writer.write("$XDIM 2");
        writer.newLine();
        writer.write("$YDIM " + numData);
        writer.newLine();
        for (String element : dataNames) {
            writer.write(element + " " + dataHash.get(element));
            writer.newLine();
        }
    ...
    public void writeToFileTabSeparated(String fileName) throws IOException, SOMLibFileFormatException {
        BufferedWriter writer = new BufferedWriter(new FileWriter(fileName));
        for (String element : dataNames) {
            writer.write(element + "\t" + getClassName(element));
            writer.newLine();
        }
        writer.flush();
        writer.close();
        ...
        try {
            ...
            while ((line = br.readLine()) != null & !done) {
                if (i >= numClasses()) {
                    ...
                    strcols = line.split(" ");
                    if (strcols.length != 3) {
                        logger.severe("Color file: Error in line '" + line + "' - did not find 3 int color parts (RGB)!");
                        br.close();
                        return false;
                    }
    ...
    public static void main(String[] args) throws SOMToolboxException, IOException {
        ...
        JSAPResult config = OptionFactory.parseResults(args, OptionFactory.getOptClassInformationFile(true), OptionFactory.getOptOutputFileName(true));
        new SOMLibClassInformation(config.getString("classInformationFile")).writeToFile(config.getString("output"));
    }
}
