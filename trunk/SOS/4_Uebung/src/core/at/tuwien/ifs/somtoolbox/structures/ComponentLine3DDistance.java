   ...
   //         distance2 = 10000;
    ...
    public static double euclideanLineDistanceDifferentNumberOfStops(Point3d[] line1, Point3d[] line2) {
        ...
        for (int i = 0; i < line2.length; i++) {
            ...
            distance1 += Math.min(line1[i].distance(getPreviousValidPoint(line2, i)), line1[i].distance(getNextValidPoint(line2, i)));
            distance2 += Math.min(line2[endIndex].distance(getPreviousValidPoint(line1, endIndex)), line2[endIndex].distance(getNextValidPoint(line1,
                    endIndex)));
    ...
    /**
     * Computes the distance between two lines in terms of edit operations necessary to move one line onto the other. This basically computes
     * distances for beginning and end points of single lines and penalises non parallel lines .
     */
    ...
    /**
     * Computes if two lines are parallel to each other, 
     * by checking the cross product of their direction vectors. 
     **/
    public static boolean linesParallel(Point3d line1Begin, Point3d line1End, Point3d line2Begin, Point3d line2End) {
        ...
        Point3d direction1 = new Point3d(line1Begin.x - line1End.x, line1Begin.y - line1End.y, line1Begin.z - line1End.z);
        Point3d direction2 = new Point3d(line2Begin.x - line2End.x, line2Begin.y - line2End.y, line2Begin.z - line2End.z);
    ...
    public ComponentLine3D meanObject(Cluster<? extends ComponentLine3D> elements) {
        ...
        for (int i = 0; i < mean.length; i++) {
            ...
            x = x / (double) elements.size();
            y = y / (double) elements.size();
            z = z / (double) elements.size();
