...
/**
 * This class generates the Report as LATEX output. It holds a reference to an instance of ReportFileWriter, to which all Strings that shall appear in
 ...
 * @version $Id: OutputReportLATEX.java 2874 2009-12-11 16:03:27Z frank $
 */
public class OutputReportLATEX implements OutputReport {
    ...
    private void printReportStart() {
        ...
        for (int i = 0; i < filenames.length; i++) {
            this.writer.appendOutput("\\textbf{" + filenames[i] + "} " + this.datasetInformation.getTrainingDataInfo()[i] + "\\\\\n");
        }
    ...
    private void printDatasetReport() {
        ...
        String pca_desc = this.testruns.getRun(0).getScientificDescription("pca");
        ...
        this.writer.appendLatexOutput("Dataset has been normalized: " + StringUtils.formatBooleanValue(0, this.datasetInformation.isNormalized())
                + "\\\\\n");
        ...
        for (int i = (int) dim_array[0][0], counter = 0; counter < dim_array.length; counter++, i = (counter < dim_array.length)
            ...
            this.writer.appendLatexOutput(String.format("%.3f", this.datasetInformation.getNumericalDataProps(DatasetInformation.MIN_VALUE, i))
                    + "&\n");
            this.writer.appendLatexOutput(String.format("%.3f", this.datasetInformation.getNumericalDataProps(DatasetInformation.MAX_VALUE, i))
                    + "&\n");
            this.writer.appendLatexOutput(String.format("%.5f", this.datasetInformation.getNumericalDataProps(DatasetInformation.MEAN_VALUE, i))
                    + "&\n");
            this.writer.appendLatexOutput(String.format("%.5f", this.datasetInformation.getNumericalDataProps(DatasetInformation.VAR_VALUE, i))
                    + "&\n");
            this.writer.appendLatexOutput((int) this.datasetInformation.getNumericalDataProps(DatasetInformation.ZERO_VALUE, i)
                    + " ("
            ...
            this.writer.appendLatexOutput(StringUtils.formatBooleanValue(0, datasetInformation.getBoolDataProps(DatasetInformation.DISCRETE, i))
                    + "&\n");
            this.writer.appendLatexOutput(StringUtils.formatBooleanValue(0, datasetInformation.getBoolDataProps(DatasetInformation.ONLY01, i))
            ...
            this.writer.appendLatexOutput("\\includegraphics[width=50pt]{" + SOMDescriptionHTML.imgSubdir + "/component_" + i
                    + "_SingleMetroMapComponent.jpg}\\\\");
        }
        ...
        if (this.datasetInformation.getNumberOfClasses() > 0) {
            ...
            for (int i = 0; i < this.datasetInformation.getNumberOfClasses(); i++) {
                ...
                this.writer.appendLatexOutput(String.format("%.2f", (double) this.datasetInformation.getNumberOfClassmembers(i)
                        / (double) this.datasetInformation.getNumberOfInputVectors() * 100)
                        + "%&\n");
                this.writer.appendLatexOutput("\\cellcolor{" + StringUtils.getLatexRGBString(this.datasetInformation.getClassColorRGB(i)) + "}\\\\\n");
            }
            this.writer.appendLatexOutput("\\caption{Class Details}\n" + "\\label{tab:classdetails}" + "\\end{longtable}\n");
        }
    }

    /**
     * first outputs the section containing the results from the different teststruns specified, then a short comparison of the testruns (SOM
    ...
    private void printTestrunsReport() {
        ...
        if (this.testruns.getNumberOfRuns() > 1) {
            ...
            this.writer.appendLatexOutput("In this section, details about the training and the resuling SOM are " + "presented. Therefore,");
    ...
    private void printReportOnTestrun(int r, boolean moreRuns) {
        ...
        if (somtype.equals("gg")) {
            ...
            somdescr = new SOMGGDescriptionLATEX(this.writer, this.datasetInformation, this.testruns.getRun(r), this.outputDirPath);
        } else if (somtype.equals("ghsom")) {
            // a growing hierachical som: has more than one level of soms
            somdescr = new SOMGHSOMDescriptionLATEX(this.writer, this.datasetInformation, this.testruns.getRun(r), this.outputDirPath);
        } else {
            // mainly GrowingSOM, but we use it also as generic outputter for types with only small differences to GrowingSOM
            somdescr = new SOMDescriptionLATEX(this.writer, this.datasetInformation, this.testruns.getRun(r), this.outputDirPath);
        }
    ...
    private void printComparingReport() {
        ...
        if (this.datasetInformation.classInfoAvailable()) {
            ...
            for (int i = 0; i < this.testruns.getNumberOfRuns(); i++) {
                TestRunResult temp = this.testruns.getRun(i);
                temp.createClassDistributionImage(this.outputDirPath + "images" + System.getProperty("file.separator"), "classDistribution.jpg", -1);
        ...
        for (int i = 0; i < this.testruns.getNumberOfRuns(); i++) {
            TestRunResult temp = this.testruns.getRun(i);
            temp.createInputDistributionImage(this.outputDirPath + "images" + System.getProperty("file.separator"), "inputDistribution.jpg");
        ...
        for (int i = 0; i < this.testruns.getNumberOfRuns(); i++) {
            TestRunResult temp = this.testruns.getRun(i);
            temp.createUDMatrixImage(this.outputDirPath + "images" + System.getProperty("file.separator"), "umatrix_small.jpg", 10, 0);
