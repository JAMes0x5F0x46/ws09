 ...
 * 
 * @author Doris Baum
 * @version $Id: ArrowPNode.java 2874 2009-12-11 16:03:27Z frank $
 */
public class ArrowPNode extends PNode {
    ...
    public ArrowPNode(InputCorrections.CreationType creationType, String[][] attributes, String tooltip, Color color, double width, double height,
            Point pointBegin, Point pointEnd) {
        this(pointBegin, pointEnd, width / 2, height / 2);
        this.creationType = creationType;
        for (int i = 0; i < attributes.length; i++) {
            addAttribute(attributes[i][0], attributes[i][1]);
        }
    ...
    /**
     * Calculate the coordinates for the 3 points defining each arrowhead (the arrowhead is a triangle) and generate polygon objects with these
     * points. The polygon objects are then held in attributes startHead and endHead. The lenght of the arrow line is adjusted so that it doesn't
     * overlap with the arrowheds
     */
    private void calculateArrow() {
        ...
        startHead.addPoint((int) (x1 + deltax1 * signA + deltax2 * signB), (int) (y1 + deltay1 * signB - deltay2 * signA));
        startHead.addPoint((int) (x1 + deltax1 * signA - deltax2 * signB), (int) (y1 + deltay1 * signB + deltay2 * signA));
    ...
    /**
     * (Re)sets the coordinates of the arrow to (x1, y1) (x2, y2); produces a line and appropriate arrowheads and resets the Node's bounding box to
     * hold the new arrow
     */
    ...
    public static ArrowPNode createInputCorrectionArrow(InputCorrection c, InputCorrections.CreationType creationType,
        ...
        return new ArrowPNode(creationType, attributes, tooltipBegin + " '" + c.getLabel() + "' " + c.getSourceUnit().printCoordinates() + " -> "
                + c.getTargetUnit().printCoordinates(), colour, sourceUnitNode.getWidth(), sourceUnitNode.getHeight(), sourceUnitNode.getPostion(),
