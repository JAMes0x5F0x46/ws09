...
import at.tuwien.ifs.somtoolbox.apps.viewer.controls.table.ButtonCellEditor;
import at.tuwien.ifs.somtoolbox.apps.viewer.controls.table.ButtonCellRenderer;
import at.tuwien.ifs.somtoolbox.apps.viewer.controls.table.ColorCellRenderer;
...
/**
 * @author Jakob Frank
 * @version $Id: PocketSOMConnector.java 2874 2009-12-11 16:03:27Z frank $
 */
public class PocketSOMConnector extends AbstractViewerControl {
    ...
    public PocketSOMConnector(String title, CommonSOMViewerStateData state) {
        super(title, state);
        if (state != null) {
            pathHitHist = new double[state.growingLayer.getXSize()][state.growingLayer.getYSize()];
    ...
    private void initialize() {
        ...
        lblPort.setText("Connector Port:");
        ...
        mainP.add(getBtnServerCtrl(), gridBagConstraints2);
        mainP.add(getBtnExtra(), gridBagConstraints02);
        mainP.add(getBtnExtraReset(), gridBagConstraints03);
    ...
    private JButton getBtnExtraReset() {
        if (btnExtraReset == null) {
            btnExtraReset = new JButton("Reset");
            btnExtraReset.addActionListener(new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent e) {
    ...
    private JButton getBtnExtra() {
        if (btnExtra == null) {
            ...
            btnExtra.addActionListener(new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent e) {
                    state.mapPNode.showHistogramOverlayVisualization(pathHitHist, 0); // 2nd param is visualization index
                }
            });
        }
        return btnExtra;
    ...
    public Dimension minimumSize() {
        return new Dimension(state.controlElementsWidth, 120);
    }
    ...
    private JScrollPane getScpUsers() {
        if (scpUsers == null) {
            ...
            scpUsers.setViewportView(getTblUsers());
    ...
    private JPanel getPnlMerge() {
        if (pnlMerge == null) {
            ...
            gc.gridwidth = 2;
            pnlMerge.add(new JLabel("Merge selected Paths:"), gc);
            gc.gridwidth = 1;
            pnlMerge.add(getChkMergeDebugPrint(), gc);
            gc.gridy++;

            pnlMerge.add(getBtnMerge1(), gc);
            pnlMerge.add(getBtnMerge2(), gc);
            pnlMerge.add(getBtnMerge3(), gc);
    ...
    private JButton getBtnMerge1() {
        ...
        JButton btnMerge1 = new JButton("unitBasedMerge");
        btnMerge1.addActionListener(new ActionListener() {
    ...
    private JButton getBtnMerge2() {
        // TODO Auto-generated method stub
        JButton btnMerge2 = new JButton("lineBasedMerge");
        btnMerge2.addActionListener(new ActionListener() {
        ...
        /*
         * btnMerge2.addActionListener(new ActionListener() {
         * @Override public void actionPerformed(ActionEvent e) { try { Socket s = new Socket(con.server.getInetAddress(), con.server.getLocalPort());
         * PrintStream out = new PrintStream(s.getOutputStream()); BufferedReader in = new BufferedReader(new InputStreamReader(s.getInputStream()));
         * out.println("Login: Pin"); out.println("PIN: " + con.pin); if (in.readLine().equalsIgnoreCase("Login: OK")) { out.println("Sending: Line");
         * out.println("LineStart user=cheater"); out.println("LinePoint: .5/.1"); out.println("LinePoint: .9/.9"); out.println("LineEnd"); }
         * out.close(); in.close(); s.close(); } catch (UnknownHostException e1) { // TODO Auto-generated catch block e1.printStackTrace(); } catch
    ...
    private JButton getBtnMerge3() {
        // TODO Auto-generated method stub
        JButton btnMerge3 = new JButton("Highlight");
        btnMerge3.addActionListener(new ActionListener() {
        ...
        private void informListeners(String message) {
            ...
            synchronized (notificationReceivers) {
                for (Socket s : notificationReceivers) {
                    ...
                    try {
                        // System.out.printf("Message to %s:%d - %s", s.getInetAddress().getHostAddress(), s.getPort(), msg);
                        s.getOutputStream().write(msg.getBytes());
                        s.getOutputStream().flush();
                    } catch (Exception e) {
                        System.out.printf("Socket has problems: %s:%d%n", s.getInetAddress().getHostAddress(), s.getPort());
                        // e.printStackTrace();
                        deadSockets.add(s);
                    }
                }
        ...
        private class Worker extends Thread {// implements Runnable {
            ...
            public void run() {
                try {
                    ...
                    for (String line = br.readLine(); line != null; line = br.readLine()) {
                        ...
                        } else if (false) { // This is old and therefore inactive!
                            System.err.println("Illegal Communication!");
                            System.err.println("Communication dump:");
            ...
            private void clientLogin(String line, BufferedReader in, PrintStream out) {
                ...
                if (lType.equalsIgnoreCase("PIN")) {
                    ...
                    out.printf("%s %s%n", L_ER, "Not implemented");
                }
            }
                ...
                try {
                    ...
                    while ((line = br.readLine()) != null) {
                        ...
                        if (line.startsWith("LinePoint:")) {
                            ...
                            currentLine.setStroke(new BasicStroke(lineWidth, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
                        ...
                        if (state.selectionPanel instanceof PlaySOMPanel) {
                            ...
                            synchronized (player) {
                                ...
                                for (String song : songs) {
                                    Unit u = mapContains(song, map);
                                    if ((u) != null) {
                                        player.addToList(song, u);
                                    } else {
                                        player.addToList(song, receivedFileNamePrefix, u);
                                    }
                                }
    ...
    class UserTableModel extends AbstractTableModel {
        ...
        private PNode addPath(String username, Color color) {
            UserTableRow u = null;
            synchronized (users) {
        ...
        @Deprecated
        @SuppressWarnings("unused")
        private PNode addPath(String username) {
            UserTableRow u = null;
            synchronized (users) {
                for (int i = 0; i < users.size(); i++) {
                    UserTableRow utr = (UserTableRow) users.get(i);
        ...
        public Object getValueAt(final int rowIndex, final int columnIndex) {
            final UserTableRow utr = (UserTableRow) users.get(rowIndex);
            switch (columnIndex) {
                case 0:
                    return utr.username;
                case 1:
                ...
                while (running) {
                    ...
                    if (toRemove.size() > 0) {
                        for (int i = 0; i < toRemove.size(); i++) {
                            ((UserTableRow) toRemove.get(i)).clearPaths();
                        }
                        synchronized (users) {
