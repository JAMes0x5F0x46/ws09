...
/**
 * @author Doris Baum
 * @version $Id: ShiftsControlPanel.java 2874 2009-12-11 16:03:27Z frank $
 */
public class ShiftsControlPanel extends AbstractViewerControl implements ActionListener, ItemListener {
    ...
    static final String positionToolTip = "Position of the second (target) SOM relative to the " + "first (source) SOM.";

    static final String positionManualToolTip = "Set position of the second (target) SOM " + "manually by specifying an offset.";
    ...
    public void initGUIElements() {
        ...
        outlierCountSpinner = new JSpinner(new SpinnerNumberModel(mapPane.getSOMComparision().getOutlierCountThreshold(), 1,
        ...
        stableCountSpinner = new JSpinner(new SpinnerNumberModel(mapPane.getSOMComparision().getStableCountThreshold(), 1,
                mapPane.getSOMComparision().getMaxCount(), 1));
        ...
        minAbsoluteCountSpinner = new JSpinner(new SpinnerNumberModel(mapPane.getSOMComparision().getMinAbsoluteCount(), 1,
                mapPane.getSOMComparision().getMaxCount(), 1));
        ...
        clusterSpinner = new JSpinner(new SpinnerNumberModel(mapPane.getSOMComparision().getClusterNo(), 1, mapPane.getSOMComparision().MAXCLUSTERNO,
                1));
    ...
    public void switchCountSpinners(boolean absolute) {
        ...
        if (absolute) {
            ...
            for (int i = 0; i < listeners.length; i++) {
                outlierCountSpinner.removeChangeListener(listeners[i]);
            }
            outlierCountSpinner.addChangeListener(new outlierAbsoluteChangeListener());
            outlierCountSpinner.setModel(new SpinnerNumberModel(mapPane.getSOMComparision().getOutlierCountThreshold(), 1,
                    mapPane.getSOMComparision().getMaxCount(), 1));

            listeners = stableCountSpinner.getChangeListeners();
            for (int i = 0; i < listeners.length; i++) {
                stableCountSpinner.removeChangeListener(listeners[i]);
            }
            stableCountSpinner.addChangeListener(new stableAbsoluteChangeListener());
            stableCountSpinner.setModel(new SpinnerNumberModel(mapPane.getSOMComparision().getStableCountThreshold(), 1,
                    mapPane.getSOMComparision().getMaxCount(), 1));
            ...
            outlierCountSpinner.setModel(new SpinnerNumberModel(mapPane.getSOMComparision().getOutlierPercentThreshold(), 0, 100, 1));
            ...
            stableCountSpinner.setModel(new SpinnerNumberModel(mapPane.getSOMComparision().getStablePercentThreshold(), 0, 100, 1));
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("bottom") || e.getActionCommand().equals("top") || e.getActionCommand().equals("left")
                || e.getActionCommand().equals("right")) {
