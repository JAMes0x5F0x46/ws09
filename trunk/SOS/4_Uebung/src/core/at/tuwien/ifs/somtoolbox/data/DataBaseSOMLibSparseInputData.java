 ...
 * @author liegl
 * @author Rudolf Mayer
 * @version $Id: DataBaseSOMLibSparseInputData.java 2874 2009-12-11 16:03:27Z frank $
 */
public class DataBaseSOMLibSparseInputData extends AbstractSOMLibSparseInputData {
    ...
    public DataBaseSOMLibSparseInputData(MySQLConnector dbConnector, boolean sparse, String classInfoFileName, boolean norm, int numCacheBlocks,
    ...
    private DoubleMatrix1D readVectorFromDataBase(int rowindex) {
        ...
        try {
            // Build the query
            String query = "SELECT termNumber, weightNormalised from " + dbConnector.getDocumentTermsTableName() + " WHERE documentNumber="
                    + rowindex + " ORDER BY termNumber ASC";
            ResultSet r = dbConnector.executeSelect(query);
            // Build the vector
    ...
    protected void calculateMeanVector() throws SQLException {
        ...
        String query = "SELECT SUM(weight)/COUNT(*) FROM  " + dbConnector.getDocumentTermsTableName() + " GROUP BY termNumber";
