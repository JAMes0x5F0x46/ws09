...
/**
 * @author Sebastian Skritek (0226286, Sebastian.Skritek@gmx.at) extended by Martin Waitzbauer(0226025)
 * @version $Id: TestRunResult.java 2874 2009-12-11 16:03:27Z frank $
 */
public class TestRunResult {
    ...
    private HashMap<String, String> scientificDescriptions = new HashMap<String, String>();
    ...
    private ThematicClassMapVisualizer thematic_visualizer = (ThematicClassMapVisualizer) Visualizations.getVisualizationByName("Thematic Class Map").getVis();

    public EntropyVisualizer entropy_visualizer = (EntropyVisualizer) Visualizations.getVisualizationByName("Entropy Visualiser").getVis();

    private SmoothedDataHistograms sdh_visualizer = (SmoothedDataHistograms) Visualizations.getVisualizationByName("Smoothed Data Histograms").getVis();

    private TrustwothinessVisualizer trustworthyness_visualizer = (TrustwothinessVisualizer) Visualizations.getVisualizationByName("Trustworthiness").getVis();
    ...
    public TestRunResult(DatasetInformation datasetInfo, String mapFilePath, String propertyFilePath, String unitFilePath, String weightFilePath,
            String dwFilePath, int runId, int type, CommonSOMViewerStateData state) {

        this.state = state;
        this.type = type;
        this.runId = runId;
        if (!state.growingSOM.isLabelled()) {
            new LabelSOM().label(state.growingSOM, state.inputDataObjects.getInputData(), AbstractNetworkModel.DEFAULT_LABEL_COUNT);
        }
        ...
        try {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("Cannot read map description file of run " + this.runId + ". Reason: " + e);
    ...
    public TestRunResult(DatasetInformation datasetInfo, String mapFilePath, String propertyFilePath, String unitFilePath, String weightFilePath,
            String dwFilePath, int runId, int type) {
        this.type = type;
        this.runId = runId;
    ...
    public int getNumberOfInputsOnUnit(int x, int y) {
        ...
        return (int) MathUtils.getSumOf(this.classDistribution[x][y]);
    }
    ...
    /**
     * returns the mqe of the trained SOM returns the mqe of the trained SOM, as specified by the QuantizationError Object - see there for more
     * information
     * 
    ...
    /**
     * returns the mmqe of the trained SOM returns the mmqe of the trained SOM, as specified by the QuantizationError Object - see there for more
     * information
     * 
    ...
    public UnitQEContainer getMinUnitQE() {
        ...
        try {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("Cannot get unit qualities for run " + this.runId + ". Reason: " + e);
            return null;
        } catch (LayerAccessException e) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("Cannot get unit in for run " + this.runId + ". Reason: " + e);
            return null;
        }
    }
     ...
     * 
     * @return an object storing information about both: the biggest topographic error of the map as well as of the unit containing this error, null
     ...
     * 
     * @return an object storing information about both: the smallest topographic error (> 0!!) of the map as well as of the unit containing this
     *         error, null in case of an error
     */
     ...
     * 
     * @return an object storing information about both: the smallest silhouette error (> 0!!) of the map as well as of the unit containing this
     *         error, null in case of an error
     */
    ...
    /**
     * Returns the quantization error of the unit at the specified coordinates That is, for all inputs mapped to this unit, the distance between the
     * weigh vector of the input item and the unit is summed and returned
     * 
    ...
    public double getQEForUnit(int x, int y) {
        try {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning(
                    "Cannot retrieve quantization error for unit " + x + "," + y + " in run " + this.runId + ". Reason: " + e);
            return -1;
        }
    }
     ...
     * 
     * @return an object containing both, the value of the qe as well as information about the unit this qe is located on, null in case of any error
     */
    ...
    /**
     * returns the smallest mean quantization error of all units where at least one input vector is mapped on. (that is: sum of distances / number of
     * vectors)
     * 
     * @return an object containing the value of the smallest mqe of all units on the map as well as information about the unit the error is located
     *         on. null in case of any problem.
     */
    ...
    /**
     * returs the mean quantization error for the unit with the specified coordinates the mqe of a unit = sum or qe errors / number of input items
     * mapped to this unit. This value is returned
     * 
     * @param x the x-coordinate of the unit on the map
     * @param y the y-coordinate of the unit on the map
    ...
    public double getMQEForUnit(int x, int y) {
        try {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning(
                    "Cannot retrieve mean quantization error for unit " + x + "," + y + " in run " + this.runId + ". Reason: " + e);
            return -1;
        }
    }

    /**
     * returns the biggest mean quantization error of all units where at least one input vector is mapped on. (that is: sum of distances / number of
     * vectors)
     * 
     * @return an object containing the value of the biggest mqe as well as information about the unit this error is located on, null in case of any
     *         problems
     */
    ...
    /**
     * calculates the qe for the specified input vector Calculates the quantization error of the input vector specified by its name, that is the
     * distance of the given input vector to the unit it is mapped to.
     * 
    ...
    /**
     * mappes the vector to the current SOM and returns x and y coordinates of the winning unit. searches the winning unit on the SOM for the given
     * weight vector (that ist the SOM whose weight vector is closest to the given vecotr and returns the coordinates of this unit.
     * 
    ...
    /**
     * returns the mean x and y coordinates (and the standard derivation) of a class the following is done: for all input elements of the given class,
     * the sum of the x coordinates of the unit the item is mapped onto is build, and then divided by the number of input items in this class. (the
     * same for y) That is, the mean unit in terms of the units the item are really mapped onto is calculated. (meanx = sum(x)/n, where n is the
     * number of input items in the class, and the sum is over all input items, and x is the x-coordinate of the unit the item is mapped onto -
    ...
    public int[] getClassMeanUnit(int classId) {
        ...
        for (String in : ins) {
            Unit temp = layer.getUnitForDatum(in);
            mean[2] += ((double) temp.getXPos() - mean[0]) * ((double) temp.getXPos() - mean[0]) / (double) (n - 1);
            mean[3] += ((double) temp.getYPos() - mean[1]) * ((double) temp.getYPos() - mean[1]) / (double) (n - 1);
        }
        mean[2] = Math.sqrt(mean[2]);
        mean[3] = Math.sqrt(mean[3]);
        return new int[] { (int) Math.round(mean[0]), (int) Math.round(mean[1]), (int) Math.round(mean[2]), (int) Math.round(mean[3]) };
    }

    /**
     * creates an image that visualizes the "mean unit" of a class, its standard derivation and the unit onto which the mean weight vector of the
     * class would be mapped Creates an image, that visualizes the units of the SOM. The position of the mean unit (defined by the indexes 0,1 of the
     * meanUnit parameter) is marked by a square, the position of the unit onto which the mean weight vector would be mapped by a circle. The radius
     * (standard derivation of the mean unit) is denoted by a rectangle, going meanUnit[2] units to the left and right of the mean unit, and
     * meanUnit[3] items to the top and bottom the image is named run_RUNID_classCenter_CLASSID.jpg";
     * 
     * @param meanUnit [0]: the x coordinate of the mean unit, [1]: the y coordinate, [2]: the derivation in x-direction, [3]: the derivation in y
     *            direction
     * @param mappedMeanVector [0]: the x coordinate of the unit onto which the mean weight vector of the class would be mapped, [1]: the y coordinate
    ...
    /**
     * calculates the qe for the specified input vector Calculates the quantization error of the input vector specified by its name, that is the
     * distance of the given input vector to the winner unit on the som. (it is not clear yet, whether this unit must be the same as the unit this
     * element is mapped onto, or whether during training the map can change in a way that the mapped unit is not the winner at the end of the
     * training. There would be a chance if an input vector at the beginning of the training is lying between two units, but during the rest of the
     * training, the unit chosen moves away from the input vector, while the other one comes closer ...)
     * 
     * @param name the name specifying the input vector
     * @return
     */
    ...
    /**
     * returns the min qe of any input item compared with the unit it is mapped to returns the value of the smallest distance between the weight
     * vector of an input item and the unit this item is mapped to. It also returns the input item(s) and unit(s) between those error arises
     * 
    ...
    /**
     * returns the max qe of any input item compared with the unit it is mapped to returns the value of the greatest distance between the weight
     * vector of an input item and the unit this item is mapped to. It also returns the input item(s) and unit(s) between those error arises
     * 
     * @return object containing the information specified above
     */
    ...
    /**
     * returns the min qe of any input item compared with all units on the map returns the value of the smallest distance between the weight vector of
     * an input item and alle the weight vectors of units on the map. It also returns the input item(s) and unit(s) between those error arises.
     * 
     * @return object containing the information specified above
     */
    ...
    /**
     * returns the max qe of any input item compared with all units on the map returns the value of the greatest distance between the weight vector of
     * an input item and alle the weight vectors of units on the map. It also returns the input item(s) and unit(s) between those error arises.
     * 
     * @return object containing the information specified above
     */
    ...
    /**
     * returns the QuantizationError Object for the SOM specified by this TestRunResult Creates the some according to the data stored by this object,
     * and returns the Quantization Error Object provided by the Layer (which can be retrieved from the GrowingSOM)
     * 
     ...
     * 
     * @return array of length numberOfClasses + 1, where the index denotes the number of classes, and the value the number of corresponding units
     */
    public int[] getClassPurity1() {
        ...
        int count = 0;
        for (double[][] element : this.classDistribution) {
            for (double[] element2 : element) {
                count = 0;
                for (double element3 : element2) {
                    if (element3 > 0) {
                        count++;
                    }
                }
     ...
     * 
     * @param purity an array storing at each index i how many units exist that have items of i different classes mapped onto it
     * @return a string giving an estimation of how good the SOM separates the classes
     */
    public String getClassPurity2(int[] purity) {

        String concl = "";
        double corr = (double) (MathUtils.getSumOf(purity) - purity[0] - purity[1]) / (double) (MathUtils.getSumOf(purity) - purity[0]);
        if (corr < 0.05) {
            ...
            concl = "The SOM separates the classes good. (" + String.format("%.2f", corr * 100) + "% of non-empty units contain more than one class)";
        } else if (corr < 0.5) {
            concl = "The classes are separated average by the SOM. (" + String.format("%.2f", corr * 100)
                    + "% of non-empty units contain more than one class)";
        } else if (corr < 0.7) {
            concl = "The SOM separates the classes poor. (" + String.format("%.2f", corr * 100) + "% of non-empty units contain more than one class)";
        } else if (corr < 0.85) {
        ...
        double average = ((double) this.getGrowingSOM().getLayer().getXSize() * (double) this.getGrowingSOM().getLayer().getYSize())
                / (double) this.datasetInfo.getNumberOfClasses();
        concl += " This is also supported by the fact, that for " + this.getGrowingSOM().getLayer().getXSize() + " x "
                + this.getGrowingSOM().getLayer().getYSize() + " units " + "of the Map, " + String.format("%.2f", average)
                + " units are reserved in average for one Class," + "with each Class having an average span of "
                + String.format("%.2f", this.getMeanClassSpread()) + " units";
        return concl;
    }
    ...
    /**
     * UPDATE creates a jpg showing the class distribtion on the trained SOM The jpg is saved to the output dir with the following name:
     * run_RUNID_FILENAME
     * 
     * @param outputDir string defining the output directory where the image shall be saved (neither checked nor created)
     ...
     * @return true if everything worked fine, false in case of any error (false does not give any information about whether the image exists or not)
     */
    public boolean createClassDistributionImage(String outputDir, String filename, int ClassID) {
        ...
        try {
            ...
            AbstractBackgroundImageVisualizer.drawBorder(buffImage);
            return writeImagesAsJPG(outputDir, filename, buffImage, visName);
    ...
    public boolean createDMatrixImage(String outputDir, String filename, int unitScale) {
        ...
        Palette palette = Palettes.getPaletteByName("RGB, 256 Gradient");
        visualizer.setPalette(Palettes.getPaletteIndex(palette), palette);

        String visName = "dmatrix visualization";
        BufferedImage buffImage = visualizer.createVisualization(1, this.getGrowingSOM(), getSomXSize() * unitScale, getSomYSize() * unitScale);
        return writeImagesAsJPG(outputDir, filename, buffImage, visName);
    }
    ...
    /**
     * creates an image visualizing the quantization error of this SOM the image is saved under the given path and name, although a prefix run_runId_
     * is added to the image name
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created)
     ...
     * @return true if everything worked fine, false if there was any problem. False does neither assure that the image exists, nor that is does not
     */
    public boolean createQuantizationErrorImage(String outputDir, String filename, int variant) {
        ...
        Palette palette = Palettes.getPaletteByName("Cartography Color, 128 Gradient (SOMToolbox 0.4.x)");
        visualizer.setPalette(Palettes.getPaletteIndex(palette), palette);
        int unitScale = 30;
        ...
        AbstractBackgroundImageVisualizer.drawUnitGrid(buffImage, getGrowingSOM(), width, height);
        return writeImagesAsJPG(outputDir, filename, buffImage, visName);
    }

    /** Creates a Silhouette image */
    public boolean createSilouetteImage(String outputDir, String filename, int variant) {
        SilhouetteVisualizer visualizer = (SilhouetteVisualizer) Visualizations.getVisualizationByName("Silhouette Value").getVis();
        Palette palette = Palettes.getPaletteByName("Cartography Color, 128 Gradient (SOMToolbox 0.4.x)");
        visualizer.setPalette(Palettes.getPaletteIndex(palette), palette);
        int unitScale = 30;
        int width = getSomXSize() * unitScale;
    ...
    /**
     * creates an image visualizing the topographic error of this SOM the image is saved under the given path and name, although a prefix run_runId_
     * is added to the image name
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created)
     * @param filename the name under which the image (together with the prefix run_[runId]_ ) is saved
    ...
    public boolean createIntrinsicDistanceImage(String outputDir, String filename) {
        IntrinsicDistanceVisualizer visualizer = (IntrinsicDistanceVisualizer) Visualizations.getVisualizationByName("Intrinsic Distance").getVis();
        int unitScale = 30;
        int width = getSomXSize() * unitScale;
        int height = getSomYSize() * unitScale;
    ...
    /**
     * create the U-D-Matrix visualization of the SOM the image is saved under the given path (which is neither checked nor created) under the given
     * name, only the prefix run_runId_ is added to the filename.
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created
     * @param filename the name under which the image (together with the prefix run_[runId]_ ) is saved
     * @param unitScale defines how many pixels shall be used to represent one unit (so one unit has size unitScalexunitScale pixels
     * @param type 0 = U Matrix, 1 = D Matrix;
     * @return true if everything worked fine, false if there was any problem. False does neither assure that the image exists, nor that is does not
     */
    ...
    /**
     * create the Flow & BorderLine visualization of the SOM the image is saved under the given path (which is neither checked nor created) under the
     * given name, only the prefix run_runId_ is added to the filename.
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created
     * @param filename the name under which the image (together with the prefix run_[runId]_ ) is saved
     * @param unitScale defines how many pixels shall be used to represent one unit (so one unit has size unitScalexunitScale pixels
    ...
    /**
     * create the Distortion visualization of the SOM the image is saved under the given path (which is neither checked nor created) under the given
     * name, only the prefix run_runId_ is added to the filename.
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created
     * @param filename the name under which the image (together with the prefix run_[runId]_ ) is saved
    ...
    /**
     * create a SDH visualization of the SOM the image is saved under the given path (which is neither checked nor created) under the given name, only
     * the prefix run_runId_ is added to the filename.
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created
     * @param filename the name under which the image (together with the prefix run_[runId]_ ) is saved
     * @param unitScale defines how many pixels shall be used to represent one unit (so one unit has size unitScalexunitScale pixels
    ...
    public void createSDHImages(String outputDir, String filename, int unitScale, boolean Latex, int step) {
        ...
        // in my testruns, this palette worked quite well
        Palette palette = Palettes.getPaletteByName("Cartography Color, 256 Gradient, less water");
        sdh_visualizer.setPalette(Palettes.getPaletteIndex(palette), palette);
        ...
        try {
            for (int smoothing = 1; smoothing < dim; smoothing += step) {
                ...
                BufferedImage buffImage = sdh_visualizer.getVisualization(0, smoothing, this.getGrowingSOM(), X * unitScale, Y * unitScale);
                AbstractBackgroundImageVisualizer.drawUnitGrid(buffImage, getGrowingSOM(), X * unitScale, Y * unitScale);
                writeImagesAsJPG(outputDir, name, buffImage, visName);
                Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").info("Created SDH image " + smoothing + " / " + (dim - 1));
            }
        } catch (SOMToolboxException e) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("Problems creating the " + visName + " for run " + this.runId + ". Reason: " + e);
        }
    ...
    /**
     * create TrustWortyNess pictures of the SOM the image is saved under the given path (which is neither checked nor created) under the given name,
     * only the prefix run_runId_ is added to the filename.
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created
     * @param filename the name under which the image (together with the prefix run_[runId]_ ) is saved
     * @param unitScale defines how many pixels shall be used to represent one unit (so one unit has size unitScalexunitScale pixels
    ...
    public void createTrustworthyNessImages(String outputDir, String filename, int unitScale, boolean Latex, int step) {
        ...
        if (!Latex) {
            ...
            for (int i = 1; i < dim; i = i + dim / 3) {
                BufferedImage buffImage = trustworthyness_visualizer.createTWImage(this.getGrowingSOM(), width, height, i);
                AbstractBackgroundImageVisualizer.drawUnitGrid(buffImage, getGrowingSOM(), width, height);
    ...
    /**
     * create a TopographicProduct visualization of the SOM the image is saved under the given path (which is neither checked nor created) under the
     * given name, only the prefix run_runId_ is added to the filename.
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created
     * @param filename the name under which the image (together with the prefix run_[runId]_ ) is saved
    ...
    public void createTopographicProductImages(String outputDir, String filename, int unitScale, boolean Latex, int step) {
        ...
        if (!Latex) {
            for (int i = 1; i < dim; i = i + step) {
                ...
                BufferedImage buffImage = topographicproduct_visualizer.createTPImage(this.getGrowingSOM(), getSomXSize() * unitScale, getSomYSize()
                        * unitScale, i);
                AbstractBackgroundImageVisualizer.drawUnitGrid(buffImage, getGrowingSOM(), X * unitScale, Y * unitScale);
                writeImagesAsJPG(outputDir, name, buffImage, visName);
            }
    ...
    /**
     * creates a visualization of the distribution of the input data on the SOM the image is saved under the given path (which is neither checked nor
    ...
    public boolean createInputDistributionImage(String outputDir, String filename) {
        ...
        for (int i = 0; i < us.length; i++) {
            for (int j = 0; j < us[i].length; j++) {
                hist[i][j] = (double) us[i][j].getNumberOfMappedInputs();
            }
        }
        visualizer.setHistogram(hist);
        String visName = "visualization of the distribution of input values";
        try {
            BufferedImage buffImage = visualizer.createVisualization(0, this.getGrowingSOM(), getSomXSize() * 10, getSomYSize() * 10);
            return writeImagesAsJPG(outputDir, filename, buffImage, visName);
    ...
    public boolean createSingleMetroMapComponentImage(String outputDir, String filename, int component) {
        ...
        BufferedImage buffImage = metroVis.createMetromapImage(0, getGrowingSOM(), this.getGrowingSOM().getLayer().getXSize() * 10,
                this.getGrowingSOM().getLayer().getYSize() * 10, component);
     ...
     * 
     * @param ClassID if ClassID is set to -1, the Picture generated by default is that of all classes of the SOM. If ClassID is > -1, the Class with
     *            the specified ClassID-index is displayed
     */
    public boolean createEntropyImage(String outputDir, String filename, int ClassID) {
        ...
        Palette palette = Palettes.getPaletteByName("Redscale, 32 Gradient");
        this.entropy_visualizer.setPalette(Palettes.getPaletteIndex(palette), palette);

        String visName = "visualization of the entropy distribution";
        try {
            BufferedImage buffImage = entropy_visualizer.createVisualization(ClassID, this.getGrowingSOM(), getSomXSize() * 10, getSomYSize() * 10);
            AbstractBackgroundImageVisualizer.drawBorder(buffImage);
            return writeImagesAsJPG(outputDir, filename, buffImage, visName);
        } catch (SOMToolboxException e) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("Problems creating the " + visName + " for run " + this.runId + ". Reason: " + e);
    ...
    /**
     * calculates for each unit on the SOM, how many members of each class are mapped to it the resulting value is stored in a 3-dim. array, where the
     * first dimension denots the x position of the unit, the second the y position, and the third the id of the class. There the number of input
     * items mapped to this unit belongig to this class is saved. this array is stored in this.classDistribution
     */
    private void prepareClassDistribution() {
        // x, y, class
        if (this.datasetInfo.getNumberOfClasses() > 0) {
            this.classDistribution = new double[getSomXSize()][getSomYSize()][this.datasetInfo.getNumberOfClasses()];
        } else {
            this.classDistribution = new double[getSomXSize()][getSomYSize()][1];
        }
    ...
    /**
     * creates for each unit having at least one input item mapped to it an image of the class distribution the images are stored in an subdirectory
     * of the specified directory (which is neither checked nor created) named "pieCharts_runId". All content in this directory is deleted before the
     * images are created.
     * 
     * @param outputDir the path to the output dir where the image shall be saved (must exists, is neither checked nor created
     */
        ...
        for (int x = 0; x < this.classDistribution.length; x++) {
            for (int y = 0; y < this.classDistribution[x].length; y++) {
                if (MathUtils.getSumOf(this.classDistribution[x][y]) > 0) {
                    ...
                    PieChartPNode piechart = new PieChartPNode(-20, -20, 60, 60, this.classDistribution[x][y]);
                    piechart.drawChart(graphics, 0, 0);

                    String filename = x + "_" + y + "_" + (int) MathUtils.getSumOf(this.classDistribution[x][y]) + ".jpg";
                    writeImagesAsJPG(outputDir, filename, buffImage, visName);
                }
            }
        }
    ...
    /**
     * creates and returns the clustering tree for this SOM the cluster method used is WardsLinkageTreeBuilder. If an error occurs, null is probably
     * returned.
     * 
    ...
    public ClusteringTree getClusterTree() {
        if (this.clusterTree == null) {
            ...
            try {
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("Problems trying to create clusters for run " + this.runId + ". Reason: " + e);
                return null;
            }
        }
    ...
    /**
     * tries to sort all possible clusters by their "stability" that is, tries to give a measure about how good a cluster is. Thereby, as simple
     * qualification method, simply the improvement of the distance between all units within a cluster and the mean vector of a cluster, divided by
     * the number of units within a cluster compared to the same value in the parent cluster is chosen. This works surprisingly well. (that is:
     * biggest difference of parentvalue - my value)
     * 
     ...
     * @param boundary a boundary for the level of clusters, where the calculation shall be stopped, that is a boundary for the depth the subtree of
     *            node shall be scanned
     * @param stableClusterList a sorted list of clusters, into which the clusters of this subtree shall be inserted-
     * @return an ordered list containing all the clusters specified in the last argument as well as all clusters in the subtree rooted in node "node"
     *         having level smaller than boundary
     */
    public Vector<double[]> getStableClusters2(ClusterNode node, double parentValue, int boundary, Vector<double[]> stableClusterList) {
        ...
        try {
            for (GeneralUnitPNode unit : units) {
                dist += this.getGrowingSOM().getLayer().getMetric().distance(unit.getUnit().getWeightVector(), node.getMeanVector());
            }
        } catch (MetricException e) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("Error calcuating distance in run " + this.runId + " ... Reason: " + e);
            dist = -1;
        }
    ...
    /**
     * returns for the given cluster, how many input elements of the different classses are mapped to units of this cluster returns an array, that
     * contains for each class, how many input vectors belonging to this class are mapped to units of the cluster
     * 
    ...
    /**
     * creates a visualization of the given cluster the visualization is an image of the SOM, where the units are displayed, an the units belonging to
     * the cluster are coloured. Further, the name of the cluster is written in the cluster (as least tried to)<br/>
     * <br/>
     * WARNING!! nodeDepths should contain the depths of the cluster nodes. But for level i, the according depth is not in nodeDepths[i-1] or [i], but
     * at [i+1]!!!!
     * 
     ...
     * @param preferredUnitSize the length a unit-side has in the ideal case (that is if there aren't too many units), that is, a unit gets
     *            dimenstions preferredUnitSizexpreferredUnitSize
     * @return true if no error was encountered during execution, false otherwise (if false is returned, nothing can be stated about whether the image
     *         exists or not)
     */
    public boolean visualizeCluster(int clusterLevel, String outputDir, int[] nodeDepths, int preferredUnitSize) {
        ...
        Vector<String> clusterNames = this.datasetInfo.getClusterName(node, (int) CommonSOMViewerStateData.getInstance().clusterByValue,
                nodeDepths[node.getLevel() + 1]);
    ...
    /**
     * returns an instance of input reader the following files are handed to the reader: this.weightFilePath, this.unitFilePath, this.mapFilePath the
     * reader created is saved in this.inputReader
     * 
    ...
    protected SOMLibFormatInputReader getInputReader() {
        if (this.inputReader == null) {
            try {
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").severe("Cannot find file for creating input reader: " + e);
            } catch (SOMLibFileFormatException e) {
    ...
    /**
     * returns an instance of CommonSOMViewerStateData, that stores basic properties for visualization of SOMs and is needed for creating most of the
     * images
     * 
    ...
    private CommonSOMViewerStateData getStateObject() {
        if (this.state == null) {
            ...
            this.state.palettes = Palettes.getAvailablePalettes();
            this.state.currentPalette = 0;
            this.state.defaultPalette = this.state.palettes[0];
            this.state.defaultPaletteIndex = 0;
     ...
     * 
     * @param ID gets the Description from the hashmap
     * @return
     */
    public String getScientificDescription(String ID) {
        String out = (String) this.scientificDescriptions.get(ID);
        if (out == null) {
    ...
    /**
     * returns every other class+hits contained in the regions with classmembers from the given index, or null if index exceeds the possible number of
     * classes
     */
    ...
    public double[] getClassesForUnit(int x, int y) {
        if (this.classDistribution == null) {
            this.prepareClassDistribution();
        }
        ...
        for (int x = 0; x < this.classDistribution.length; x++) {
            for (int y = 0; y < this.classDistribution[x].length; y++) {
                if (classDistribution[x][y][index] > 0) {
                    ...
                    temp[0] = x;
                    temp[1] = y;
                    temp[2] = (int) classDistribution[x][y][index];
                    UnitCoords.add(temp);
                }
            }
        }
    ...
    public void fillQMContainer() throws QualityMeasureNotFoundException {
        ...
        TopographicError toperr = new TopographicError(this.getGrowingSOM().getLayer(), this.datasetInfo.getInputData());
        IntrinsicDistance idist = new IntrinsicDistance(this.getGrowingSOM().getLayer(), this.datasetInfo.getInputData());
        SilhouetteValue sil = new SilhouetteValue(this.getGrowingSOM().getLayer(), this.datasetInfo.getInputData());
        SOMDistortion dist = new SOMDistortion(this.getGrowingSOM().getLayer(), this.datasetInfo.getInputData());
        SpearmanCoefficient spear = new SpearmanCoefficient(this.getGrowingSOM().getLayer(), this.datasetInfo.getInputData());
        SammonMeasure sammon = new SammonMeasure(this.getGrowingSOM().getLayer(), this.datasetInfo.getInputData());
        MetricMultiScaling mscal = new MetricMultiScaling(this.getGrowingSOM().getLayer(), this.datasetInfo.getInputData());
        ...
        for (int i = 0; i < this.QMContainer.UnitQualityMeasureNames.size(); i++) {
            this.QMContainer.classifyUnits((String) this.QMContainer.UnitQualityMeasureNames.get(i));
        }
    }
    ...
    /**
     * Reads the Textfile conatining all descriptions.
     * 
     * @return false if an error occured, true otherwise
     */
    public boolean readScientificDescriptionTXTFile() {
        try {
            Document doc = new SAXBuilder().build("./src/rsc/reportGenerator/scientificDescriptions.xml");
