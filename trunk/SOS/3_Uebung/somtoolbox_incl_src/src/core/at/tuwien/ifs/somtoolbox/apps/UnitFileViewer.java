 ...
 * 
 * @author Jakob Frank
 * @version $Id: UnitFileViewer.java 2874 2009-12-11 16:03:27Z frank $
 */
public class UnitFileViewer extends JFrame  implements SOMToolboxApp {
    ...
    public void plot(double[][][] data, String[][] labels, double xScale, double yScale, double zScale) {
        for (int i = 0; i < data.length; i++) {
            if (data[i].length > 0) {
                ScatterPlot plot = new ScatterPlot("Plot_" + (i + 1), dotColor, AbstractDrawer.ROUND_DOT, AbstractDrawer.DEFAULT_DOT_RADIUS + i,
                        data[i]);
    ...
    public void gridPlot(double[][][] data, String[][] labels, double xScale, double yScale, double zScale) {
        ...
        for (int i = 0; i < data.length; i++) {
            if (data[i].length > 0) {
                ...
                plotPanel.addPlot(new ScatterPlot("Plot_" + (i + 1), dotColor, AbstractDrawer.ROUND_DOT, AbstractDrawer.DEFAULT_DOT_RADIUS, data[i]));
    ...
    public static void main(String[] args) {
        ...
        JSAPResult config = OptionFactory.parseResults(args, OptionFactory.OPTIONS_UNIT_FILE_VIEWER);
