...
/**
 * <p>
 * <i>Created on Jun 9, 2004</i>
 * </p>
 * 
 * @author Michael Dittenbach
 * @version $Id: TrajectoryOutputter.java 2874 2009-12-11 16:03:27Z frank $
 */
public class TrajectoryOutputter implements SOMToolboxApp {
    ...
    public static void main(String[] args) {
        ...
        JSAPResult config = AbstractOptionFactory.parseResults(args, OptionFactory.OPTIONS_TRAJECTORY_OUTPUTTER);
        ...
        try {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox").severe("IOExeption while writing: " + e.getMessage()); // TODO: change string
            System.exit(-1);
        }
    ...
    public static void write(GrowingSOM gsom, String fDir, String fName, String[] dataNames, boolean drawLines) throws IOException {
        ...
        try {
            ...
            Constructor<Graphics2D> constr = c.getConstructor(String.class, OutputStream.class, int.class, int.class, int.class, int.class);
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning("EPS-Output not possible. To enable add \"jlibeps.jar\" to your classpath");
        }
    }
    ...
    private static void createGraphics(Graphics2D g, int imageWidth, int imageHeight, GrowingSOM gsom, String[] dataNames, boolean drawLines) {
            ...
            for (int i = 0; i < gsom.getLayer().getXSize(); i++) {
                ...
                if (nrDataNames[i][j] > 0) {
                    ...
                    if (maxDataNames == 1) {
                        ...
                        green = ((maxGreen - minGreen) / (maxDataNames - 1)) * (maxDataNames - nrDataNames[i][j]) + minGreen;
                        ...
                        while ((mi < u.getNumberOfMappedInputs()) && (!found)) {
                            if (dataName.equals(u.getMappedInputNames()[mi])) {
                                ...
                                if (separator) {
                                    ...
                                    g.setColor(Color.black);
                                    g.draw(new Line2D.Double((luCornerX + textPaddingX), textPosY - 6,
                                            (luCornerX + textPaddingX + unitWidth - textPaddingX), textPosY - 6));
                                    g.setColor(oldCol);
                                    textPosY += 4;
                                    separator = false;
                                }
        ...
        if (drawLines == true) {
            ...
            for (int d = 0; d < dataNames.length; d++) {
                ...
                if (u == null) {
                    Logger.getLogger("at.tuwien.ifs.somtoolbox").severe("Datum " + dataNames[d] + " not found on map. Aborting.");
                ...
                if (d == 0) {
                    ...
                    // int oldX = (gsom.getLayer().getUnitForDatum(dataNames[d - 1]).getXPos() * unitWidth) + (unitWidth / 2);
                    // int oldY = (gsom.getLayer().getUnitForDatum(dataNames[d - 1]).getYPos() * unitHeight) + (unitHeight / 2);
                }
            }
