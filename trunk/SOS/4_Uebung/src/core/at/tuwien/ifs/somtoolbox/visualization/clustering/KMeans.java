 ...
 * @version $Id: KMeans.java 2874 2009-12-11 16:03:27Z frank $
 */
...
public class KMeans {
     ...
     * @param linearInitialisation whether to initialise randomly or linearly
    ...
    /**
     * Train for as long as instances move between clusters. The first step that doesn't involve changes ends the whole thing -- it's a little bit of
     * magic so to speak.
     */
    public void train() {
        while (true) {
    ...
    /**
     * Searches for clusters which have no instances assigned. These are then replaced FIXME FIXME private void substituteEmptyClusters() {
     * System.out.println("Removing empty clusters:"); double[] replacementCentroid = new double[clusters[0].getCentroid().length]; for (int i = 0; i
     * < clusters.length; i++) { if (clusters[i].getNumberOfInstances() != 0) replacementCentroid = clusters[i].getCentroid().clone(); } for (int i =
     * 0; i < clusters.length; i++) { if (clusters[i].getNumberOfInstances() == 0) clusters[i].setCentroid(replacementCentroid); } }
     */

    private void removeEmptyClusters() {
        Vector<Cluster> nonEmptyClusters = new Vector<Cluster>();
        for (int i = 0; i < clusters.length; i++) {
            if (clusters[i].getNumberOfInstances() != 0)
     ...
     * 
     * @return whether this step brought any changes or not. Note, this one also says no if there were as many changes as in the last step.
     */
    ...
    /**
     * Get a new centroid for empty clusters. We therefore take the instance with the largest SSE to the cluster centroid having the largest SSE. Get
    ...
    /**
     * Get the index of the closest cluster for the given instance index. Note that in case of equally distant clusters we assign the first found
     * cluster. At the end of the day this means that the clusters with lower indices will have a tendency to be larger. It hopefully won't have too
     * much impact, possibly a random assignment in case of equal weights would make sense, however, this would require a couple of steps more in
     * here.
     * 
    ...
    /**
     * Get a set of labels for the given clustering based on the occurrences of attributes within clusters, i.e. there's a preference for labels
     * occurring in many instance.
     * 
     * @return labels
     */
    ...
    /**
     * cluster centres are initialised by equally sized random chunks of the input data when there's 150 instances, we assign 50 chosen randomly to
     * each cluster and calculate its centre from these (the last cluster might be larger if numInstances mod k < 0)
     * 
    ...
    private void initClustersEqualNumbers(DistanceMetric distanceFunction) {
        ...
        for (int clusterIndex = 0; clusterIndex < k; clusterIndex++) {
            ...
            for (int i = 0; i < randPermIndices.length; i++) {
                int currentIndex = randPermIndices[i];
    ...
    private void initClustersLinearly(DistanceMetric distanceFunction) {
        for (int clusterIndex = 0; clusterIndex < k; clusterIndex++) {
            double[] centroid = new double[numberOfAttributes];
            for (int attributeIndex = 0; attributeIndex < numberOfAttributes; attributeIndex++) {
                centroid[attributeIndex] = ((maxValues[attributeIndex] - minValues[attributeIndex]) / (clusters.length + 1) * (clusterIndex + 1))
    ...
    /**
     * like {@link #initClustersLinearly(DistanceMetric)}, but after computing the exact linear point, rather finds & uses the closest instance from
     * the data set as centroid.
     */
            ...
            for (int attributeIndex = 0; attributeIndex < numberOfAttributes; attributeIndex++) {
                ...
                try {
                    for (int i = 0; i < data.length; i++) {
                        if ((distanceFunction.distance(centroid, data[i]) < minDistance && !usedInstances.contains(data[i]))
                                || i == (data.length - 1)) {
                            minData = data[i];
    ...
    public void setClusterCentroids(double[][] centroids) throws MoreCentresThanKException {
        if (centroids.length != k)
            throw new MoreCentresThanKException("So, someone was trying to set " + centroids.length + " centres for " + k
    ...
    /**
     * Utility method to get the min, max, and diff values of the data set. This is used for scaling the (random) values in the initialisation
     * functions.
     */
    ...
    public double[][] getMinMaxNormalisedClusterCentroidsWithin() {
        ...
        for (int i = 0; i < data[0].length; i++) {
            for (int j = 0; j < clusters.length; j++) {
                ...
                if (clusters[j].getCentroid()[i] > max[i])
                    max[i] = clusters[j].getCentroid()[i];
                if (clusters[j].getCentroid()[i] < min[i])
                    min[i] = clusters[j].getCentroid()[i];
            }
        }
        double[][] centroids = new double[k][numberOfAttributes];
        for (int indexClusters = 0; indexClusters < k; indexClusters++) {
