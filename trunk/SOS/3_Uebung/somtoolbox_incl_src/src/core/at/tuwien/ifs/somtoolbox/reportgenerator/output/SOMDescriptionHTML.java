...
import at.tuwien.ifs.somtoolbox.util.PrefixFilenameFilter;
import at.tuwien.ifs.somtoolbox.visualization.clustering.ClusterNode;

/**
 * this class is the base class for generating the part of the reports that describes a SOM. it creates output containing information about basic
 * properties of the learning process and the created SOM, as well as about the distribution of the input data on this SOM. This class implements the
 * description of a standard GrowingSOM, and is subtyped for creating proper description of other SOMs.
 * 
 * @author Sebastian Skritek (0226286) Sebastian.Skritek@gmx.at
 * @author Martin Waitzbauer (0226025)
 * @version $Id: SOMDescriptionHTML.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMDescriptionHTML {
     ...
     * @param imgDir path to the directory where created images shall be stored
     */
    public SOMDescriptionHTML(ReportFileWriter writer, DatasetInformation dataset, TestRunResult testrun, String baseDir, EditableReportProperties EP) {
        this.EP = EP;
        this.imgDir = baseDir;
        ...
        //UPDATE MAZ
        sGrid = new SemanticInterpreterGrid(this.testrun.getGrowingSOM().getLayer().getAllUnits(), this.testrun, true, 0, this.testrun.getType(),
                this.imgDir);
    ...
    /**
     * Prints the topology and type of the SOM (unit shape, type of SOM, ...) the value is dependent of the value keyTopoology specified by the
    ...
    /**
     * Prints the dimension of the SOM, that is the number of units in x and y direction (for Growing SOM this is enough) (taken from the
    ...
    /**
     * Prints the neighbourhood type and the initial neighbourhood range used for training (taken from the MySOMMapDescription, provided by the
     * TestRunResult object)
     */
    ...
    /**
     * prints the type of how the learning rate changes, and the initial learning rate (taken from the MySOMMapDescription, provided by the
     * TestRunResult object)
     */
    ...
    public void printDataDistribution() {
        ...
        if (this.dataset.classInfoAvailable()) {
            ...
            for (int i = 0; i < this.testrun.getMaxEErrorRegionNames().length; i++) {
                this.writer.appendOutput(this.testrun.getMaxEErrorRegionNames()[i][0] + "  (" + this.testrun.getMaxEErrorRegionNames()[i][1] + ") , ");
            }
            this.writer.appendOutput("having an Entropy Error of " + this.testrun.getRegionEntropyError(1) + ".  "
                    + String.format("%.3f", this.testrun.getPercOfZeroEntropyRegions()) + "% of the  regions have an Entropy error of 0.");

            /* Embed Images.. */
            this.writer.appendOutput("<table border=\"1\"><tbody>" + "<tr>" + "<th>class name</th>" + "<th>mean vector</th>"
                    + "<th>mapped to unit</th>" + "<th>\"mean unit\"</th>" + "<th>radius</th>" + "<th>Values by Coordinates</th>" + "<th><img src=\""
            ...
            for (int i = 0; i < this.dataset.getNumberOfClasses(); i++) {
                ...
                this.writer.appendOutput("<td class=\"middleText\"><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId()
                        + "_classCenter_" + i + ".jpg\" alt=\"class center of class " + this.dataset.getNameOfClass(i) + "\"/></td>"
                        + "<td><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId() + "_classDistribution" + i
                        + ".jpg\" alt=\"class distribution of som " + this.testrun.getRunId() + "\"/></td>" + "<td>"
                        + Math.round(this.testrun.getClassEntropy(i)) + "</td>" + "<td><center>");
                ...
                this.writer.appendOutput("</center></td>" + "<td><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId()
                        + "_classEntropy" + i + ".jpg\" alt=\"class " + i + " entropy of som " + this.testrun.getRunId() + "\"/></td>");
                this.writer.appendOutput("</tr>");
            }
        ...
        if (this.EP.includeDistPic() || this.EP.includeFlowBorderLinePic() || this.EP.includeSDHVisualization() || this.EP.includeTPVisualization()
                || this.EP.includeTWVisualization()) {
    ...
    /**
     * Creates output describing the layout of the created som this includes tables showing the distribution of the input vectors of the som, as well
     * as (if available) the distribution of the classes on the som. In addition, if the user selected input items for getting their position on the
     * trained SOM, this information is also created in this function.
     * 
     * @param classInfoAvailable true if class information are available (and therefore a piechart should be inserted, false otherwise)
     */
    protected void printMapLayout(boolean classInfoAvailable) {
        ...
        Unit unit;
        String key;
        Hashtable<String, Vector<InputDatum>> lookup = new Hashtable<String, Vector<InputDatum>>(); // there we store for each unit the input vectors
        // mapped to it
        ...
        /* SDH Section display */
        int maxs = this.testrun.getGrowingSOM().getLayer().getXSize() * this.testrun.getGrowingSOM().getLayer().getYSize();
        if (this.EP.includeSDHVisualization()) {
            String sdh_desc = this.testrun.getScientificDescription("Smoothed Data Histograms");
            ...
            for (int i = 0; i < this.dataset.getNumberOfSelectedInputs(); i++) {
                ...
                for (int j = 0; j < this.qualifiers.size(); j++) {
                    if (this.qm.getUnitQualities((String) qualifiers.get(j)) != null) {
                        ...
                        this.writer.appendOutput("<td>" + String.format("%2f", unitqual[unit.getXPos()][unit.getYPos()]) + "</td>");
                    }
                }
     ...
     * the first table contains for each unit only the number of input vectors mapped to it, and a pie chart image of the class distribution within
     * this unit. (lookup == null) If no class information is available, this table is not created<br/>
     * <br/>
     * The second table additionally contains the ids of input vectors selected by the user within the unit they are mapped to. Therefore lookup must
     * be a hashtable that contains for each unit (key = "x_y") a list of InputDatum objects specifying the input elements mapped to this unit. If for
     * a key there's no list, then nothing is mapped to this unit. if no class information is available, the table does not contain the number of
     * input vectors mapped to each unit
     * 
     * @param lookup a hashtable with the content specified above to map input vectors to units
     * @param classInformationAvailable true if class information (and therefore pie chart diagrams for the units are available, false otherwise
     */
    protected void printDistributionDetailTable(Hashtable<String, Vector<InputDatum>> lookup, boolean classInformationAvailable) {
        ...
        if (classInformationAvailable) {
            pieDir = new File(this.imgDir + "pieCharts_" + this.testrun.getRunId() + System.getProperty("file.separator"));
        }
        ...
        for (int y = 0; y < this.testrun.getGrowingSOM().getLayer().getYSize(); y++) {
            ...
            for (int x = 0; x < this.testrun.getGrowingSOM().getLayer().getXSize(); x++) {
                ...
                //EDIT: it is not, the number of mapped items will be displayed also without a clss file being present
                // the class pie chart and number of input items per unit (as these number is encoded within the pie chart images names
                if (classInformationAvailable) {
                    String prefix = "run_" + this.testrun.getRunId() + "_" + x + "_" + y + "_";
                    String[] matches = pieDir.list(new PrefixFilenameFilter(prefix)); // lookup whether there's an image
                    if (matches != null && matches.length > 0) {
                        // image exists => insert it
                        String number = matches[0].substring(prefix.length());
                        number = number.substring(0, number.length() - 4);

                        img = "<img " + /* imgStyle+ */" src=\"" + SOMDescriptionHTML.imgSubdir + "/pieCharts_" + this.testrun.getRunId() + "/"
                                + matches[0] + "\" onmouseover=\"showClassInfo(this," + number + ",event);\" "
                ...
                if (lookup != null) {
                    ...
                    if (mapped != null) {
                        ...
                        for (int i = 0; i < mapped.size(); i++) {
                            ...
                            mappedInputsDiv += "<div class=\"mappedInputDiv\"" + "onmouseover=\"showInputInfo(this,'"
                                    ...
                                    + String.format("%.5f", this.testrun.getInputMappedQE(mapped.get(i).getLabel()).getQE()) + "','" + "["
                                    + unit.getXPos() + ", " + unit.getYPos() + "]',event);\" " + "onmouseout=\"hideInputInfoDiv();\" "
                                    + "onmousemove=\"positionInputInfoDiv(event);\"" + "/>\"" + mapped.get(i).getLabel() + "\"</div>";
                        }
                        mappedInputsDiv += "</div>";
                    }
                }
    ...
    /**
     * adds information about the topographic error on the map to the report beside a list containing the topographic error of the map and the min/max
     * top. error on the units, an image visualizing the distribution of the topographic error on the map is output.
     */
    protected void printTopographicErrorReport() {
        ...
        String tErr_desc = this.testrun.getScientificDescription("Topographic Error");
        this.writer.appendOutput("Next we will look at the <u onclick=\"javascript:showVisualisationDescriptions('" + tErr_desc
                + "')\"><font color =\"blue\">Topographic Error</font></u> of the SOM.<br><br>");
        this.writer.appendOutput("<li><span class=\"header2\">Topographic error of the som:</span><ul>");

        this.writer.appendOutput("<li>Topographic error of the map:" + String.format("%.5f", this.testrun.getMapTE()) + "</li>\n");
        this.writer.appendOutput("</ul></li>");
        ...
        this.writer.appendOutput("<li>Number of units without topographic error: " + this.testrun.getNumberOfTElessUnits() + "</li>");
        this.writer.appendOutput("<li>Minimal topographic error >0 of a unit : " + String.format("%.2f", min.getQE()) + " (" + unitCoord + ")"
                + "</li>\n");
        max = this.testrun.getMaxUnitTE();
        unitCoord = getUnitCoords(max);
        this.writer.appendOutput("<li>Maximal topographic error of a unit: " + String.format("%.2f", max.getQE()) + " (" + unitCoord + ")"
                + "</li>\n");
        this.writer.appendOutput("</ul></li>\n");
    ...
    /**
     * formats a list of units for use in the quantization error list the created format is "on unit(s) at [x,y] - z vectors mapped, [x2,y2] - z2
     * vectors mapped, ...
     * 
    ...
    /**
     * formats a list of input vectors for use in the quantization error list the created format is: "on input vector(s) "id" on unit at[x,y], "id2"
     * on unit at [x2,y2],...
     * 
    ...
    /**
     * this function prints some information about the possible clusters that can be found on the SOM. It tries to find some good, or stable clusters,
     * using a simple heuristic. For a description of this, please see TestRunResult.getStableClusters2. (it works at least quite well with the iris
     * and the animal dataset - more test tbd. as maximal number of clusters, the number of units on the SOM is choosen. Besides the tree of available
     * clusters, the ten "best" (according to this heuristic clusters are listed and visualized. Also the image of the UMatrix of the SOM is created
     * and attached to the report !!! I also wanted to do some tests with the k-nearest neighbour clustering, but couldn't find any implementation of
     * it. !!!
     */
    protected void printClusterInfos() {
        ...
        if (this.testrun.createUDMatrixImage(this.imgDir, "umatrix.jpg", 30, 0)
            ...
            String uMatrix_desc = this.testrun.getScientificDescription("U-Matrix");
            this.writer.appendOutput("<p class=\"header\"><u onclick=\"javascript:showVisualisationDescriptions('" + uMatrix_desc
                    + "')\"><font color =\"blue\">The U-Matrix visualization</font></u></p>");
            ...
            this.writer.appendOutput("<br><br><center><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId()
                    + "_umatrix.jpg\" alt=\"umatrix visualization\"/>");
            this.writer.appendOutput("<br>");
            this.writer.appendOutput("<img src=\"" + SOMDescriptionHTML.imgSubdir
                    + "/palette_17.jpg\" alt=\"umatrix palette visualization\"/></center>");
            String dMatrix_desc = this.testrun.getScientificDescription("D-Matrix");
            ...
            this.writer.appendOutput("<br><br><center><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId()
                    + "_dmatrix.jpg\" alt=\"dmatrix visualization\"/>");
            this.writer.appendOutput("<br>");
            this.writer.appendOutput("<img src=\"" + SOMDescriptionHTML.imgSubdir
                    + "/palette_17.jpg\" alt=\"umatrix palette visualization\"/></center>");
            // guiaguia;
        ...
        int somDim = this.testrun.getGrowingSOM().getLayer().getXSize() * this.testrun.getGrowingSOM().getLayer().getYSize();
        ...
        for (int i = 0; i < 10 && i < stableList2.size(); i++) {
            Vector<String> clusterNames = this.dataset.getClusterName(this.testrun.getClusterNodes((int) stableList2.get(i)[1], 1),
                    (int) CommonSOMViewerStateData.getInstance().clusterByValue, nodeInfos[(int) stableList2.get(i)[1]]);
            String label = "";
            ...
            this.writer.appendOutput("<tr><td><ul><li>cluster " + label + "<br/> on  level " + stableList2.get(i)[1] + " with gain = "
                    + String.format("%.4f", stableList2.get(i)[0]) + "<br/>"
                    + this.testrun.getClusterNodes((int) stableList2.get(i)[1], 1).getNumberOfInputs() + " inputs in cluster<br/>");

            // + showSemanticClusterInterpretation((int) stableList2.get(i)[1]));

            // classDistInCluster((int) stableList2.get(i)[1],this.testrun.getClusterNodes((int) stableList2.get(i)[1], 1).getNumberOfInputs()));
            //UPDATE MAZ
            if(this.dataset.classInfoAvailable())
            ...
            this.writer.appendOutput("<a href=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId() + "_clusterLevel_"
                    + (int) stableList2.get(i)[1] + "_50.jpg" + "\"><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId()
                    + "_clusterLevel_" + (int) stableList2.get(i)[1] + "_10.jpg\" alt=\"\"/></a>");
    ...
    private String classDistInCluster(int level, int numbInputs) {
        ...
        if (numbClasses != 1) {
            ...
            out = "<a href=\"javascript:showClassesInCluster(" + level + "," + this.testrun.getRunId() + ")\">" + numbClasses
                    + " classes in cluster:</a> ";
        ...
        if (numbClasses > 0) {
            ...
            for (int i = 0; i < classDist.length; i++) {
                ...
                out += "<li>" + this.dataset.getNameOfClass(i) + ": " + classDist[i] + " inputs ("
                        + String.format("%.2f", (double) classDist[i] / (double) numbInputs * 100) + "%) = "
                        + String.format("%.2f", (double) classDist[i] / (double) this.dataset.getNumberOfClassmembers(i) * 100)
                        + "% of class members" + "</li>";
            }
            out += "</ul>";
        }
    ...
    /**
     * outputs the subtree in one node of the cluster tree we add to the report. this node is collapsed by default. the cluster tree is added as
     * unorded list with sub-items to the report. This functions creates the subtree of this cluster tree in one node, by calling itself recursivley.
     * The subtree is collapsed by default.
     * 
     * @param node the node in the cluster tree, whose subtree shall be created
     * @param parentLevel the number of the level of the parent node - needed to calculate the number of new clusters this cluster remains unchanged
     ...
     * @param nodeInfos WARNING!!! this array contains two different information: at index 0, the maximal number of new clusters a cluster "survived"
     *            is saved. at index 1, the corresponding level is saved. from index i = 2 to n+1 , the depth of the clusternode of level i is saved.
     * @return the array nodeInfos (see above - last parameter), containing the correct values for the subrtree rooted in "node".
     */
    ...
    public void showSDHSwitchBox(int maxs, int step) {
        ...
        for (int i = 1; i < maxs; i += step) {
            ...
            this.writer.appendOutput("<img src=\"" + SOMDescriptionHTML.imgSubdir + "/" + "SDH_pics_" + this.testrun.getRunId() + "/run_"
                    + this.testrun.getRunId() + "_s" + i + "_SDH_Visualization.jpg\" alt=\"SDH Visualisation of som with k = " + i + "\">"
                    + "<br>Visualization 1:SDH Image with <i>s</i> = " + i + "</div>");
        }
        this.writer.appendOutput("<input type=\"button\" name=\"<<\" value=\"<<\" onclick=\"javascript:prevSDH(" + this.testrun.getRunId() + ","
                + step + ")\">" + "&nbsp;&nbsp;&nbsp;<input type=\"button\" name=\">>\" value=\">>\" onclick=\"javascript:nextSDH("
    ...
    public void showTrustworthyNessSwitchBox(int maxs, int step) {
        ...
        this.testrun.createTrustworthyNessImages(this.imgDir, "TrustworthyNessImages_Visualization.jpg", 30, false, step);
        this.writer.appendOutput("<br>");
        ...
        for (int i = 1; i < maxs; i += step) {
            ...
            this.writer.appendOutput("<img src=\"" + SOMDescriptionHTML.imgSubdir + "/" + "TrustworthyNessImages_pics_" + this.testrun.getRunId()
                    + "/run_"+this.testrun.getRunId()+ "_tw_" + i
                    + "TrustworthyNessImages_Visualization.jpg\" alt=\"TrustworthyNessImages Visualisation of som with k = " + i + "\">"
                    + "<br>Visualization 1:TrustworthyNessImages Image with <i>k</i> = " + i + "</div>");
        }
        this.writer.appendOutput("<input type=\"button\" name=\"<<\" value=\"<<\" onclick=\"javascript:prevTW(" + this.testrun.getRunId() + ","
                + step + ")\">" + "&nbsp;&nbsp;&nbsp;<input type=\"button\" name=\">>\" value=\">>\" onclick=\"javascript:nextTW("
    ...
    public void showTopographicProductSwitchBox(int maxs, int step) {
        ...
        this.testrun.createTopographicProductImages(this.imgDir, "TopographicProduct_Visualization.jpg", 30, false, step);
        this.writer.appendOutput("<br>");
        this.writer.appendOutput("<script language\"JavaScript\"  type =\"text/javascript\">" + /*
                                                                                                 * Initialize the ToporaphicProduct picture pointer
                                                                                                 * array
        ...
        for (int i = 1; i < maxs; i += step) {
            ...
            this.writer.appendOutput("<img src=\"" + SOMDescriptionHTML.imgSubdir + "/" + "TopographicProduct_pics_" + this.testrun.getRunId() + "/run_"+this.testrun.getRunId()
                    + "_tp_" + i + 
                     "TopographicProduct_Visualization.jpg\" alt=\"TopographicProduct Visualisation of som with k = " + i + "\">"
                    + "<br>Visualization 1:TopographicProduct Image with <i>k</i> = " + i + "</div>");
        }
        this.writer.appendOutput("<input type=\"button\" name=\"<<\" value=\"<<\" onclick=\"javascript:prevTP(" + this.testrun.getRunId() + ","
                + step + ")\">" + "&nbsp;&nbsp;&nbsp;<input type=\"button\" name=\">>\" value=\">>\" onclick=\"javascript:nextTP("
                + this.testrun.getRunId() + "," + step + ")\">" + "&nbsp;&nbsp;&nbsp</center><br>");
    }
    ...
    public void printRegionalOverviewReport() {
        ...
        for (int i = 0; i < sGrid.NUMBER_OF_REGIONS; i++) {
            ...
            if (EP.includeRegionReport()) {
                if( this.dataset.classInfoAvailable())
                    ...
                    this.writer.appendOutput(" A Regional Class Report could not be generated, since a (*.clss) File was missing during creation of this report.");
            }
        }
    ...
    public void printQualityMeasureReport() {
        ...
        this.writer.appendOutput("Quality Measure Values were classified in 5 classes:" + this.qm.getClassIdentifier(5) + ", "
                + this.qm.getClassIdentifier(4) + ", " + this.qm.getClassIdentifier(3) + ", " + this.qm.getClassIdentifier(2) + " and "
                + this.qm.getClassIdentifier(1) + " Values.");
        for (int i = 0; i < qualifiers.size(); i++) {
            this.writer.appendOutput("<p class=\"header\"><h3><a name =\"3." + (i + 1) + " " + (String) qualifiers.get(i)
                    + "\"><font color=\"black\">3." + (i + 1) + " " + (String) qualifiers.get(i) + ":</a></h3></font></p>");
            ...
            if (((String) qualifiers.get(i)).equals("Quantization Error")) {
                ...
                this.writer.appendOutput("Minimal (mapped) quantization error of input vector: " + String.format("%.5f", mini.getQE())
                        + getInputCoords(mini) + "\n");
                maxi = this.testrun.getMaxInputMappedQE();
                this.writer.appendOutput("\\item Maximal (mapped) quantization error of input vector: " + String.format("%.5f", maxi.getQE())
                        + getInputCoords(maxi) + "\n");
            }
            this.writer.appendOutput("</div>");
        }
    ...
    public void includeAdditionalVisualizations() {
        this.writer.appendOutput("<p class=\"header\"><h3><a name =\"3." + (qualifiers.size() + 1) + " Additional Visualizations "
                + "\"><font color=\"black\">3." + (qualifiers.size() + 1) + " Additional Visualizations :</a></h3></font></p>");
        ...
        if (this.EP.includeTPVisualization()) {
            String topProduct = this.testrun.getScientificDescription("Topographic Product");
        ...
        if (this.EP.includeTWVisualization()) {
            String Trustworthy = this.testrun.getScientificDescription("Trustworthyness");
        ...
        if (this.EP.includeFlowBorderLinePic()) {
            ...
            String flowBorderLine = this.testrun.getScientificDescription("FlowBorderline");
            ...
            this.writer.appendOutput("<br><center><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.testrun.getRunId()
        ...
        if (this.EP.includeDistPic()) {
            ...
            String Distortion = this.testrun.getScientificDescription("Distortion Values");
