 ...
 * @version $Id: Trustworthiness_NeighborhoodPreservation.java 2874 2009-12-11 16:03:27Z frank $
 */
public class Trustworthiness_NeighborhoodPreservation extends AbstractQualityMeasure {
        ...
        for (int j = 0; j < samplecount; j++) {
            ...
            for (int n = 0; (n < samplecount) && (outputDistances[j][n].getMinRank() <= _K); n++) {
                ...
                for (int m = 0; m < samplecount; m++) {
                    if (inputDistances[j][m].getTag() == tag) // if found
                        if (inputDistances[j][m].getMinRank() > _K) // If not one of the k-next, so IN set Uk
    ...
    private void calculateNP() {
        ...
        for (int j = 0; j < samplecount; j++) {
            ...
            for (int n = 0; (n < samplecount) && (inputDistances[j][n].getMinRank() <= _K); n++) {
                ...
                for (int m = 0; m < samplecount; m++) {
                    if (outputDistances[j][m].getTag() == tag) // found
                        if (outputDistances[j][m].getMinRank() > _K) // If NICHT in one of the k next, in IN set Uk
            ...
            for (int s = 0; s < samplecount; s++) {
                ...
                for (int t = 0; t < samplecount; t++) {
                    if (s == t)
                        ...
                        inputDistances[s][t] = new DistanceTag(t, metric.distance(svec, data.getInputDatum(t).getVector().toArray()));
                }
            ...
            for (int s = 0; s < samplecount; s++) {
                ...
                for (int t = 0; t < samplecount; t++) {
                    if (s == t)
                        ...
                        double distance = Math.sqrt((s_x - t_bmu.getXPos()) * (s_x - t_bmu.getXPos()) + (s_y - t_bmu.getYPos())
                                * (s_y - t_bmu.getYPos()));
    ...
    private void Rank(DistanceTag[][] distances, int s, int samplecount) {
        ...
        for (int roadrunner = 1; roadrunner < samplecount; roadrunner++) {
            ...
            {
                distances[s][roadrunner].setRank((double) samplecount);
