 ...
 * @version $Id: IntrinsicDistance.java 2874 2009-12-11 16:03:27Z frank $
 */
public class IntrinsicDistance extends AbstractQualityMeasure {
    ...
    public IntrinsicDistance(Layer layer, InputData data) {
        ...
        for (int x = 0; x < xSize1; x++) {
            for (int y = 0; y < ySize1; y++) {
                UnitSummand1[x][y] = 0.0; // Summand 1: enthaelt Distanz eines jeden Samples zu seiner BMU, gemappt auf die Unit
                UnitSummand2[x][y] = 0.0; // Summand 2: Distanz von BMU (eines Samples) zu 2nd ueber Dijkstra, gemappt auf die Unit
            }
        ...
        try {
            for (int i = 0; i < unitcount; i++) // 4 Nachbarunits bestimmen, Distanzen dazu
            {
                ...
                for (int p = 0; p < 4; p++) {
                    ...
                    if (neigh4 != null) {
                        UnitDistance ud = new UnitDistance(neigh4, metric.distance(V[i].getWeightVector(), neigh4.getWeightVector()));
                        quad.putUnitDistance(p, ud);
                    }
                }
                ((UnitInfo) units.get(V[i])).setQuad(quad); // die 2-4 nachbarn und die jeweilige distanz
            }
    ...
    private void Dijkstra(Unit[] V, int unitcount, Unit bmu, Unit sbmu, Hashtable<Unit, UnitInfo> units) {
        ...
        // relax bmu
        QuadUnitDistance q = ((UnitInfo) units.get(bmu)).getQuad();
        for (int i = 0; i < 4; i++) {
            UnitDistance ud = q.getUnitDistance(i);
            if (ud != null)
                Relax((UnitInfo) units.get(bmu), (UnitInfo) units.get(ud.getUnit()), ud.getDistance());
        }
        ...
        while (true) {
            ...
            for (int i = 0; i < unitcount; i++)
                if (!s.contains(V[i]) && (((UnitInfo) units.get(V[i])).getDistance() < mindistance)) {
                    mindistance = ((UnitInfo) units.get(V[i])).getDistance();
                    minunit = V[i];
                }
            ...
            // relax minunit
            QuadUnitDistance q1 = ((UnitInfo) units.get(minunit)).getQuad();
            for (int i = 0; i < 4; i++) {
                UnitDistance ud1 = q1.getUnitDistance(i);
                if (ud1 != null)
                    Relax((UnitInfo) units.get(minunit), (UnitInfo) units.get(ud1.getUnit()), ud1.getDistance());
            }
