 ...
 * @version $Id: GrowingCellStructuresMapOutputter.java 2874 2009-12-11 16:03:27Z frank $
 */
public class GrowingCellStructuresMapOutputter extends SOMLibMapOutputter {

    public static void write(GrowingCellStructures csom, String fDir, String fName, boolean gzipped, SOMProperties somProps, FileProperties fileProps)
            throws IOException {
    ...
    private static void writeWeightVectorFile(GrowingCellStructures csom, String fDir, String fName, boolean gzipped) throws IOException {
    ...
    private static void writeUnitDescriptionFile(GrowingCellStructures csom, String fDir, String fName, boolean gzipped) throws IOException {
        if (!fDir.endsWith(File.separator)) {
            fDir = fDir + File.separator;
        }
                    ...
                    if (u != null) {
                        ...
                        if (u.getLayer().getQualityMeasure() != null) {
                            if (u.getLayer().getQualityMeasure().getClass().getName().equals("at.tuwien.ifs.somtoolbox.layers.quality.QuantizationError")) {
                                try {
                                    bw.write("$QUANTERROR_UNIT " + u.getLayer().getQualityMeasure().getUnitQualities("qe")[i][j]);
                                    bw.newLine();
                                    bw.write("$QUANTERROR_UNIT_AVG " + u.getLayer().getQualityMeasure().getUnitQualities("mqe")[i][j]);
                                    bw.newLine();
    ...
    private static void writeMapDescriptionFile(GrowingCellStructures csom, String fDir, String fName, SOMProperties somProps,
        ...
        while (maps.size() > 0) { // process all remaining maps
            GrowingCellStructures csommap = (GrowingCellStructures) maps.pop();
            ...
            try {
                ...
                mapDescription.setProperty(SOMLibMapDescription.TOPOLOGY, somProps.tau() == 1 ? csommap.getLayer().getZSize() > 1 ? "cuboid" : "rect"
                        : "gg");
                ...
                mapDescription.setProperty(SOMLibMapDescription.URL_DATA_WINNER_MAPPING, getDataWinnerMappingFileName(fDir, fName,
                        csommap.getLayer().getIdString(), false));
                mapDescription.setProperty(SOMLibMapDescription.URL_CLASS_INFO, ""); // TODO
                mapDescription.setProperty(SOMLibMapDescription.METRIC, csommap.getLayer().getMetric().getClass().getName());
    ...
    public static void writeDataWinnerMappingFile(GrowingCellStructures csom, InputData data, int numWinners, String fDir, String fName,
        ...
        for (int d = 0; d < numVectors; d++) {
            ...
            for (int w = 0; w < numWinners; w++) { // was: gsom.getLayer().getUnitCount()
                bw.write(" " + winners[w].getUnit().getXPos() + " " + winners[w].getUnit().getYPos() + " " + winners[w].getUnit().getZPos() + " "
