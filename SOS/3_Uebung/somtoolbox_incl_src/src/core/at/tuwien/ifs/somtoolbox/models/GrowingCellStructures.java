...
/**
 * This class implements the Growing Cell Structures. It is basically a wrapper for the {@link at.tuwien.ifs.somtoolbox.layers.GrowingCellLayer} and mainly
 * handles command line execution and parameters. It implements the {@link at.tuwien.ifs.somtoolbox.models.NetworkModel} interface which is currently not
 * used, but may be used in the future.
 * 
 * @author Johannes Inführ
 * @author Andreas Zweng
 * @version $Id: GrowingCellStructures.java 2874 2009-12-11 16:03:27Z frank $
 */
public class GrowingCellStructures extends AbstractNetworkModel {
    ...
    /**
     * Constructs a new <code>GrowingCellStructures</code> with <code>dim</code>-dimensional weight vectors. Argument <code>norm</code> determines
     * whether the randomly initialized weight vectors should be normalized to unit length or not.
     * 
    ...
    public static void main(String[] args) {
        ...
        try {
            GrowingCellStructuresMapOutputter.write(som, fileProps.outputDirectory(), fileProps.namePrefix(false), true, somProps, fileProps);
        } catch (IOException e) {
        ...
        if (!skipDataWinnerMapping) {
            ...
            try {
                GrowingCellStructuresMapOutputter.writeDataWinnerMappingFile(som, data, numWinners, fileProps.outputDirectory(),
