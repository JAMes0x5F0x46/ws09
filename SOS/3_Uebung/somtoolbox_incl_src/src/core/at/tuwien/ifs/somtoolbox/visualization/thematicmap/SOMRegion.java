 ...
 * @version $Id: SOMRegion.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMRegion extends Polygon implements IPnt {
    ...
    public SOMRegion(Unit unit, SOMLibClassInformation classInfo, PaintList paintList, int zoom) {
        this.center = new Pnt(unit.getXPos() * zoom + RegionManager.BORDER, unit.getYPos() * zoom + RegionManager.BORDER);
        this.unit = unit;
        ...
        for (int i = 0; i < unit.getNumberOfMappedInputs(); i++) {
            values[classInfo.getClassIndex(unit.getMappedInputName(i))] += 1;
        }
        ...
        if (!resolved) {
            ...
            if (chessboard) {
                ...
                for (int i = 0; i < grids.size(); i++) {
                    ...
                    g.fillRect((int) grid.topLeft.coord(0), (int) grid.topLeft.coord(1), (int) Grid.SIZE, (int) Grid.SIZE);
                }
            }
        }
    }
