 ...
 * @version $Id: MultiDocumentSummarisationPanel.java 2931 2009-12-14 23:38:58Z mayer $
 */
public class MultiDocumentSummarisationPanel extends JPanel implements ActionListener {
    ...
    public MultiDocumentSummarisationPanel(NavigationPanel navPanel, Scorer sc, ResultHandler rh, DocumentDisplayer docdis) {
        ...
        similaritySpinner.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                ...
                documentdispl.showAllResults(resulth.getMultiResultDocs(), resulth.getMultiResultScores(), resulth.getMultiResultFilenames(),
                        navP.isScores(), navP.getTypeHighlight(), navP.isFileNames());
            }
        });
    ...
    private void showAll() {
        ...
        if (getMDSType() == MULTI_SDS) {
            for (int i = 0; i < this.navP.itemNames.length; i++) {
                documentdispl.showOriginal(i, resulth.getResultDoc(i), resulth.getDocumentScores(i), navP.isScores(), navP.getTypeHighlight(),
                        navP.isFileNames());
            }
        }
        if (getMDSType() == MULTIDOC_SUM || getMDSType() == SIM_METHOD) {
            documentdispl.showOriginal(resulth.getMultiResultDocs(), resulth.getDocumentScores(), navP.isScores(), navP.getTypeHighlight(),
                    navP.isFileNames());
        }
    ...
    private void method_1() {
        ...
        for (int i = 0; i < this.navP.getFileNames().size(); i++) {
            ...
            documentdispl.showResult(i, resulth.getResultDoc(i), resulth.getResultScores(i), navP.isScores(), navP.getTypeHighlight(),
                    navP.isFileNames());
        }
    }
