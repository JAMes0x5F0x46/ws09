...
\begin{alertblock}{}
Matrix- und graph-controlled Grammatiken können in Arraygrammatiken direkt \"{u}bergef\"{u}hrt werden, indem ihre Produktionen durch Arrayproduktionen ersetzt werden.
\end{alertblock}
}
...
\[ G_t = (V_N,V_T,(P,R,F),S),
\]
$G = (V_N,V_T,P,S)$ ist eine kontextfreie Grammatik, $R$ ist eine endliche Menge von Teams aus $P$ und $F$ ist die Menge von Produktionen, die bei dem appearence checking \"{u}bersprungen werden können. Ist $F= \emptyset$, dann kann $G_t$ als Grammatik mit prescribed teams ohne appearence checking bezeichnet werden.
}
...
\item Preprocessing
\begin{itemize}
	\item Normalisierung und Rauscheliminierung: anschließend abbilden auf eine 20 x 25 Gitter
	\item Ausdünnung (Skelletierung)
\end{itemize}
...
\frame{\frametitle{Theorie und Realität (1)} 
\begin{columns}
\begin{column}{7cm}

Zur Ableitung der horizontalen Linie muss z.B. zusätzlich zu der Produktion
...
\frame{\frametitle{Theorie und Realität (2)} 
...
\begin{itemize}
	...
	\end{itemize}
	\item Buchstaben verschiedener Größe über Sprachen in $PT^{\left\lceil k \right\rceil} (2$-$cf)$ charakterisieren
\end{itemize}
}
