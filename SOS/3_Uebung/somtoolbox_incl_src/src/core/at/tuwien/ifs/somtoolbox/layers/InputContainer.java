 ...
 * @author Khalid Latif
 * @version $Id: InputContainer.java 2874 2009-12-11 16:03:27Z frank $
 */
public class InputContainer {
     ...
     * ordering by ascending distance. If two inputs have the same distance, ordering is lexicographically.
     * 
    ...
    protected void addMappedInput(String name, Double dist) {
        ...
        for (int i = 0; i < inputNames.size(); i++) {
            if (dist0 <= ((Double) inputDistances.get(i)).doubleValue() && name.compareTo(inputNames.get(i)) < 0) {
    ...
    public double[] getMappedInputDistances() {
        ...
        for (int i = 0; i < inputDistances.size(); i++) {
            res[i] = ((Double) inputDistances.get(i)).doubleValue();
        }
        return res;
    }

    /**
     * Returns the distance between this unit's weight vector and the vector of the mapped input data at the specified <code>index</code>.
     * 
    ...
    public String getMappedInputName(int index) {
        return (String) inputNames.get(index);
    }
