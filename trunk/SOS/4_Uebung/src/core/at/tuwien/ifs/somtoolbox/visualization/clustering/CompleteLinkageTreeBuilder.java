 ...
 * This class is not compatible with mnemonic SOMs (and probably also not 
 * compatible with hierarchical SOMs) 
 * The updating uf the distances can probabla be optimised (see {@link WardsLinkageTreeBuilderAll} - lazyUpdate) 
 * 
 * @author Angela Roiger
 * @version $Id: CompleteLinkageTreeBuilder.java 2874 2009-12-11 16:03:27Z frank $
 */
public class CompleteLinkageTreeBuilder extends TreeBuilder {
     ...
     *  
     *  This code is only compatible with rectangular, non hierarchical SOMs!
     *  
        ...
        while (dists.size() > 0) {
            ...
            for (Iterator<NodeDistance> i = dists.iterator(); i.hasNext();) {
                ...
                if ((x.n1 == newNode) || (x.n2 == newNode)) {
                    ...
                    // replaces any existing enty with the same pair
            ...
            // contains the longest distances betweet 2 clusters
                ...
                try {
                    for (int k = 0; k < xdim; k++) {
                        for (int l = 0; l < ydim; l++) {
                            // if ((i!=k)&&(j!=l))
                            if (!((i == k) && (j == l))) {
                                dists.add(new NodeDistance(tmp[i][j], tmp[k][l], l1.distance(units[i][j].getUnit().getWeightVector(),
