 ...
 * @version $Id: QMContainer.java 2874 2009-12-11 16:03:27Z frank $
 */
public class QMContainer {
    ...
    private ArrayList damagedcriteriaQM = new ArrayList();;
    
    private HashMap QualityIdentifier = new HashMap();
    ...
    public double[][] getUnitQualities(String Name) {
        return (double[][]) UnitQualities.get(Name);
    }
    ...
    public Unit[] getMaxUnit(String Name, Unit[] Area) {
        ...
        Unit[] units = null;
        errors = (double[][]) this.UnitQualities.get(Name);
        ...
        if (errors != null) {
            try {
                if (Area != null) {
                    for (Unit u : Area) {
                        if (errors[u.getXPos()][u.getYPos()] > maxError && run.getGrowingSOM().getLayer().hasMappedInput(u.getXPos(), u.getYPos())) {
                            // new maximum
                ...
                for (int i = 0; i < argmaxx.size(); i++) {
                    units[i] = this.run.getGrowingSOM().getLayer().getUnit(argmaxx.get(i).intValue(), argmaxy.get(i).intValue());
                }
                return units;
            ...
            try {
                if (Area != null) {
                    ...
                    for (int i = 0; i < errors.length; i++) {
                        for (int j = 0; j < errors[i].length; j++) {
                            // look whether the te is smaller
                            if (errors[i][j] != 0 && errors[i][j] < minError && run.getGrowingSOM().getLayer().hasMappedInput(i, j)) {
                                // new maximium
                                minError = errors[i][j];
                                argmaxx.removeAllElements();
                                argmaxy.removeAllElements();
    ...
    public void classifyUnits(String Name) {
        ...
        double[][] currQual = (double[][]) this.UnitQualities.get(Name);
        int[][] currclassQual = new int[currQual.length][currQual[0].length];
        double min = -1, max = -1;

        // rudi: inverted this check, as it was *NOT* preventing a nullpointer exception: if the key is NOT contained, get will always return null!
        if (this.UnitMINQualities.containsKey(Name)) { // Very bad programming, u i s never used :(   
            min = this.UnitMINQualities.get(Name);
        }

        // rudi: inverted this check, as it was *NOT* preventing a nullpointer exception: if the key is NOT contained, get will always return null!
        if (this.UnitMAXQualities.containsKey(Name)) { // Very bad programming, u i s never used :(
    ...
    public int[][] getClassifiedUnits(String Name) {
        int[][] values = (int[][]) this.UnitQualitiesClassified.get(Name);
        return values;
    }

    /**
     * Returns the number of classified units for class clss and identifier Name
     * 
     * @param Name
     * @param clss
     * @return
    ...
    /**
     * NOT USEDCURRENTLY returns the averager distance from all units, classified with "clss" from QM "Name"
     * 
     * @param Name
     * @return
     */
    ...
    /**
     * Retunrs the Map qualities of the "Name" Qualitymeasure
     * 
     * @param Name
     * @return
     */
    public Double getMapQualities(String Name) {
        return (Double) this.MapQualities.get(Name);
    }

    /**
     * creates an image visualizing the Quality Measure "Name" of this SOM the image is saved under the given path and name, although a prefix
     * run_runId_ is added to the image name
     * 
    ...
    public double getUnitMAXQualities(String Name) {
        return (Double) this.UnitMAXQualities.get(Name);
    }

    public double getUnitMINQualities(String Name) {
        return (Double) this.UnitMINQualities.get(Name);
    }

    /**
     * Retunrs the stringrepresnation of the INt value c
     * 
     * @param c
     * @return
     */
    ...
    public int getMaximumClassifiedRegionValue(Unit[] region, String Name) {
        int[][] classArray = (int[][]) this.UnitQualitiesClassified.get(Name);
    ...
    /**
     * Compares the 2 QM with each other,i.e looks through all units (only in the Region if region != NULL) and looks for similar values at the same
     * Units (TRY: Enlarge the search radius to a certain neighborhood radius (1))
     * 
     * @param Name1
     * @param Name2
     * @param Region
     * @param, Type 1 = Max-Max, 2 = Min-Min, 3 = Max-Min, 4 = Min-Max Compares according to Min /Max Values return a Array [0] gives th percentage
     *         ofsimilarity, [1] gives the absolute difference in strength between max/min 1/2, [2] the region of intersection(1-9)
     */
    public double[] compareQualities(String Name1, String Name2, Unit[] Region, int type) {
        int[][] classArray1 = (int[][]) this.UnitQualitiesClassified.get(Name1);
        int[][] classArray2 = (int[][]) this.UnitQualitiesClassified.get(Name2);
        ...
        for (Unit u : Region) {
            ...
            if (classArray1[u.getXPos()][u.getYPos()] == minmax1 || classArray1[u.getXPos()][u.getYPos()] == (minmax1 - 1)
                    || classArray1[u.getXPos()][u.getYPos()] == (minmax1 + 1)) {
                counter1++;
                if (classArray2[u.getXPos()][u.getYPos()] == minmax2 || classArray2[u.getXPos()][u.getYPos()] == (minmax2 - 1)
    ...
    /**
     * Sets the SemanticGrid for calculations PLEASE NOTE: only the "Master- Grid can be set here" -> wich is the Grid over the full Map, inluding all
     * ClassReports
     */
     ...
     * @return an Array of arraylists, built up as following: out[0] = high Density out[1] = low Density out[2] = mixed classes out[3] = one class
     *         out[4] = Units on Map edges out[5] = Units not on Map edges out[6] = Having neighboring empty units out[7] = having no neighboring empt
     *         unist out[8] = Units on Cluster edges out[9] = Units in Clusters out[10] = Big Distance of mapped Vectors to Prototype vector. out[11]
     *         = Small Distance of mapped Vectors to Prototype vector. out[12] = Big average Distance of mapped Vectors compared to each other.
     *         out[13] = Small average Distance of mapped Vectors compared to each other. out[14] = Input Vectors equally distributed over the Map
     *         out[15] = Input Vectors not equally distributed over the Map out[16] = Intra Cluster Distances == 0, Inter Cluster Distances != 0
     *         out[17] = Intra Cluster Distances != 0, Inter Cluster Distances == 0
     */
    public QMConfigurationProfile getOriginalConfiguration(String qualifier, int type) {
        ...
        if (type == 1) { // return how the QM should be ideally in the original configuration when looked at MAXIMUM Values
        ...
        if (type == 2) { // return how the QM should be ideally in the original configuration when looked at MINIMUM Values
            if (qualifier.equals("Topographic Error")) {
                out_array.createNewElement(1);
                out_array.insert(1,1.0);
    ...
    public QMConfigurationProfile getActualConfiguration(String qualifier, Unit[] units, int type) {
        ...
        if (type == 1) { // WHat should be taken into account when looked at MAX properties
            if (qualifier.equals("Mean Quantization Error")) {
                ...
                for (Unit unit : units) { // Test all units
                    boolean LowDens = this.hasLowDensity(unit);
                ...
                String pLD_MQE = (units.length - out_array.lengthOfElement(1)) + " / " + units.length;// 1.0 -(double)out_array[1].size()/(double)units.length;
            ...
            if (qualifier.equals("Topographic Error")) {
                ...
                String pHD_TE = (units.length - out_array.lengthOfElement(0)) + " / " + units.length;// 1.0 -(double)out_array[1].size()/(double)units.length;
                ...
                String pME_TE = (units.length - out_array.lengthOfElement(4)) + " / " + units.length;// 1.0 - (double)out_array[4].size()/(double)units.length;
            ...
            if (qualifier.equals("Intrinsic Distance")) {
                ...
                for (Unit unit : units) { // Test all units
                    ...
                    Boolean hasMixedClasses = this.hasMixedClasses(unit);
                    boolean isMapedge = this.isOnMapEdge(unit);
                    ArrayList isClusterEdge = this.isOnClusterEdge(unit);
                    Boolean neighborsempty = this.hasNeighboringEmptyUnits(unit);
                    int hh = 7;
                    if (HighDens = false) {
                        out_array.insert(0,unit);
        ...
        if (type == 2) { // WHat should be taken into account when looked at MIN properties
            if (qualifier.equals("Mean Quantization Error")) {
                ...
                String pHD_MQE = (units.length - out_array.lengthOfElement(0)) + " / " + units.length;// 1.0 -(double)out_array[1].size()/(double)units.length;
            ...
            if (qualifier.equals("Topographic Error")) {
                ...
                for (int i = 0; i < units.length; i++) { // Test all units
                    boolean LowDens = this.hasLowDensity(units[i]);
                    Boolean isSingleClasses = this.hasSingleClass(units[i]);
                    boolean isNotOnMapedge = !this.isOnMapEdge(units[i]);
                    ArrayList isNotClusterEdge = this.isOnClusterEdge(units[i]);
                ...
                String pHD_TE = (units.length - out_array.lengthOfElement(1)) + " / " + units.length;// 1.0 -(double)out_array[1].size()/(double)units.length;
                ...
                String pME_TE = (units.length - out_array.lengthOfElement(5)) + " / " + units.length;// 1.0 - (double)out_array[4].size()/(double)units.length;
    ...
    /**
     * Tests wheter the Unit U is having a High Mapped Input Density This is made upon an assumption, we say a unit is highly dense when the average
     * value of the first 15% of units with highest values is equal or obove that value that
     * 
     * @param u
    ...
    /**
     * Returns weter there are mixed classes on the Unit 
     * @EDIT If no clss file was used during report Creation, die measure is left out
     * 
     * @param u
     * @return
     */
    ...
    /**
     * true if Unit is on the Map Edge
     * 
     * @param u
     * @return
     */
     ...
     * @EDIT this need a clss file to b presents, returns null otherwise
     */
    public ArrayList<SemanticClass> isOnClusterEdge(Unit u) {
        ...
        if(this.sGrid.SemanticClassesIndex!=null){
            Iterator clsses = sGrid.SemanticClassesIndex.values().iterator();
            out = new ArrayList<SemanticClass>();
            while (clsses.hasNext()) {
                SemanticClass s = (SemanticClass) clsses.next();
    ...
    /**
     * Returns true if 8-fold neighborship shows any Units with no Inputs
     * * @EDIT this need a clss file to b presents, returns null otherwise
     * 
     * @param u
     * @return
    ...
    public Boolean hasNeighboringEmptyUnits(Unit u) {
        ...
        for (int x_new = -1; x_new <= 1; x_new++) {
            for (int y_new = -1; y_new <= 1; y_new++) {
                ...
                c[0] = u.getXPos() + x_new;
                c[1] = u.getYPos() + y_new;
                if(c[0] >= 0 && c[0] <= this.sGrid.XLength-1 && c[1] >= 0 && c[1] <= this.sGrid.YLength-1 && c[0] != u.getXPos() && c[1] != u.getYPos()){
                    SemanticNode s = this.getNode(c);
                    if (s != null) {
                        out = s.containsNoClasses(); // true if no classes , null if no class file was given
                        if(out != null) // Test if we found one empty unit
                            if(out)
     ...
     * @EDIT a node is null if no clss file was attached
     * 
     * @param coords
     * @return
     */
        ...
        if(this.sGrid.labels != null){
            for (SemanticNode[] label : this.sGrid.labels) {
                for (int j = 0; j < label.length; j++) {
                    coords = label[j].realCoordinates;
     ...
     * 
     * @param quality_list_index, gives a Description to the Index of the quality List below quality_list[0] = high Density quality_list[1] = low
     *            Density quality_list[2] = mixed classes quality_list[3] = one class quality_list[4] = Units on Map edges quality_list[5] = Units not
     *            on Map edges quality_list[6] = Having neighboring empty units quality_list[7] = having no neighboring empt unist quality_list[8] =
     *            Units on Cluster edges quality_list[9] = Units in Clusters quality_list[10] = Big Distance of mapped Vectors to Prototype vector.
     *            quality_list[11] = Small Distance of mapped Vectors to Prototype vector. quality_list[12] = Big average Distance of mapped Vectors
     *            compared to each other. quality_list[13] = Small average Distance of mapped Vectors compared to each other. quality_list[14] = Input
     *            Vectors equally distributed over the Map quality_list[15] = Input Vectors not equally distributed over the Map quality_list[16] =
     *            Intra Cluster Distances == 0, Inter Cluster Distances != 0 quality_list[17] = Intra Cluster Distances != 0, Inter Cluster Distances
     *            == 0
     */
    public String getQualityIdentifier(int quality_list_index) {
        String out = (String) this.QualityIdentifier.get(quality_list_index);
        if(out == null)
    ...
    public ArrayList getClassFileDependantQualities(){
