...
\]
Der Vektor $(0,...,0) \in Z^n$ wird als $\Omega_n$ bezeichent.
}
...
$p$ kann als $\Lambda$-frei bezeichnet werden, falls $shape(A_2) \not = 0$ ist.
\[ shape(A_i) = \{v \in W \ | \ A_i(v) \not = \# \}, \ 1 \leq i \leq 2
\]
%\newline
...
\[ G_M = (V_N,V_T,(M,F),S),
\]
$M$ ist eine endliche Menge von Matrizen, $M = \{ m_i \ | \ 1 \leq i \leq n \}$. Die Matrizen $m_i$ sind Sequenzen von der Form 
\[m_i = (m_{i,1},...,m_{i,n_i}), \ n_i \geq 1, \ 1 \leq i \leq n.
\] 
...
\]
Falls $\varphi (l(r))$ leer ist f\"{u}r alle $r \in R$, dann kann $G_P$ als graph-controlled Grammatik ohne appearence checking bezeichnet werden. \newline
...
\end{eqnarray*}
Andere Art von endlicher Index Restriktion nur Objekte $w$ beachtet, die von einer Grammatik $G$ mit einer Ableitung $ind_{G,Y}(w) \leq k$ f\"{u}r $Y \in \{min,max\}$ erzeugt werden.
}
...
\newline
Wiederum beschr\"{a}nken wir die Anzahl von non-terminal Symbolen, die in einem Zwischenableitungsschritt vorkommen d\"{u}rfen, indem nur Teams angewendet werden d\"{u}rfen, die alle non-terminale im Zwischenschritt betreffen $\Rightarrow$ durch max. Anzahl an kontextfreien Produktionen in einem Team beschr\"{a}nkt.\newline
$L^{\left\lceil k \right\rceil} (G)$ bezeichnet nun die Sprache, die durch die Grammatik $G$ mit endlicher Index Restriktion von $k$ erzeugt wird. 
}
...
\section{Endliche Index Restriktion} 
\frame{\frametitle{Gefolgerte Resultate} 
$PT_{ac}^{\left\lceil k \right\rceil}(cf)$ bezeichnet die Sprachfamilie, die von String-Grammatiken mit prescribed teams aus kontextfreien Produktionen mit endlicher Index Restriktion erzeugt werden.\newline
\newline
$PT_{ac}^{\left\lceil k \right\rceil}(X)$ bezeichnet die Familie aller Arraysprachen, die von Array-Grammatiken mit prescribed teams  aus Array-Produktionen des Types $X$, mit $X \in \{n$-$\#$-$cf, n$-$cf, n$-$scf \ | \ n \geq 1 \}$ erzeugt werden.
\newline
...
\frame{\frametitle{Gefolgerte Resultate (3) - 1-dimensionaler Fall}
\begin{alertblock}{Theorem 4}
	F\"{u}r alle $X\in \left\{ 1$-$cf_1, 1$-$scf_1\right\}$ gilt
\[PT^{\left\lceil  1\right\rceil} (X) = PT_{ac}^{\left\lceil  1\right\rceil} (X) = Z^{Y,\left\lceil 1 \right\rceil} (X) = Z^{min,\cap \left\lceil 1 \right\rceil} (X) =
 L(1\mathrm{-}reg)\]
...
\begin{itemize}
	...
	\item Array-Grammatiken verarbeiten nicht nur Symbol- sondern auch Positionsinformationen $\Rightarrow$ Ein-Weg Automat nicht m"oglich
	\item selbe Information darf nur einmal gelesen werden\newline
\end{itemize}
\pause

Ein Ableitungsschritt f"ur ein selektiertes Team ist nur m"oglich, wenn
\begin{itemize}
	...
	\item Jede Position eines Terminalsymbol im aktuellen Array muss mit der Position des originalen Arrays "ubereinstimmen.
\end{itemize}
}

\frame{\frametitle{Definition}
Ein n-dimensionaler k-head finite Array-Automat vom Typ $X$ ($X \in \{n$-\#-$cf, n$-$cf, n$-$scf, n$-$cf_1, n$-$scf_1 \ | \ n \geq 1 \}$) ist folgenderma"sen definiert
...
\begin{itemize}
	...
	\item f"ur jede Array-Produktion $p \in P$, ist $p$ von der Form	
\end{itemize}
\[ p = (W,\{(\Omega_n,A)\} \cup \{(v,\#) \ | \ v \in W \backslash \{\Omega_n\}\}, \{(v,X_v) \ | \ v \in W\})
\]
}

\frame{\frametitle{Funktionsweise}
Der Automat $M$ arbeitet auf einem gegebenen Array aus $V_T^{*n}$ nun wie folgt:\newline
$M$ arbeitet auf Objekten von $\{(a,a),(a,X) \ | \ a \in V_T,X \in V_N \cup \{\#\}\}^{*n}$, wo die erste Komponente das gegebene Array beinhaltet und die zweite Komponente das bis jetzt durch $G$ generierte Array.
\newline
\newline
...
\pause
\begin{alertblock}{Definition der durch den Automaten $M$ akzeptierten Array-Sprache}

\begin{eqnarray*}
L(M) = \{A \ | \ A \in V_T^{*n},\{(v,(A(v),\#)) \ | \ v \in shape(A)\} \Longrightarrow_M^* \\ \{(v,(A(v),A(v))) \ | \ v \in shape(A)\}\}
\end{eqnarray*}
\end{alertblock}
...
\begin{itemize}
	\item Regulated Array-Grammatiken mit endlichen Index ist geeignete Methode f"ur syntaktisches Pattern Recognition
	\item M"ogliche Erweiterung zur Erkennung von 3-dimensionalen Objekten \newline
\end{itemize}
