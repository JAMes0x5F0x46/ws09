...
public class PlaybackThread extends Thread {
    ...
    protected byte[][] buffer = { new byte[1 * 1024 * datalineframesize], new byte[1 * 1024 * datalineframesize] }; // the buffer
    ...
    /**
     * Convenience constructur that takes a <code>TPlaybackThreadDataRecord</code> and pulls all data from it to create a new thread
     * <p>
        ...
        try {
            ...
            for (int j = 0; j < record.avMusic.length; j++) {
                ...
                if (vTmp.size() >= 1) {
                    ...
                    aFiles[j] = (File[]) vTmp.toArray(new File[0]);
                    vTmp.clear();
                } else {
    ...
    public AudioInputStream getNextSong(int channel, boolean stats) throws IOException, UnsupportedAudioFileException {
        ...
        if (bRepeat_Shuffle) {
            ...
            if (aFiles[channel].length > 1) {
                ...
                Vector tmp = new Vector(Arrays.asList(aFiles[channel]));
                tmp.remove(0);
                aFiles[channel] = (File[]) tmp.toArray(new File[0]);
            } else {
        ...
        if (tempfile.getName().endsWith(".wav")) {
            ...
            if (Commons.lhmDecoded.containsKey(tempfile)) {
                ...
                tempfile = (File) Commons.lhmDecoded.get(tempfile);
