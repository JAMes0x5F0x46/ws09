...
/**
 * This class gives a semantic interpretation of the Coordinates of the Units of a SOM.It calculates North/South/east/West and Middle Regions of the
 * given Array of UnitNodes. This is used for the semantic Report of the ReportGenerator.
 * 
...
public class SemanticInterpreterGrid {
    ...
    public SemanticInterpreterGrid(Unit[] units, TestRunResult testrun, boolean master, int Region, int type, String imgDir) {
        ...
        int[] xPartition;
        int[] yPartition;
        if (XLength <= 2) // If the side is too short, computate simpler approach
            xPartition = computeMiddlePartition(XLength);
        else
            xPartition = getPartition(XLength);
        if (YLength <= 2) // If the side is too short, computate simpler approach
        ...
        if (master && XLength >= 10 && YLength >= 10) { /* Only makes sense if the Grid is sufficient large */
            ...
            SemanticInterpreterGrid NW = new SemanticInterpreterGrid(cutArray(0, xPartition[0] - 1, 0, yPartition[0] - 1), testrun, false, 1,
                    this.type, this.imgDir);
            SemanticInterpreterGrid NMiddle = new SemanticInterpreterGrid(cutArray(xPartition[0], xPartition[1], 0, yPartition[0] - 1), testrun,
                    false, 2, this.type, this.imgDir);
            SemanticInterpreterGrid NE = new SemanticInterpreterGrid(cutArray(xPartition[1] + 1, XLength - 1, 0, yPartition[0] - 1), testrun, false,
                    3, this.type, this.imgDir);
            SemanticInterpreterGrid WMiddle = new SemanticInterpreterGrid(cutArray(0, xPartition[0] - 1, yPartition[0], yPartition[1]), testrun,
                    false, 4, this.type, this.imgDir);
            SemanticInterpreterGrid MiddleMiddle = new SemanticInterpreterGrid(cutArray(xPartition[0], xPartition[1], yPartition[0], yPartition[1]),
                    testrun, false, 5, this.type, this.imgDir);
            SemanticInterpreterGrid EMiddle = new SemanticInterpreterGrid(cutArray(xPartition[1] + 1, XLength - 1, yPartition[0], yPartition[1]),
                    testrun, false, 6, this.type, this.imgDir);
            SemanticInterpreterGrid SW = new SemanticInterpreterGrid(cutArray(0, xPartition[0] - 1, yPartition[1] + 1, YLength - 1), testrun, false,
                    7, this.type, this.imgDir);
            SemanticInterpreterGrid SMiddle = new SemanticInterpreterGrid(cutArray(xPartition[0], xPartition[1], yPartition[1] + 1, YLength - 1),
                    testrun, false, 8, this.type, this.imgDir);
            SemanticInterpreterGrid SE = new SemanticInterpreterGrid(cutArray(xPartition[1] + 1, XLength - 1, yPartition[1] + 1, YLength - 1),
                    testrun, false, 9, this.type, this.imgDir);
    ...
    /**
     * Estimates the Areals for the Middle part of the given Axis
     * 
     * @param axislength
     */
    ...
    /**
     * Computes the Middle Partition (& let it grow) for the giveb axis
     * 
     * @param axislength
     * @return
     */
    ...
    /**
     * Computates the Areals for the left(type==1) or right (type==2) side of the Middle part for the given Axis and divides the parts equally among
     * the granularity measures defines by the [] Gravity
     * 
     * @param axislength
     * @param type
     * @return
    ...
    /**
     * return sthe semantic node array
     * 
     * @return
     */
    ...
    /**
     * Creates a small overview on the abilities of the Region (1-9)
     * 
     * @param index the region to be adressed
     * @return
     */
    /*
     * public String createRegionReport(int index){ }
     */
    /**
     * Creates all the Info needed for Report @ param regularGrid true ifthe Grid is regularsquared
     */
    public void initializeGridwithClasses(boolean regularGrid) {
        // ArrayList<int[]> UnitswithClassList;
        double[] currUnitClasses;
     ...
     * @param QMIntersection [0] = Name of Quality Measure, [1] = 1 = MAX, 2 = MIN, [3..length] intersectio nunits of the QM to be analyzed
     */
    public String createClassReportOnGrid(int indexReport, int clusterlevelReport, ArrayList QMReportIntersection) {
        ...
        // wich class can be found most often
        ...
        while (io.hasNext()) {
            SemanticClass s = (SemanticClass) io.next();
        ...
        while (ii.hasNext()) {
            ...
            SemanticClass sem = (SemanticClass) ii.next();
            ...
            if (QMReportIntersection != null) {
                ...
                for (int i = 0; i < this.qm.UnitQualityMeasureNames.size(); i++) { // for all qualityMeasures
                    ...
                    if (!QMReportIntersection.get(0).equals(this.qm.UnitQualityMeasureNames.get(i))) { // that are not the current qualifier
                        ArrayList<int[]> classUnits = this.TestRun.getAllUnitsContainingClass(sem.index);
                        for (int j = 0; j < classUnits.size(); j++) { // for all units
                            int[] currUnit = (int[]) classUnits.get(j);
                            int[][] qmUnits = this.qm.getClassifiedUnits((String) this.qm.UnitQualityMeasureNames.get(i));
                            if ((Integer) QMReportIntersection.get(1) == 1) { // MAX-MAX REPORT
                                if (qmUnits[currUnit[0]][currUnit[1]] == 5 || qmUnits[currUnit[0]][currUnit[1]] == 4) // we have a maximum of another
                                    // qm there aswell
                                    tempIntersectMAX.add(this.getNode(currUnit));
                            }
                            if ((Integer) QMReportIntersection.get(1) == 2) { // MIN-MIN REPORT
                                if (qmUnits[currUnit[0]][currUnit[1]] == 2 || qmUnits[currUnit[0]][currUnit[1]] == 1) // we have a minimum of another
                                    // qm there aswell
                                    tempIntersectMIN.add(this.getNode(currUnit));
                            }
                            if ((Integer) QMReportIntersection.get(1) == 3) { // MAX-MIN REPORT
                                if (qmUnits[currUnit[0]][currUnit[1]] == 5 || qmUnits[currUnit[0]][currUnit[1]] == 4) // we have a minimum of another
                                    // qm there aswell
                                    tempIntersectMAX.add(this.getNode(currUnit));
                                if (qmUnits[currUnit[0]][currUnit[1]] == 2 || qmUnits[currUnit[0]][currUnit[1]] == 1) // we have a minimum of another
                                    // qm there aswell
                        ...
                        if (tempIntersectMAX.size() > 0) { // we had intersectional nodes with other QM
                            currClassIntersectionsMAX.add(this.qm.UnitQualityMeasureNames.get(i)); // QM Name on uneven Places;
                        ...
                        if (tempIntersectMIN.size() > 0) { // we had intersectional nodes with other QM
                            currClassIntersectionsMIN.add(this.qm.UnitQualityMeasureNames.get(i)); // QM Name on uneven Places;
            ...
            if (indexReport != 0) { // Report on Partial Grid
                if (sem.regionmix[indexReport - 1] > 0) { // Class from this Region
                    RegionsharedClasses += sem.SharedClasses; // how many classes share their points with how many other classes
                ...
                if (QMReportIntersection != null) {
                    ...
                    for (int k = 2; k < QMReportIntersection.size(); k++) {
                        ...
                        double[] classes = this.TestRun.getClassesForUnit(s.realCoordinates[0], s.realCoordinates[1]);
        ...
        if (this.type == 1) { // HTML REPORT
            if (classeshmap.size() > 0) {
                ...
                out += (ClassesOutsideofRegion.size() + ClassesCompleteinRegion.size())
                        + " ("
                        + Math.round((((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) / (double) this.TestRun.getNumberofClasses()) * 100)
                ...
                if (ClassesCompleteinRegion.size() > 0 || ClassesOutsideofRegion.size() > 0) {
                    if (indexReport != 0 && ClassesCompleteinRegion.size() > 0) {
                        out += "<a href=\"javascript:showSemanticClassesinRegion('in',"
                                ...
                                + "("
                                + Math.round((double) ClassesCompleteinRegion.size()
                                        / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100) + ")%</a> ";
                        out += "<div id =\"classes_in_" + indexReport + "_" + this.TestRun.getRunId() + "\" style=\"display:none\">";
                        // out+="<b>(</b>"+inside+"<b>)</b>";
                    ...
                    if (clusterlevelReport != 0 && ClassesCompleteinRegion.size() > 0) {
                        out += "<a href=\"javascript:showSemanticClassesinRegion('in',"
                                ...
                                + Math.round((double) ClassesCompleteinRegion.size()
                                        / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100) + ")%</a> ";
                        out += "<div id =\"classes_in_" + (10 + clusterlevelReport) + "_" + this.TestRun.getRunId() + "\" style=\"display:none\">";
                        // out+="<b>(</b>"+inside+"<b>)</b>";
                        out += this.showClassInfos(ClassesCompleteinRegion, null);
                        out += "</div> are only found in this Sector. ";
                    }
                    if (QMReportIntersection != null && ClassesCompleteinRegion.size() > 0) {
                        out += "<a href=\"javascript:showSemanticClassesinRegion('in','"
                                ...
                                + Math.round((double) ClassesCompleteinRegion.size()
                                        / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100) + ")%</a> ";
                        out += "<div id =\"classes_in_" + QMReportIntersection + "_" + this.TestRun.getRunId() + "\" style=\"display:none\">";
                        // out+="<b>(</b>"+inside+"<b>)</b>";
                    ...
                    if (indexReport != 0 && ClassesOutsideofRegion.size() > 0) {
                        out += "<a href=\"javascript:showSemanticClassesinRegion('out',"
                                ...
                                + "("
                                + Math.round((double) ClassesOutsideofRegion.size()
                                        / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100) + ")%</a> ";
                        out += "<div id =\"classes_out_" + indexReport + "_" + this.TestRun.getRunId() + "\" style=\"display:none\">";
                        // out+="<b>(</b>"+outside+"<b>)</b>";
                    ...
                    if (clusterlevelReport != 0 && ClassesOutsideofRegion.size() > 0) {
                        out += "<a href=\"javascript:showSemanticClassesinRegion('out',"
                                ...
                                + Math.round((double) ClassesOutsideofRegion.size()
                                        / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100) + ")%</a> ";
                        out += "<div id =\"classes_out_" + (10 + clusterlevelReport) + "_" + this.TestRun.getRunId() + "\" style=\"display:none\">";
                        // out+="<b>(</b>"+outside+"<b>)</b>";
                        out += this.showClassInfos(ClassesOutsideofRegion, null);
                        out += "</div>";
                    }
                    if (QMReportIntersection != null && ClassesOutsideofRegion.size() > 0) {
                        out += "<a href=\"javascript:showSemanticClassesinRegion('out','"
                                ...
                                + Math.round((double) ClassesOutsideofRegion.size()
                                        / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100) + ")%</a> ";
                        out += "<div id =\"classes_out_" + QMReportIntersection + "_" + this.TestRun.getRunId() + "\" style=\"display:none\">";
                        // out+="<b>(</b>"+outside+"<b>)</b>";
        ...
        if (this.type == 2) { // LATEX REPORT
            if (ClassesCompleteinRegion.size() > 0 || ClassesOutsideofRegion.size() > 0) {
                out += (ClassesOutsideofRegion.size() + ClassesCompleteinRegion.size())
                        ...
                        + "("
                        + Math.round((double) ClassesOutsideofRegion.size()
                                / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100) + ")% are"
                        + " spread among ";
                if (indexReport != 0) {
                    String regions = "";
                    for (int i = 0; i < regionMix.length; i++) {
        ...
        if (QMReportIntersection == null) {
            ...
            if (EmptyNodes.size() > 0) {
                out += "The empty Nodes of this  Region are found in " + EmptyNodesTrail.size() + " connected 'Trails'.";
            }
        }
        if (ClassesCompleteinRegion.size() > 0 || ClassesOutsideofRegion.size() > 0) {
            out += overallCompactness
                    + " ("
                    + Math.round(((double) overallCompactness / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size())) * 100)
            ...
            out += FullyConnected
                    + " ("
                    + Math.round((double) FullyConnected / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100)
            ...
            if (QMReportIntersection == null) {
                ...
                if (InOutRatio < 0.6) { // Only 40% of the regions classes are seperated within
                    ...
                    out += "Each class shares its units with an average "
                            + String.format("%.2f", (double) RegionsharedClasses
                            ...
                            + String.format("%.2f", (double) TotalsharedClasses / (double) this.TestRun.getNumberofClasses()) + ". ";
                    double percentage = ((double) RegionsharedClasses / (double) (ClassesOutsideofRegion.size() + ClassesCompleteinRegion.size()))
    ...
    /**
     * Creates a Class Repot on Class with index j, saves it in the SemantiClass currClass and puts it on the SemanticNode
     * 
     * @param j
     */
    public void createClassReport(int j) {
        if (!this.SemanticClassesIndex.containsKey(j)) { // Only calculate if we didnt see this class so far
            ...
            // nOw that we know the ClassMiddle, try to give information if the class is connected, if it is centered around the middle point, or so
            ...
            for (int i = 0; i < UnitswithClassList.size(); i++) {
                ...
                if (n != null) {
                    ...
                    concentrationnmix[n.Region - 1] += (double) temp[2];
                    double[] otherClasses = this.TestRun.getClassesForUnit(temp[0], temp[1]);
            ...
            for (int i = 0; i < concentrationnmix.length; i++) {
                concentrationnmix[i] = concentrationnmix[i] / (double) currClass.MemberCount * 100;
            }

            currClass.addRegionMix(regionmix); // In how many of the Regions is the Class present
            currClass.addConcentrationMix(concentrationnmix); // How many Members of the Class are seperated over Regions
            String report = "";
            if (this.type == 1)
            ...
            report += "Around its Class Center " + String.format("%.2f", currClass.getCompactness() * 100) + " % of the Class Units are located.";
            ...
            if (currClass.ClassPartitions.size() > 1) {
                ...
                for (int z = 0; z < currClass.ClassPartitions.size(); z++) {
                    int temp[] = (int[]) currClass.ClassPartitions.get(z);
                    SemanticNode n;
                    ...
                    if (!location.containsKey(n.Region))
                        ...
                        int nodes[] = (int[]) location.get(n.Region);
                ...
                while (node_values.hasNext()) {
                    int reg = (Integer) node_key.next();
                    int[] nodes = (int[]) node_values.next();
                    counter++;
                ...
                for (int c = 0; c < trail.length; c++) {
                    int[] array = (int[]) UnitswithClassList.get(c);
                    amount += array[2];
                }
                report += ". Whereof the biggest connected Cluster is maintained in the " + getRegion(m.Region) + " containing "
                        + String.format("%.2f", ((double) trail.length / (double) UnitswithClassList.size() * 100))
                        + "% of all of the Class units and " + String.format("%.2f", (double) amount / (double) currClass.MemberCount * 100)
                        + " % of its members. ";
            } else {
            ...
            report += "The furthest Class member of " + this.TestRun.datasetInfo.getNameOfClass(j) + " is located in the "
                    + SemanticInterpreterGrid.getRegion(t.Region);
                ...
                for (int p = 0; p < this.TestRun.getNumberofClasses(); p++) {
                    if (classmix[i][p] > 0) {
                        if (p != j) {
                            ...
                            for (int z = i + 1; z < UnitswithClassList.size(); z++) {// Look for the same classon other units
            ...
            if (ClassList.size() > 0) {
                ...
                for (int i = 0; i < ClassList.keySet().size(); i++) {
                    int classindex = (Integer) ii.next();
                    ...
                    while (ia.hasNext()) {
                        int classindex1 = (Integer) ia.next();
                ...
                for (int i = 0; i < ClassList.keySet().size(); i++) {
                    int classindex = (Integer) bb.next();
                ...
                if (ClassList.size() > 1) {// we have more than one shared class, make more calculations...
                    ...
                    ArrayList[] sharedClasses = new ArrayList[2]; // Contains the classes on [0] and their shared units on [1];
                    ...
                    for (int i = 0; i < maximumSharedValues.size(); i++) {
                        ...
                        if (!sharedClasses[0].contains(temp[0].get(0))) {
                            ...
                            if (maximumSharedValues.size() > 1) // if we have enough class entries to possible build a trail
                                // sharedClasses[0] = getclassTrail(maximumSharedValues,(Integer)temp[1].get(0),sharedClasses[0]);
                                sharedClasses = getclassTrail(maximumSharedValues);
                            else
                                sharedClasses[0].add(temp[1].get(0)); // we have just 2 classes that share the same units
                    ...
                    for (int i = 0; i < maximumUnitValues.size(); i++) {
                        report += this.TestRun.datasetInfo.getNameOfClass((Integer) maximumUnitValues.get(i)) + ", ";
                    }
                        ...
                        for (int i = 0; i < Container.size(); i++) {
                            ...
                            for (int o = 0; o < l[0].size(); o++) {
                                ...
                                if (o == l[0].size() - 1) {// G
                                    ...
                                    classes += this.TestRun.datasetInfo.getNameOfClass((Integer) l[0].get(o)) + " intersecting "
                                            + this.TestRun.datasetInfo.getNameOfClass(j) + " ";
                                    ArrayList interS = currClass.getIntersectionDirections(sharedClasses[1]);
                                    for (int n = 0; n < interS.size(); n++) {
                                        classes += interS.get(n) + ", ";
                                    }
                                    classes += "of its Class Center in a total amount of " + biggestInteresctionAmount + " units";
                                } else {
                                    ArrayList h = (ArrayList) ClassList.get(l[0].get(o));
                            ...
                            if (Count > 0) {
                                if (Count == 1) {
                                    ...
                                    report += Math.round((double) Count / (double) maximumUnitValues.size() * 100) + " % of these "
                                            + maximumUnitValues.size() + " Classes are also Part of a the biggest Interesecting Area.";
                                }
                            }
                    ...
                    while (iu.hasNext()) {
                        int classindex = (Integer) iu.next();
            ...
            for (int i = 0; i < UnitswithClassList.size(); i++) { // Save the class on the semantic nodes
                ...
                SemanticNode sNode = (SemanticNode) this.getNode(temp);
                sNode.addClass(currClass);
                currClass.addNode(sNode);
            }
            putClassinContainer(currClass);
            for (int i = 0; i < maximumUnitValues.size(); i++) {
                if (!this.SemanticClassesIndex.containsKey((Integer) maximumUnitValues.get(i))) {
                    this.createClassReport((Integer) maximumUnitValues.get(i));
                }
            }
        } else {
    ...
    /**
     * Compares two array lists and retunr show many elements are equal
     * 
     * @param a1
     * @param a2
     * @return
    ...
    /**
     * Goes through l and tries to see which classes are connected
     * 
     * @param l [0] class 1, [1]class 2
     * @return
     */
    private ArrayList[] getclassTrail(ArrayList l) {
        ...
        for (int i = 0; i < l.size(); i++) {
            ...
            ArrayList class1 = (ArrayList) temp[0];
            ArrayList class2 = (ArrayList) temp[1];
            ArrayList units = (ArrayList) temp[2];
     ...
     */
    /**
     * Used if the Grid wasnt made from a square Grid, so we use the Class Descriptions from a squaredone in SOMDEscriptionHTLM
     * 
     * @param classes
     */
    ...
    /**
     * Gives an arraylist containing all connect/uncopnnected parts of the empty units
     * 
     * @param index
     * @param EmptyNodes
     * @param temptrail
    ...
    public static Unit[] sortNodes(Unit[] units) {
        ...
        while (pointer < units.length) {
            ...
            for (int i = 0; i < units.length; i++) { // add minimal Xvalues to array
            ...
            for (int t = 0; t < minYvalues.size(); t++) { // in this array look fo�r minimal Yvalues
    ...
    public void addClasstoGrid(int j) {
        ...
        SemanticClass sem = (SemanticClass) fatherGrid.getClassfromGrid(j);
    ...
    /**
     * returns a detailed information on the calsses according to the parameters entered during creation
     * 
     * @return
     */
    /*
    ...
    public void putClassinContainer(SemanticClass cl) {
        ...
        if (EP.getMINCompactness() != -1 || EP.getMAXCompactness() != -1) { // Check ob die Klasse die EPvorraussetzungen erf�llt
    ...
    public SemanticClass getClassfromGrid(int index) {
         ...
         * = false; break; } } }
         */
        SemanticClass cl = (SemanticClass) this.SemanticClassesIndex.get(index);
        return cl;
                    ...
                    for (SemanticNode element : m) {
                        ...
                        if (x1 == x2 && y1 == y2 && element.Classes != null) {
                            for (int u = 0; u < element.Classes.size(); u++) {
                                SemanticClass c = (SemanticClass) element.Classes.get(u);
                                boolean ok = true;
                                if (this.EP.getMAXCompactness() != -1 || this.EP.getMINCompactness() != -1) {
                                    if (!c.matchesCompactnessRequirements(this.EP))
                                        ok = false;
    ...
    /**
     * Prints a report on all selected Quality Measures, and adds a picture Frame if a visualization is available qualifiers is a String array,
     * containing all Names from the quality measures
     */
    ...
    public String printQualityMeasureReport(String qualifier) {
        ...
        out += "<ul>\n";
        /* MAP SECTION */
        String curr_desc = this.TestRun.getScientificDescription(qualifier);
        out += "Information on <u onclick=\"javascript:showVisualisationDescriptions('" + curr_desc + "')\"><font color =\"blue\">" + qualifier
                + "</font></u> of  the SOM.<br>";
        if ((k = this.qm.getMapQualities(qualifier)) != null) {
            out += "The Map Quality Measure is the average from the sum of all Units with mapped input";
            out += "<br><li><span class=\"header2\">Map Quality Measure:" + String.format("%.2f", this.qm.getMapQualities(qualifier)) + "</span>";
            out += "</ul></li>";
        }
        /* UNIT SECTION */
        ...
        if (min != null && max != null) {
            out += " Underneath MIN / MAX Values of " + qualifier + "are listed. The Units Coordinates are given in Brackets";
            out += "<br>";
            ...
            out += "<li>Minimal unit quality measure: " + String.format("%.2f", this.qm.getUnitMINQualities(qualifier)) + " (" + unitCoord + ")"
                    + "</li>\n";
            unitCoord = "";
            ...
            out += "<li>Maximal unit quality measure: " + String.format("%.2f", this.qm.getUnitMAXQualities(qualifier)) + " (" + unitCoord + ")"
                    + "</li>\n";
            out += "</ul>";
            ...
            for (int i = 1; i <= 5; i++) {
                double qPerc = (double) this.qm.getNumberOfClassifiedUnits(qualifier, i) / (double) this.TestRun.getSOMDimensions();
                if (qPerc > maxx) {
                    maxx = qPerc;
                    mostCommonValue = i;
                }
            ...
            /*
             * What of the values are outliers.. Since we have 5 classes of strongness, we can assume that if there is a percentage of occurenceof
             * 0.0% the directclass above, or below is an outlier
             */
            ...
            double HighDP = ((double) this.qm.getNumberOfClassifiedUnits(qualifier, 5) / (double) this.TestRun.getSOMDimensions()) * 100;
            double LowDP = ((double) this.qm.getNumberOfClassifiedUnits(qualifier, 1) / (double) this.TestRun.getSOMDimensions()) * 100;
            ...
            if (!outliers.contains(5)) {
                ...
                HighDP += ((double) this.qm.getNumberOfClassifiedUnits(qualifier, 4) / (double) this.TestRun.getSOMDimensions()) * 100;
                ArrayList[] veryStrong = getRegionOccurance(qualifier, 5);
                ArrayList[] Strong = getRegionOccurance(qualifier, 4);
            ...
            if (!outliers.contains(1)) {
                ...
                LowDP += ((double) this.qm.getNumberOfClassifiedUnits(qualifier, 2) / (double) this.TestRun.getSOMDimensions()) * 100;
            ...
            out += HighDN + " (" + (int) Math.round(HighDP) + " %) of all the Units have " + this.qm.getClassIdentifier(5) + high + " Values.";
            out += "They can be found in: " + highRegionDesc + " ";
            if (outliers.contains(5) == true)
                out += "We can consider these values as being outliers, since there are no units on the map with " + this.qm.getClassIdentifier(4)
                        + " Values.";
            ...
            if (this.qm.getClassIdentifier(1).equals(this.qm.getClassIdentifier(mostCommonValue))
                ...
                out += this.qm.getClassIdentifier(1) + low + " Values can be found on " + LowDN + " (" + (int) Math.round(LowDP)
                        + " %) of all Units.";
            } else {
                out += this.qm.getClassIdentifier(1) + low + " Values can be found on " + LowDN + "(" + (int) Math.round(LowDP) + " %) of all Units.";
                out += "They can be found in: " + lowRegionDesc + ". ";
            }
            if (outliers.contains(1) == true)
                out += "We can consider these values as being outliers, since there are no units on the map with " + this.qm.getClassIdentifier(2)
                        + " Values.";
        ...
        if (this.qm.createQualityMeasureImage(imgDir, qualifier + ".jpg", qualifier)) {
            ...
            // add the palette
            if (qualifier.equals("Quantization Error") || qualifier.equals("Mean Quantization Error") || qualifier.equals("Silhouette Value")) {
                int index = 0; // 1st palette
                out += "<br>";
                this.TestRun.createPaletteImage(imgDir, "palette_" + index + ".jpg", "Cartography Color, 128 Gradient (SOMToolbox 0.4.x)");
                out += "<img src=\"" + SOMDescriptionHTML.imgSubdir + "/" + "palette_" + index + ".jpg" + "\" alt=\"visualization of the palette "
                        + index + "\"/>";
            }
        ...
        if (this.qm.UnitQualityMeasureNames.contains(qualifier)) { // && this.TestRun.datasetInfo.classInfoAvailable()) {
            ...
            out += this.createQMComparisonReportonGrid(qualifier, 0); // Create a QM Comparison reporton qualifier and the whole map
        }
        return out;
    }

    /**
     * PRetunr an ArrayList with the SemanticNodes of ocurrances of theQualityMeasure "Name" and the Class "clss"
     * 
     * @param Name
     * @param clss
     */
    public ArrayList<SemanticNode>[] getRegionOccurance(String Name, int clss) {
        ...
        int[][] values = (int[][]) this.qm.getClassifiedUnits(Name);
        ...
        if (regions[0].size() > 0 && regions[1].size() > 0 && regions[2].size() > 0) {
            ...
            for (int i = 0; i < max.size(); i++) {
                for (int j = 0; j < globalMAX.size(); j++) {
                    if ((Integer) max.get(i) == (Integer) globalMAX.get(j)) {
                        globMax.add((Integer) max.get(i));
                        intersect.add((Integer) max.get(i));
            ...
            if (globMax.size() > 0) {
                ...
                for (int z = 0; z < globMax.size(); z++) {
                    if (z < globMax.size() - 1)
                        s += getRegion((Integer) globMax.get(z) + 1) + "(" + regions[(Integer) globMax.get(z)].size() + ") and ";
                    else
            ...
            if (max.size() > 0) {
                if (s.charAt(s.length() - 1) == '.') {
                    ...
                    for (int v = 0; v < max.size(); v++) {
                        if (!globMax.contains((Integer) max.get(v))) {
                            if (v < max.size() - 1)
                                s += getRegion((Integer) max.get(v) + 1) + "(" + regions[(Integer) max.get(v)].size() + ") and ";
                            else
                    ...
                    for (int k = 0; k < max.size(); k++) {
                        if (k < max.size() - 1)
                            s += getRegion((Integer) max.get(k) + 1) + "(" + regions[(Integer) max.get(k)].size() + ") and ";
                        else
        ...
        if (west == true && south == true && north == true && east == true) {
            ...
            for (int i = 0; i < globalMAX.size(); i++) {
                if (i < globalMAX.size() - 1)
                    s += getRegion((Integer) globalMAX.get(i) + 1) + "(" + regions[i].size() + ") and ";
                else
                    s += getRegion((Integer) globalMAX.get(i) + 1) + "(" + regions[i].size() + ")";
            }
            s += ".";
            temp.add(s);
            ...
            for (int i = 0; i < intersectGlobalMax.size(); i++) {
                Integer index_to_remove = (Integer) intersectGlobalMax.get(i);
     ...
     * @param wanteRegions: specifies a selectrion of Region, NULL if whole map regions wanted
     * @return
     */
    public ArrayList<Integer> getMaxRegionValue(ArrayList[] regions, Integer[] wantedRegions) {
        ...
        for (int i = 0; i < out.size(); i++) {
            int index = (Integer) out.get(i);
            if (max == regions[index].size())
                out1.add((Integer) out.get(i));
        }
        return out1;
    }

    /**
     * Creates a Comparison report on all Quality Measures(qualifier=null), or on a specific Quality Measure(=Name), on the given Region, or on the
     * whole grid if the Region is 0
     * 
     * @param Region
     * @param qualifier
     * @return
    ...
    public String createQMComparisonReportonGrid(String qualifier, int Region) {
        String out = "";
        Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").info("Making Semantic Quality Measure Interpretation for " + qualifier + " in region " + Region);
        Unit[] units = null;
            ...
            for (int i = 0; i < temp.size(); i++) {
                ...
                try {
                    ...
                    Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning(
                            "Cannot acces Units for Quality Measure Report for run " + this.TestRun.getRunId() + ". Reason: " + e);
                }
            }
        } else
            // units = this.units;
        ...
        ArrayList maxmaxValues = new ArrayList();
        ArrayList minminValues = new ArrayList();
        ArrayList maxminValues = new ArrayList();
        ArrayList minmaxValues = new ArrayList();
        ArrayList maxmaxUnitValues = new ArrayList();
        ArrayList minminUnitValues = new ArrayList();
        ArrayList maxminUnitValues = new ArrayList();
        ArrayList minmaxUnitValues = new ArrayList();

        if (qualifier == null) { // Reporton all Quality Measures
            for (int i = 0; i < this.qm.UnitQualityMeasureNames.size(); i++) {
                String qualifier1 = (String) this.qm.UnitQualityMeasureNames.get(i); // 1st Quality Measure(0..n)
                ...
                for (int j = 0; j < this.qm.UnitQualityMeasureNames.size(); j++) { // 2nd Quality Measure (i+1...n)
                    String qualifier2 = (String) this.qm.UnitQualityMeasureNames.get(j);
                ...
                if (tempStorageMAXMAX.size() > 0) {
                    for (int v = 0; v < tempStorageMAXMAX.size() - 1; v = v + 2) {
                        ...
                        q = (ArrayList) maxmaxValues.get(i);
                        if (a[0] == maxmax) {
                            int index = (Integer) tempStorageMAXMAX.get(v + 1);
                            q.add((String) this.qm.UnitQualityMeasureNames.get(index));
                        }
                    }
                    q.add(a[0]);
                }
                ...
                if (tempStorageMINMIN.size() > 0) {
                    for (int v = 0; v < tempStorageMINMIN.size() - 1; v = v + 2) {
                        ...
                        q = (ArrayList) minminValues.get(i);
                        if (a[0] == minmin) {
                ...
                if (tempStorageMAXMIN.size() > 0) {
                    for (int v = 0; v < tempStorageMAXMIN.size() - 1; v = v + 2) {
                        ...
                        q = (ArrayList) maxminValues.get(i);
                        if (a[0] == maxmin) {
                ...
                if (tempStorageMINMAX.size() > 0) {
                    for (int v = 0; v < tempStorageMINMAX.size() - 1; v = v + 2) {
                        ...
                        q = (ArrayList) minmaxValues.get(i);
                        if (a[0] == minmax) {
            ...
            String qualifier1 = "";
            for (int i = 0; i < this.qm.UnitQualityMeasureNames.size(); i++) {
                if (this.qm.UnitQualityMeasureNames.get(i).equals(qualifier)) // foudn wanted QM
                    qualifier1 = (String) this.qm.UnitQualityMeasureNames.get(i);
            }
            ArrayList qualifiermaxmaxValues = new ArrayList();
            qualifiermaxmaxValues.add(qualifier1);
            maxmaxValues.add(qualifiermaxmaxValues);
            ...
            if (tempStorageMAXMAX.size() > 0) {
                for (int v = 0; v < tempStorageMAXMAX.size() - 1; v = v + 2) {
                    a = (double[]) tempStorageMAXMAX.get(v);
                    q = (ArrayList) maxmaxValues.get(0);
                    if (a[0] == maxmax) {
                        int index = (Integer) tempStorageMAXMAX.get(v + 1);
                        q.add((String) this.qm.UnitQualityMeasureNames.get(index));
                    }
            ...
            if (tempStorageMINMIN.size() > 0) {
                for (int v = 0; v < tempStorageMINMIN.size() - 1; v = v + 2) {
                    a = (double[]) tempStorageMINMIN.get(v);
                    q = (ArrayList) minminValues.get(0);
                    if (a[0] == minmin) {
                        int index = (Integer) tempStorageMINMIN.get(v + 1);
                        q.add((String) this.qm.UnitQualityMeasureNames.get(index));
                    }
            ...
            if (tempStorageMAXMIN.size() > 0) {
                for (int v = 0; v < tempStorageMAXMIN.size() - 1; v = v + 2) {
                    a = (double[]) tempStorageMAXMIN.get(v);
                    q = (ArrayList) maxminValues.get(0);
                    if (a[0] == maxmin) {
                        int index = (Integer) tempStorageMAXMIN.get(v + 1);
                        q.add((String) this.qm.UnitQualityMeasureNames.get(index));
                    }
            ...
            if (tempStorageMINMAX.size() > 0) {
                for (int v = 0; v < tempStorageMINMAX.size() - 1; v = v + 2) {
                    a = (double[]) tempStorageMINMAX.get(v);
                    q = (ArrayList) minmaxValues.get(0);
                    if (a[0] == minmax) {
                        int index = (Integer) tempStorageMINMAX.get(v + 1);
                        q.add((String) this.qm.UnitQualityMeasureNames.get(index));
                    }
        ...
        if (this.type == 1) {// html Report
            ...
            for (int i = 0; i < maxmaxValues.size(); i++) {
                ArrayList list = (ArrayList) maxmaxValues.get(i);
                if (list.size() > 1) {
                    out += (String) list.get(0) + " shares its maximum Values with " + (list.size() - 2)
                            + " other Quality Measures to a percentage of " + Math.round((Double) list.get(list.size() - 1) * 100);
                    out += "<br>";
                    ArrayList semnodearray = new ArrayList();
                    semnodearray.add((String) list.get(0));
                    semnodearray.add(1);
                    for (int j = 1; j < list.size() - 1; j++) {
                        out += "<li>";
                        out += "<b>" + (String) list.get(j) + ":</b><br>";
                        // Say where we have the intersections
                        ArrayList[] intersectionList = this.getQMIntersectionList((String) list.get(0), (String) list.get(j), units, 1);
            ...
            for (int i = 0; i < minminValues.size(); i++) {
                ArrayList list = (ArrayList) minminValues.get(i);
                if (list.size() > 1) {
                    ...
                    for (int j = 1; j < list.size() - 1; j++) {
                        ...
                        out += "<b>" + (String) list.get(j) + ":</b><br>";
                        // Say where we have the intersections
                        ArrayList[] intersectionList = this.getQMIntersectionList((String) list.get(0), (String) list.get(j), units, 2);
                        out += "Intersecting Areas can be found " + this.simplifyRegionDescription(intersectionList);
                        out += "<br>";
            ...
            for (int i = 0; i < maxminValues.size(); i++) {
                ArrayList list = (ArrayList) maxminValues.get(i);
                if (list.size() > 1) {
                    out += (String) list.get(0) + " shows a reverse proportional behavior (MAX - MIN) with " + (list.size() - 2)
                            + " other Quality Measures to a percentage of " + Math.round((Double) list.get(list.size() - 1) * 100);
                    out += "<br>";
                    ArrayList semnodearray = new ArrayList();
                    semnodearray.add((String) list.get(0));
                    ...
                    for (int j = 1; j < list.size() - 1; j++) {
                        ...
                        out += "<b>" + (String) list.get(j) + ":</b><br>";
                        // Say where we have the intersections
                        ArrayList[] intersectionList = this.getQMIntersectionList((String) list.get(0), (String) list.get(j), units, 3);
                        out += "Intersecting Areas can be found " + this.simplifyRegionDescription(intersectionList);
                        out += "<br>";
            ...
            for (int i = 0; i < minmaxValues.size(); i++) {
                ArrayList list = (ArrayList) minmaxValues.get(i);
                if (list.size() > 1) {
                    out += (String) list.get(0) + " shows a reverse proportional behavior (MIN - MAX) with " + (list.size() - 2)
                            + " other Quality Measures to a percentage of " + Math.round((Double) list.get(list.size() - 1) * 100);
                    out += "<br>";
                    ArrayList semnodearray = new ArrayList();
                    semnodearray.add((String) list.get(0));
                    ...
                    for (int j = 1; j < list.size() - 1; j++) {
                        ...
                        out += "<b>" + (String) list.get(j) + ":</b><br>";
                        // Say where we have the intersections
                        ArrayList[] intersectionList = this.getQMIntersectionList((String) list.get(0), (String) list.get(j), units, 4);
                        out += "Intersecting Areas can be found " + this.simplifyRegionDescription(intersectionList);
                        out += "<br>";
    ...
    public String showClassInfos(ArrayList<Integer> classes, Integer type) {
        String out = "";
        if (this.type == 1) {// HTML
            out += "<table border=\"1\"><tbody>" + "<tr>" + "<th>class name</th>" + "<th>mean vector</th>" + "<th>mapped to unit</th>"
                    + "<th>\"mean unit\"</th>" + "<th>radius</th>" + "<th>Values by Coordinates</th>" + "<th><img src=\""
                    + SOMDescriptionHTML.imgSubdir + "/run_" + this.TestRun.getRunId() + "_classDistribution.jpg\" alt=\"class distribution of som "
                    + this.TestRun.getRunId() + "\"/></th>" + "<th>Class Entropy </th><th>Other classes in Area</th>" + "<th><img src =\""
                    + SOMDescriptionHTML.imgSubdir + "/run_" + this.TestRun.getRunId() + "_classEntropy.jpg\" alt=\"class entropy of som "
                    + this.TestRun.getRunId() + "\"/></th>";
            if (type != null) {
                if (type == 1) // MAX-MAX
            ...
            for (int i = 0; i < classes.size(); i++) {
                int cIndex = (Integer) classes.get(i);
                SemanticClass c = (SemanticClass) this.getClassfromGrid(cIndex);
                ...
                out += "<td class=\"middleText\"><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.TestRun.getRunId() + "_classCenter_"
                        + c.index + ".jpg\" alt=\"class center of class " + this.TestRun.datasetInfo.getNameOfClass(c.index) + "\"/></td>"
                        + "<td><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.TestRun.getRunId() + "_classDistribution" + c.index
                        + ".jpg\" alt=\"class distribution of som " + this.TestRun.getRunId() + "\"/></td>" + "<td>"
                        + String.format("%2f", this.TestRun.getClassEntropy(c.index)) + "</td>" + "<td><center>";
                // BREAKPOINT
                ...
                out += "</center></td>" + "<td><img src=\"" + SOMDescriptionHTML.imgSubdir + "/run_" + this.TestRun.getRunId() + "_classEntropy"
                        + c.index + ".jpg\" alt=\"class " + c.index + " entropy of som " + this.TestRun.getRunId() + "\"/></td>";
                if (type != null) {
                    if (type == 1) { // MAX-MAX
                        out += "<td><center>";
    ...
    /**
     * Puts this semanticGrid on the QuMContainer, uised or calculations ONLY the masterGrid should be set here, witch is the one covering the whole
     * Map
     */
     ...
     * 
     * @param QM_compare the QM Identifier that is QM_to is compared to if QM_compare == null, we dont make a comparison, but just a test of the
     *            quality Measure
            ...
            if (act_configuration != null) {
                ...
                for (int i = 0; i < act_configuration.SIZE; i++) {
                    if (!act_configuration.isNullatPos(i)) {
                        if (act_configuration.lengthOfElement(i) > 1) { // We have at least one Unit that fails the conditions on this position
                ...
                if (act_conf.length() > 0) { // we had Units that did not match all conditions
                    act_conf += "</table>";
                    out += "However, the Quality Measure " + QM_to
                    ...
                    if (this.qm.getClassFileDependantQualities().size() > 0) {
                        out += "For the correct Calculation of following Criteria a *.clss File would be needed, but none was give for the creation of this report. Therefore the information could not be interpreted.<br>";
                    ...
                    if (this.qm.getClassFileDependantQualities().size() > 0) {
                        out += ". However for the correct Calculation of following Criteria a *.clss File would be needed, but none was give for the creation of this report <br>";
                ...
                if (orig_conf_compare != null && orig_conf_to != null) {
                    ...
                    if (possible_IntersectionCriteria.size() > 0) {
                        ...
                        for (int b = 0; b < possible_IntersectionCriteria.size(); b++) {
                            out += this.qm.getQualityIdentifier((Integer) possible_IntersectionCriteria.get(b)) + " leads for " + QM_compare
                                    + " to a " + clue_orig_ompare + " and for " + QM_to + " to a " + clue_orig_to + "<br>";
                        }
                        out += "</ul>";
                    }
                }
