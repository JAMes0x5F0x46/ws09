...
/**
 * This class represents the panel for the PlaySOM,<br/> it provides controls for exporting and editing of selected playlists.
 * 
 * @author Michael Dittenbach
 * @version $Id: PlaySOMPanel.java 2874 2009-12-11 16:03:27Z frank $
 */
public class PlaySOMPanel extends AbstractSelectionPanel implements ActionListener {
    ...
    private EvalSOMPanel evalPanel;
    ...
    /**
     * compare "ordering index" of a filename if it is smaller or greater than (a) certain value(s) ordering index is assumed to be a number between
     * two points before the filename extension e.g. mypieceofmusic.0345.mp3 -> ordering index is 345
     * 
     * @param string filename string containing an ordering index (e.g. mypieceofmusic.0345.mp3)
     * @param ind_greater integer, ordering index must be greater in comparison (if set to -1, ind_greater will be ignored)
     * @param ind_smaller integer, ordering index must be smaller in comparison (if set to -1, ind_smaller will be ignored)
     * @return true if comparison result is true, false if comparison result is false or ordering index could not be parsed
     */
    private boolean compareOrderIndex(String string, int ind_greater, int ind_smaller) {
        ...
        if (p2 == -1) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning("Index of . could not be found in filename '" + string + "'");
            return false;
        } else if (p2 == 0) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning("Second last . could not be found in filename '" + string + "'");
            return false;
        }
        ...
        try {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning("Order index could not be found in filename '" + string + "'! Search failed!");
            return false;
        }
    ...
    /**
     * traverses all units of the map and searches for the query string as substring of the document filenames updates the playlist with the filenames
     * found and marks the according units on the map
     * 
     * @param query the string for querying the filenames
     * @param createHistogram if set to true, the results is shown in an Islands of Music / SDH like manner if set to false, the units containing the
     *            items found are marked with a single color
     */
    private void searchFilenames(String query, boolean createHistogram) {
        ...
        double[][] hist = null; // storing unit counts as histogram
        String[] mappedNames;
        int nfound = 0;
        int ind_greater = -1;
        int ind_smaller = -1;
                ...
                if (units[x][y] != null) {// check for MnemonicSOM
                    ...
                    if (mappedNames != null) {
                        for (int i = 0; i < mappedNames.length; i++) {
                            if (mappedNames[i].indexOf(query) != -1) {
                                boolean add = true;
                                if ((ind_greater > -1) || (ind_smaller > -1)) {
                                    add = compareOrderIndex(mappedNames[i], ind_greater, ind_smaller);
                                }

                                if (add) {
                                    // add to playlist
                                    addToList(mappedNames[i], units[x][y]);
                                    nfound++;
                                    if (createHistogram) {
                                        hist[x][y] += 1;
                                    } else {
        ...
        if (createHistogram) {
            state.mapPNode.showHistogramOverlayVisualization(hist, rbSmoothed.isSelected() ? 1 : 0); // 2nd param is visualization index
        }
    }
