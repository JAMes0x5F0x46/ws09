...
/**
 * FIXME 2: most probably all the methods in this class should be part of {@link InputData} ! <br>
 * this class collects all available information about the values in the input dataset, like from the input file, the template vector file, ... and
 * maybe computes some properties of its own. It's job is to give one centralized placed where the actual report generators (the output object) can
 * ask for the data.
 * 
 * @author Sebastian Skritek (0226286, Sebastian.Skritek@gmx.at)
 * @version $Id: DatasetInformation.java 2979 2009-12-16 23:15:34Z mayer $
 */
public class DatasetInformation {
     ...
     */
    public DatasetInformation(Vector<Integer> selectedIndices, String inputDataFilename, String tvFilename, String classInformationFile,
    ...
    public DatasetInformation(Vector<Integer> selectedIndices, String inputDataFilename, String tvFilename, String classInformationFile,
            EditableReportProperties EP, CommonSOMViewerStateData state) {
    ...
    /**
     * returns whether class information are attached to the input vectors does not check whether it is a valid file, only whether a String with
     * length > 0 has been specified as path
     * 
     * @return true if a class information file (.clsinf) has been specified, false otherwise
     */
    ...
    /**
     * returns the number of input vectors used for training the SOM, that is the number of different vectors present in the input file for the SOM
     * training.
     * 
    ...
    /**
     * returns whether our heuristic estimates this dimension to contain discrete values This is the case, if all values in this dimension are exact
     * integer values.
     * 
     ...
     * returns the requested value describing the distribution of the input values. The types of information available are described by the constant
     * members of this class (this function returns numerical properties):
     * <ul>
     ...
     * 
     * @param type specifies the type of information to be returned: allowed are some constants defined by this class (see above)
    ...
    public double getNumericalDataProps(int type, int attribute) {
        ...
        switch (type) {
            ...
            default:
                Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning(
                        "Requested unknown dataset distribution value: " + type + " (in DatasetInformation.getNumericalDataProps())");
                return -1;
        }
    }
     ...
     * FIXME: split this into simple single getter methods... !<br>
     * returns the requested value describing the distribution of the input values. The types of information available are described by the constant
     * members of this class (this function returns boolean properties):
     * <ul>
    ...
    public boolean getBoolDataProps(int type, int attribute) {
        ...
        switch (type) {
            ...
            default:
                Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning(
                        "Requested unknown dataset distribution value: " + type + " (in DatasetInformation.getBoolDataProps())");
                return false;
        }
    ...
    /**
     * returns the label (that is the name defined for an attribute in the template vector file) for the specified attribute. If no template file is
     * given, only the index of the attribute is returned.
     * 
    ...
    /**
     * runs over all dimensions of the input vectors and tries to fetch some information about their data ranges and other properties information
     * gathered are:
     * <ul>
    ...
    /**
     * returns the InputData object storing information about the input data used for training the som. Needed by objects of type TestRunResult for
     * some analysis
     * 
    ...
    /**
     * returns the InputDatum labeld with the specified name
     * 
     * @param name
     * @return
     */
    ...
    /**
     * returns the id of the inputVector at position index in the list of selected inputs each input vector is identified by an id, which is its index
     * in the complete input. The vectors selected by the user (to display their position on the SOM) are also stored in a list. To retrieve the
     * "real" id of the vector at position index in this list, this function should be used
     * 
    ...
    /**
     * returns the complete filename of the file containing the input data complete filename means including the path. The string is not verified to
     * point to a valid input file (or any file at all).
     * 
    ...
    /**
     * returns the complete filename of the file containing the template data complete filename means including the path. The string is not verified
     * to point to a valid template file (or any file at all).
     * 
    ...
    /**
     * Tries to name a cluster by the input data mapped to units lying within the cluster For naming the cluster, some very simple heuristics are
     * used: First, if there are any labels of the clusters, which correpsond to 0/1 attributes, and their values are all 0 (or 1) in the cluster, the
     * name of this attribute is included to the name of the cluster. (attributes of 0/1 type are supposed to encode any "has this property" yes/no
     * information, thereby the value 1 is interpreted as "cluster has this property", whereas 0 is interpreted as "has not") If there are any labels
     * that don't correspond not 0/1 attributes, it is checked whether both subclusters have the same value for this label. If yes, the name of this
     * label is included to the name of the cluster If none of the properties above is valid, the first nodeDepth-1 labels of the cluster suggested by
     * the clustering algorithm is used. (at least for the animal map this works quite well)
     * 
     * @param node the node representing the cluster tha shall be named
     * @param clusterByValue indicates whether the labels for the cluster shall be created by value (is handed unchanged to
     *            ClusterNode.getLabels(clusterByValue, boolen)
     * @param nodeDepth the depth of the node in the tree, whereby the root (i.e. the cluster containing the whole map) node has depth 1
    ...
    /**
     * This method calculates the most important Dimensions of the Dataset according to the results of a PCA, and rows the resulting dim-index in a
     * new array on first index. On index 2, the corresponding % of the TotalVariance is calculated (as a quality measure)
     * 
    ...
    public double[][] getPCAdeterminedDims() {
        double[][] result_array;
        if (this.EP == null) {// If selected Number >this.getVectorDim() or if no specified value was entered, take all PCAComponents
    ...
    /**
     * this method is just a small helper method, used to display the Dimensions in the top-part of the output document It accumulates the Variances
     * and calculates this Percentage from the total Variance
     * 
     * @return
     */
    ...
    /**
     * Returns the names of teh 3 files, used for training
     * 
     * @return
     */
    ...
    /**
     * Returns the Editable Report Propeties for the Semantic Report
     * 
     * @return
     */
