...
/**
 * Implementation of a growing Self-Organizing Map layer that can also be static in size. Layer growth is based on the quantization errors of the
 * units and the distance to their respective neighboring units.
 * 
 * @author Michael Dittenbach
 * @author Rudolf Mayer
 * @version $Id: GrowingLayer.java 2874 2009-12-11 16:03:27Z frank $
 */
//
//
...
public class GrowingLayer implements Layer {
    ...
    private final String revision = "$Revision: 2874 $";
    ...
    /**
     * Stores the number of units that are not empty, i.e. that have at least one input mapped. This field should not be used directly, but only via
     * {@link #getNumberOfNotEmptyUnits()}, as the calculation of the value is only performed by that method and stored for future access.
     */
    ...
    /**
     * Convenience constructor for top layer map of GHSOM or a single map. The identifier of the map is set to 1 and the superordinate unit is set to
     * <code>null</code>.
     * 
     ...
     * @param normalized the type of normalization that is applied to the weight vectors of newly created units. This is usually
     *            <code>Normalization.NONE</code> or <code>Normalization.UNIT_LEN</code> (see {@link at.tuwien.ifs.somtoolbox.data.Normalisation}).
     ...
     * @param seed the random seed for creation of the units' weight vectors.
     */
    public GrowingLayer(int xSize, int ySize, int zSize, String metricName, int dim, boolean normalized, boolean usePCA, long seed, InputData data) {
     ...
     * @param seed the random seed for creation of the units' weight vectors.
     */
    public GrowingLayer(int id, Unit su, int xSize, int ySize, String metricName, int dim, boolean normalized, boolean usePCA, long seed,
            InputData data) {
     ...
     * @param seed the random seed for creation of the units' weight vectors.
     */
    public GrowingLayer(int id, Unit su, int xSize, int ySize, int zSize, String metricName, int dim, boolean normalized, boolean usePCA, long seed,
            InputData data) {
        ...
        try {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox").severe("Could not instantiate metric \"" + metricName + "\" for layer.");
            System.exit(-1);
        }
        ...
        if (usePCA) {
            ...
            double cellSizeX = diffX / (double) xSize;
            double cellSizeY = diffY / (double) ySize;
                ...
                for (int j = 0; j < ySize; j++) {
                    for (int i = 0; i < xSize; i++) {
                        ...
                        for (int curPoint = 0; curPoint < data.numVectors(); curPoint++) {
                            ...
                            curCellCoords[0] = (double) i * cellSizeX + cellSizeX / 2;
                            curCellCoords[1] = (double) j * cellSizeY + cellSizeY / 2;
    ...
    /**
     * Constructor for an already trained layer as specified by 2-dimensional array of d-dimensional weight vectors as argument <code>vectors</code>.
     * 
     * @param xSize the number of columns.
     * @param ySize the number of rows.
     * @param metricName the name of the distance metric to use.
     ...
     * @param seed the random seed for creation of the units' weight vectors.
     * @throws SOMToolboxException if arguments <code>x</code>, <code>y</code> and <code>d</code> do not correspond to the dimensions of argument
     *             <code>vectors</code>.
     */
    public GrowingLayer(int id, Unit su, int xSize, int ySize, String metricName, int dim, double[][][] vectors, long seed)
     ...
     *             <code>vectors</code>.
     */
    public GrowingLayer(int id, Unit su, int xSize, int ySize, int zSize, String metricName, int dim, double[][][][] vectors, long seed)
            throws SOMToolboxException {
        ...
        if ((vectors.length != xSize) || (vectors[0].length != ySize) || vectors[0][0].length != zSize
            ...
            throw new SOMToolboxException("Dimensions provided by arguments mismatch dimensions of weight vector array: size " + vectors.length + "x"
                    + vectors[0].length + "x" + vectors[0][0].length + " vs. " + xSize + "x" + ySize + "x" + zSize + ", dimension: "
                    + getDimension(xSize, ySize, zSize, vectors) + " vs " + dim + ".");
        }
        rand = new Random(seed);
        randSkipProbability = new Random();
        this.xSize = xSize;
    ...
    public void calculateQuantizationErrorForUnits() {
        ...
        StdErrProgressWriter progressWriter = new StdErrProgressWriter(xSize * ySize * zSize, "Calculating qe for unit ");
        for (int k = 0; k < zSize; k++) {
            for (int j = 0; j < ySize; j++) {
                for (int i = 0; i < xSize; i++) {
                    if (zSize > 1) {
                        progressWriter.progress("Calculating qe for unit " + i + "/" + j + "/" + k + ", ", (currentUnitNum + 1));
                    } else {
    ...
    public String[] getAllMappedDataNames(boolean sort) {
        ...
        String[] result = (String[]) dataNames.toArray(new String[dataNames.size()]);
        if (sort) {
            Arrays.sort(result);
        }
        return result;
    ...
    public ArrayList<String> getAllMappedDataNamesAsList() {
        ...
        for (int i = 0; i < units.length; i++) {
            if (units[i] != null && units[i].getMappedInputNames() != null) {
                dataNames.addAll(Arrays.asList(units[i].getMappedInputNames()));
            }
        }
        return dataNames;
    }
    ...
    public int[][] getBinAssignment(int component, int bins) {
        if ((component >= 0) && (component < dim)) {
            ...
            double binStep = distance / (double) bins;
                ...
                for (int j = 0; j < ySize; j++) {
                    for (int k = 0; k < zSize; k++) {
                        if (units[i][j][k] != null) { // check needed for mnemonic SOM
                            ...
                            // } else {
                            // // System.out.println("setting -1 value for 0 comp: " + units[i][j][k].printCoordinates() + ", d: " + d);
    ...
    public Point2D[][] getBinCentres(int bins) {
        ...
        if (cachedResult != null) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").info("Getting bin assignment for " + bins + " bins from cache.");
            return cachedResult;
        } else {
            StdErrProgressWriter progress = new StdErrProgressWriter(dim, "Calculating bin assignment for component ", Math.min(dim / 10, 10));
    ...
    private void processRegionRecursive(int[][] binAssignment, int[][] regionAssignment, int x, int y) throws LayerAccessException {
        ...
        Unit[] neighbours = getNeighbouringUnits(x, y, 1);

        for (int i = 0; i < neighbours.length; i++) { // check all neighbours
            Unit neighbour = neighbours[i];
    ...
    private int getNumberOfRegions(int[][] regionAssignement) {
        ...
        for (int i = 0; i < regionAssignement.length; i++) {
            for (int j = 0; j < regionAssignement[i].length; j++) {
                if (regionAssignement[i][j] > numberOfRegions) {
                    numberOfRegions = regionAssignement[i][j];
                }
            }
        }
    ...
    public ArrayList<ComponentRegionCount> getNumberOfRegions(int bins) throws LayerAccessException {
        ...
        if (cachedResult != null) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").info("Getting region assignment for " + bins + " bins from cache.");
            return cachedResult;
        } else {
            StdErrProgressWriter progress = new StdErrProgressWriter(dim, "Calculating number of regions for component ", Math.min(dim / 10, 10));
            ArrayList<ComponentRegionCount> result = new ArrayList<ComponentRegionCount>();
            for (int i = 0; i < dim; i++) {
                int[][] binAssignement = getBinAssignment(i, bins);
                result.add(new ComponentRegionCount(getNumberOfRegions(getRegionAssignement(binAssignement, bins)), new Integer(i)));
                progress.progress();
            }
    ...
    /**
     * Returns a <code>Vector</code> of units that are subject to be expanded or already have a subordinate map assigned. The criterion for expansion
     * is determined by tau2 and mqe0.
     * 
    ...
    /**
     * Calculates and returns the mean quantization error of the map based on the mean of the quantization errors of the single units.
     * 
    ...
    protected Unit getMostDissimilarNeighbor(Unit u) {
        ...
        double distance = 0;
        try {
            Unit[] neighbouringUnits = getNeighbouringUnits(u);
            for (int i = 0; i < neighbouringUnits.length; i++) {
                try {
                    distance = metric.distance(u.getWeightVector(), neighbouringUnits[i].getWeightVector());
                } catch (MetricException e) {
                    Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(e.getMessage());
                }
                if (distance > largestDistance) {
                    neighbor = neighbouringUnits[i];
        ...
        if (neighbor != null) {
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox").severe("A unit on a one-unit SOM has no neighbors. Something went terribly wrong ;-) Aborting.");
            System.exit(-1);
            return null;
        }
    }
    ...
    public Unit getUnit(int x, int y, int z) throws LayerAccessException { // TODO: check for invalid x/y
        try {
            ...
            throw new LayerAccessException("Position " + x + "/" + y + "/" + z + " is invalid. Map size is " + xSize + "x" + ySize + "x" + zSize);
        }
    }
    ...
    public Unit getWinner(InputDatum input, AbstractWeightedMetric metric) {
        ...
        for (int k = 0; k < zSize; k++) {
            for (int j = 0; j < ySize; j++) {
                for (int i = 0; i < xSize; i++) {
                    try {
                        double distance = metric.distance(units[i][j][k].getWeightVector(), inputVector, units[i][j][k].getFeatureWeights());
                        if (distance < smallestDistance) {
                            smallestDistance = distance;
                            winner = units[i][j][k];
                        }
    ...
    public String[] getWinningInputDataFromQuery(String query, int num, SOMLibTemplateVector templateVector) {
        ...
        for (int i = 0; i < winners.length; i++) {
            String[] mappedInputNames = winners[i].getMappedInputNames();
            if (mappedInputNames != null) {
                for (int j = 0; j < mappedInputNames.length; j++) {
                    data[counter] = mappedInputNames[j];
                    counter++;
    ...
    private double[] getInputVector(String query, SOMLibTemplateVector templateVector) {
        ...
        for (int t = 0; t < terms.length; t++) {
            if (termMap.get(terms[t]) != null) {
                Integer v = (Integer) termMap.get(terms[t]);
                termMap.put(terms[t], new Integer(v.intValue() + 1));
            } else {
                termMap.put(terms[t], new Integer(1));
            }
        }
    ...
    /**
     * Returns a number of best-matching units sorted by distance (ascending) for a given input datum. If the number of best-matching units is greater
     * than the total number of units on the map, all units of the map are returned (appropriately ranked).
     * 
    ...
    /**
     * Returns a number of best-matching units and distances sorted by distance (ascending) for a given input datum. If the number of best-matching
     * units is greater than the total number of units on the map, all units of the map are returned (appropriately ranked).
     * 
     * @param input the input datum for which the winner unit will be searched.
     * @param num the number of best-matching units.
     * @return the <code>Vector</code> containing an array of Unit (elementAt(0)) and array of double (elementAt(1)) containing best-matching units
     *         sorted ascending by distance from the input datum.
     */
        ...
        for (int k = 0; k < zSize; k++) {
            for (int j = 0; j < ySize; j++) {
                for (int i = 0; i < xSize; i++) {
                    ...
                    boolean inserted = false;
                    while ((inserted == false) && (element < num)) {
                        if (res[element] == null || distance < res[element].getDistance()) { // found place to insert unit
                            for (int m = (num - 2); m >= element; m--) { // move units with greater distance to right
                                res[m + 1] = res[m];
                            }
    ...
    private void insertRowColumn(Unit a, Unit b, ProgressListener listener) {
        ...
        if (a.getXPos() != b.getXPos()) { // insert column
            ...
            String message = "Inserted column " + insertPos + ". New size is " + (xSize + 1) + "x" + (ySize) + "x" + zSize;
                ...
                for (int j = 0; j < ySize; j++) {
                    for (int k = 0; k < zSize; k++) {
                        if (i < insertPos) {
                            ...
                            try {
                                newUnits[i][j][k] = new Unit(this, i, j, k, AbstractMetric.meanVector(units[i - 1][j][k].getWeightVector(),
                                        units[i][j][k].getWeightVector()));
                            } catch (MetricException e) {
                                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(e.getMessage());
                                System.exit(-1);
                            }
                ...
                for (int j = 0; j < (ySize + 1); j++) {
                    for (int i = 0; i < xSize; i++) {
                        if (j < insertPos) {
                            ...
                            try {
                                newUnits[i][j][k] = new Unit(this, i, j, k, AbstractMetric.meanVector(units[i][j - 1][k].getWeightVector(),
                                        units[i][j][k].getWeightVector()));
                            } catch (MetricException e) {
                                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(e.getMessage());
                                System.exit(-1);
            ...
            String message = "Inserted depth-level " + insertPos + ". New size is " + xSize + "x" + (ySize) + "x" + (zSize + 1);
            Logger.getLogger("at.tuwien.ifs.somtoolbox").info(message);
                ...
                for (int j = 0; j < ySize; j++) {
                    for (int i = 0; i < xSize; i++) {
                        if (k < insertPos) {
                            ...
                            try {
                                newUnits[i][j][k] = new Unit(this, i, j, k, AbstractMetric.meanVector(units[i][j][k - 1].getWeightVector(),
                                        units[i][j][k].getWeightVector()));
                            } catch (MetricException e) {
                                Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(e.getMessage());
                                System.exit(-1);
    ...
    /**
     * Assigns an object that implements the {@link TrainingInterruptionListener} interface to perform actions in certain intervals during training
     * determined by argument <code>i</code>.
     * 
    ...
    /**
     * Trains the layer with the input data. If the value of argument <code>tau</code> is 1, a fix-sized layer is trained, otherwise the layer grows
     * until a certain quality criterion determined by <code>tau</code> and the mean quantization error of the data (which is automatically
     * calculated) is reached. The process ends with all training data being mapped onto the growing SOM. The units' quantization errors are
     * calculated.
     * 
    ...
    public QualityMeasure train(InputData data, double iniLearnrate, double iniSigma, int numIterations, double tau, String qualityMeasureName,
    ...
    public QualityMeasure train(InputData data, double iniLearnrate, double iniSigma, int numIterations, int startIteration, double tau,
        ...
        return train(data, iniLearnrate, iniSigma, numIterations, 0, tau, Double.MAX_VALUE, qualityMeasureName, trainingProps);
    }

    /**
     * Trains the layer with the input data. If the value of argument <code>tau</code> is 1, a fix-sized layer is trained, otherwise the layer grows
     * until a certain quality criterion determined by <code>tau</code> and the mean quantization error specified by argument <code>mqe0</code> is
     * reached. The process ends with all training data being mapped onto the growing SOM. The units' quantization errors are calculated.
     * 
     * @param data input data to train the layer with.
     * @param iniLearnrate initial value of learnrate.
     * @param iniSigma initial sigma determining the neighborhood of the winner.
     * @param numIterations the number of iterations until the mapping quality check.
     * @param tau the fraction of the map's quantization error that determines the quality of the mapping.
     * @param targetQualityValue mean quantization error determining the desired granularity of data representation. Used for layers in GHSOMs.
     */

    public QualityMeasure train(InputData data, double initialLearnrate, double initialSigma, int numIterations, double tau,
            double targetQualityValue, String qualityMeasureName, SOMProperties trainingProps) {
        return train(data, initialLearnrate, initialSigma, numIterations, 0, tau, targetQualityValue, qualityMeasureName, trainingProps);
    }
    ...
    public QualityMeasure train(InputData data, double initialLearnrate, double initialSigma, int numIterations, int startIteration, double tau,
            double targetQualityValue, String qualityMeasureName, SOMProperties trainingProps) {
        ...
        if (qualityMeasureName == null) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning("No quality measure provided for training. Using mean quantization error of map.");
        ...
        if ((targetQualityValue == Double.MAX_VALUE)) {// && (tau<1)) { // to save time when using large data sets
            GrowingLayer layer0 = new GrowingLayer(1, 1, 1, "at.tuwien.ifs.somtoolbox.layers.metrics.L2Metric", dim, false, trainingProps.pca(), 7, data);
            ...
            try {
                qm0 = AbstractQualityMeasure.instantiate(qmNameMethod[0], layer0, data);// new QuantizationError(layer0, data);
            } catch (Exception e) {
        ...
        if (specialClassMode(trainingProps) || trainingProps.datumToUnitMappings().size() > 0) {
            ...
            // FIXME: hidden Parameter
            double favourSelected = 3.0;
            ...
            if (trainingProps.getSelectedClassMode() == SOMProperties.MODE_EXCEPT) {
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                        "Skipping classes: " + trainingProps.getSelectedClasses() + ", total " + selectedInstances + " inputs.");
                ...
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                        "Favouring classes: " + trainingProps.getSelectedClasses() + ", total " + selectedInstances + " inputs, favour factor: "
                                + favourSelected + " --> min probability: " + minProbability);
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
                        "Total inputs: " + data.numVectors() + " --> iterations: ~" + (int) (numIterations / favourSelected));
            } else {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info("No classes selected, normaly training with " + data.numVectors() + " inputs.");
            }
        ...
        ProgressListener progressWriter = ProgressListenerFactory.getInstance().createProgressListener(numIterations, "Iteration ",
                Math.max(1, Math.min(1000, numIterations / 200)), numIterations / 20);
        while (reachedQuality == false) {
            if (specialClassMode(trainingProps) || trainingProps.datumToUnitMappings().size() > 0) {
                trainSpecial(data, numIterations, 0, trainingProps, initialLearnrate, initialSigma, expParam, expParam2, classInfo, minProbability,
                        progressWriter);
            } else {
                trainNormal(data, numIterations, 0, trainingProps, initialLearnrate, initialSigma, expParam, expParam2, progressWriter);
            }
            ...
            if (tau == 1) { // fixed size, just map the data and finish
                ...
                System.out.println(currentQuality + " <= " + targetQualityValue + "*" + tau + "=" + (targetQualityValue * tau));
                if (currentQuality <= (targetQualityValue * tau)) {
                    reachedQuality = true;
                } else { // grow it
                    // determine unit with highest qe/mqe
                    Unit e = getErrorUnit(qm, qm.getUnitQualityNames()[0]); // TODO: attention! alternative criteria possible
    ...
    private void trainSpecial(InputData data, int numIterations, int startIteration, SOMProperties trainingProps, double initialLearnrate,
            double initialSigma, double expParam, double expParam2, SOMLibClassInformation classInfo, double minProbability,
        ...
        for (int i = startIteration; i < numIterations; i++) {
            ...
            try {
                ...
                if (contains && trainingProps.getSelectedClassMode() == SOMProperties.MODE_EXCEPT) {
                    ...
                    Logger.getLogger("at.tuwien.ifs.somtoolbox").info("Found class " + classInfo.getClassName(inpLabel) + " at input #" + inpLabel);
                    i--;
                    continue;
                } else {
                    ...
                    for (int j = 0; j < trainingProps.datumToUnitMappings().size(); j++) {
                        SOMProperties.DatumToUnitMapping mapping = (SOMProperties.DatumToUnitMapping) trainingProps.datumToUnitMappings().get(j);

                        if (currentInput.getLabel().equals(mapping.label)) {
                            try {
                                System.out.println("\n   *** Using a datum with fixed mapping, label: " + mapping.label + "unit x, y: "
                                        + mapping.unitX + ", " + mapping.unitY);
                    ...
                    currentLearnrate = (initialLearnrate * (Math.exp(-1.0 * (double) i / expParam))); // exponential
                    currentSigma = (initialSigma * (Math.exp(-1.0 * (double) i / expParam2))); // exponential
    ...
    private void trainNormal(InputData data, int numIterations, int startIteration, SOMProperties trainingProps, double initialLearnrate,
            double initialSigma, double expParam, double expParam2, ProgressListener progressWriter) {
        double currentLearnrate = initialLearnrate;
        double currentSigma = initialSigma;
            ...
            if (trainingProps.batchSom()) {
                try {
                    ...
                    for (int j = 0; j < data.numVectors(); j++) {
                        ...
                        Unit[] winner_n = getNeighbouringUnits(winner, trainingProps.neighbourWidth());
                        for (int k = 0; k < winner_n.length; k++) {
                            winner_n[k].addBatchSomNeighbour(currentInput);
                        }
                    }
                    for (int k = 0; k < zSize; k++) {
                        for (int j = 0; j < ySize; j++) {
    ...
    private void initCPURanges() {
        ...
        Logger.getLogger("at.tuwien.ifs.somtoolbox.GrowingLayer").info("Splitting map into (at most) " + threadsUsed + " slices.");
        ...
        while (cubs.size() < threadsUsed) {
            ...
            for (int i = 0; i < cubs.size(); i++) {
                Cuboid c = cubs.get(i);
                int curS = (c.getEndX() - c.getStartX()) * (c.getEndY() - c.getStartY()) * (c.getEndZ() - c.getStartZ());
                if (curS > bigS) {
                    bigS = curS;
                    bigI = i;
                }
            ...
            if (xSpan >= ySpan && xSpan >= zSpan) {
                ...
                new1 = new Cuboid(biggest.getStartX(), split, biggest.getStartY(), biggest.getEndY(), biggest.getStartZ(), biggest.getEndZ());
                new2 = new Cuboid(split, biggest.getEndX(), biggest.getStartY(), biggest.getEndY(), biggest.getStartZ(), biggest.getEndZ());
            } else if (ySpan >= xSpan && ySpan >= zSpan) {
                // We split along y
                int split = biggest.getStartY() + (ySpan / 2);
                new1 = (new Cuboid(biggest.getStartX(), biggest.getEndX(), biggest.getStartY(), split, biggest.getStartZ(), biggest.getEndZ()));
                new2 = (new Cuboid(biggest.getStartX(), biggest.getEndX(), split, biggest.getEndY(), biggest.getStartZ(), biggest.getEndZ()));
            } else if (zSpan >= xSpan && zSpan >= ySpan) {
                // We split along z
                int split = biggest.getStartZ() + (zSpan / 2);
                new1 = (new Cuboid(biggest.getStartX(), biggest.getEndX(), biggest.getStartY(), biggest.getEndY(), biggest.getStartZ(), split));
                new2 = (new Cuboid(biggest.getStartX(), biggest.getEndX(), biggest.getStartY(), biggest.getEndY(), split, biggest.getEndZ()));
            } else {
                Logger.getLogger("at.tuwien.ifs.somtoolbox.GrowingLayer").severe("Serious error while splitting into Cuboides for Multi-CPU");
                System.exit(1);
        ...
        // for the splits we need to make sure that the end value of the lower and the start value of the higher range are equal, i.e. regard it
        // as the range length rather than the endpoint
        System.out.println("Working with multiple CPUs!");
        ...
        for (int q = 0; q < qm.getUnitQualityNames().length; q++) {
            ...
            for (int j = 0; j < matr[0].length; j++) {
                for (int i = 0; i < matr.length; i++) {
                    System.out.print(StringUtils.format2FractionDigits.format(matr[i][j]) + "\t");
                }
                System.out.println();
            }
        }
    ...
    protected void updateUnitsNormal(Unit winner, InputDatum input, double learnrate, double sigma) {
        ...
        if (threadsUsed == 1) {
            ...
            for (int i = 0; i < ranges.length; i++) {
                e.execute(new UpdaterThread(winner, learnrate, opt1, inputVector, ranges[i]));
            }
            try {
    ...
    private void updateUnitsInArea(Unit winner, double learnrate, double opt1, double[] inputVector, Cuboid range) {
        updateUnitsInArea(winner, learnrate, opt1, inputVector, range.getStartX(), range.getEndX(), range.getStartY(), range.getEndY(),
                range.getStartZ(), range.getEndZ());
    }

    private void updateUnitsInArea(Unit winner, double learnrate, double opt1, double[] inputVector, int startX, int endX, int startY, int endY,
            int startZ, int endZ) {
        double[] unitVector = null;
        double hci;
    ...
    public String[] getNNearestInputs(String datumlabel, int n, InputData data) throws LayerAccessException, MetricException {
        ...
        Unit[] neighbourUnits = getNeighbouringUnits(u);
        for (int i = 0; i < neighbourUnits.length; i++) {
            for (int j = 0; j < neighbourUnits[i].getNumberOfMappedInputs(); j++) {
                String label = neighbourUnits[i].getMappedInputName(j);
    ...
    public String[] getNNearestInputs(Unit u, int n) throws LayerAccessException {
        ...
        for (int i = 0; i < neighbourUnits.length; i++) {
            for (int j = 0; j < neighbourUnits[i].getNumberOfMappedInputs(); j++) {
                otherItems.add(new InputNameDistance(neighbourUnits[i].getMappedInputDistance(j), neighbourUnits[i].getMappedInputName(j)));
            }
        }
        Collections.sort(otherItems);
    ...
    /**
     * Get direct neighbours of the given unit. Direct neighbours are neighbours in the same column or row of the SOM, thus this method returns at
     * most six neighbours (two for each of the x, y and z dimensions).
     */
    ...
    private Unit[] getNeighbouringUnits(int x, int y, int z) throws LayerAccessException {
        ...
        return (Unit[]) neighbourUnits.toArray(new Unit[neighbourUnits.size()]);
    }

    /**
     * Gets neighbours within a certain radius. <br>
     * FIXME: This method is currently used only for the Batch SOM training, and seems to return also only neighbours in the same column / row,
     * whereas it seems more logical to return all units that have a distance smaller than the radius.
     */
    protected Unit[] getNeighbouringUnits(Unit u, int radius) throws LayerAccessException {
        int xPos = u.getXPos();
        int yPos = u.getYPos();
        int zPos = u.getZPos();
        ArrayList<Unit> neighbourUnits = new ArrayList<Unit>();

        int upperLimitX = xPos + radius;
        if (upperLimitX >= getXSize()) {
            upperLimitX = getXSize() - 1;
        }
        int lowerLimitX = xPos - radius;
        if (lowerLimitX < 0) {
            lowerLimitX = 0;
        }
        int upperLimitY = yPos + radius;
        if (upperLimitY >= getYSize()) {
            upperLimitY = getYSize() - 1;
        }
        int lowerLimitY = yPos - radius;
        if (lowerLimitY < 0) {
            lowerLimitY = 0;
        }
        int upperLimitZ = zPos + radius;
        if (upperLimitZ >= getZSize()) {
            upperLimitZ = getZSize() - 1;
        }
        int lowerLimitZ = zPos - radius;
        if (lowerLimitZ < 0) {
            lowerLimitZ = 0;
        }
        ...
        for (int x = lowerLimitX; x <= upperLimitX; x++) {
            for (int y = lowerLimitY; y <= upperLimitY; y++) {
                for (int z = lowerLimitZ; z <= upperLimitZ; z++) {
                    if (x != xPos || y != yPos || z != zPos) {
                        neighbourUnits.add(getUnit(x, y, z));
                    }
                }
            }
        }
    ...
    public DoubleMatrix2D getUnitDistanceMatrix() {
        ...
        StdErrProgressWriter progress = new StdErrProgressWriter(comparisons, "Calculating unit distance matrix ", stepSize);

        // FIXME: it might be worth to implement a sub-class of DoubleMatrix2D that holds only the needed values of the symmetric matrix.
        DoubleMatrix2D matrix2d = new DenseDoubleMatrix2D(unitCount, unitCount);

        try {
            for (int index1 = 0; index1 < units.length; index1++) {
                for (int index2 = index1 + 1; index2 < units.length; index2++) {
                    distance = metric.distance(units[index1].getWeightVector(), units[index2].getWeightVector());
                    matrix2d.setQuick(index1, index2, distance);
                    matrix2d.setQuick(index2, index1, distance);
                    progress.progress();
                }
            }
    ...
    public InputCorrections computeUnitFeatureWeights(InputCorrections corrections, InputData data, FeatureWeightMode mode)
            throws SOMToolboxException {
        if (data == null) {
        ...
        while (!allMoved && iteration < maxIterations) {
            for (InputCorrection correction : corrections.getInputCorrections()) {
                ...
                if (winner != correction.getTargetUnit()) { // check if the computed mapping is as desired
                    System.out.println("Computed mapping not correct for " + correction.getLabel() + ", winner is " + winner.printCoordinates()
                            + ", should be " + correction.getTargetUnit().printCoordinates());
                    ...
                    for (InputCorrection corr : corrections.getInputCorrections()) {
                        ...
                        if (mode == FeatureWeightMode.GLOBAL) {
                            ...
                            // adapt feature vector only of first unit, and copy them, thus they will be the same for all other units
                            ...
                            corr.getSourceUnit().setFeatureWeights(VectorTools.multiply(corr.getSourceUnit().getFeatureWeights(), w_si));
                            // adapt target unit weights
                            double[] w_ti = VectorTools.subtract(w1, VectorTools.multiply(e_ti, learnRate));
                            corr.getTargetUnit().setFeatureWeights(VectorTools.multiply(corr.getTargetUnit().getFeatureWeights(), w_ti));
                            ...
                            for (Unit unit : allUnits) {
                                ...
                                if (mapDistanceToSource < neighbourhood || mapDistanceToTarget < neighbourhood) {
                                    ...
                                    double[] sub = VectorTools.subtract(g_sn_e_si, g_tn_e_st); // g_sn * e_sn - g_tn * e_tn
                                    double[] multiply = VectorTools.multiply(sub, learnRate); // learn * (g_sn*e_sn - g_tn*e_tn)
                                    double[] w_ni = VectorTools.add(w1, multiply); // w_1 + learn * (g_sn*e_sn - g_tn*e_tn)
                                    unit.setFeatureWeights(VectorTools.multiply(unit.getFeatureWeights(), w_ni)); // w_n * w_ni
                                }
                            }
                        }
                    }
                    ...
                    if (mode == FeatureWeightMode.GLOBAL) {
                        ...
                        allUnits[0].copyFeatureWeights(VectorTools.normaliseByLength(allUnits[0].getFeatureWeights(), dim));
                    } else {
                        for (Unit unit : allUnits) {
        ...
        for (int i = 0; i < data.numVectors(); i++) {
            ...
            if (corrections.get(input.getLabel()) != null) {
                ...
                System.out.println("factor: "
                        + (metricWeighted.distance(input, corr.getTargetUnit()) / metricWeighted.distance(input, corr.getSourceUnit())));
            }
        }
        System.out.println();
    ...
    /**
     * Flip this layer horizontal (Y-Axis).
     */
    public void flipH() {
        // FIXME: Reverse order of units in Z-Axis!
        Unit[][][] out = new Unit[getXSize()][getYSize()][getZSize()];
        for (int x = 0; x < getXSize(); x++) {
            for (int y = 0; y < getYSize(); y++) {
                // System.out.println("Moving " + x + "/" + y + " (" + getXSize() + "x" + getYSize() + ") => " + x + "/" + (getYSize() - y -
                // 1));
    ...
    /**
     * Flip this layer vertical (X-Axis).
     */
    public void flipV() {
        // FIXME: Reverse order of units in Z-Axis!
        Unit[][][] out = new Unit[getXSize()][getYSize()][getZSize()];
    ...
    /** Rotates this layer by the given degrees. */
    public void rotate(int rotation) throws SOMToolboxException {
        ...
        switch (rotation) {
            case 90:
                ...
                for (int x = 0; x < getXSize(); x++) {
                    for (int y = 0; y < getYSize(); y++) {
                        // System.out.println("Moving " + x + "/" + y + " (" + getXSize() + "x" + getYSize() + ") => " + (getXSize() - x - 1) + "/" +
                        // y);
    ...
    public static void checkRotation(int rotation) throws SOMToolboxException {
        if (!ArrayUtils.contains(ROTATIONS, rotation)) {
            throw new SOMToolboxException("Invalid rotation value '" + rotation + "', allowed values are: " + Arrays.toString(ROTATIONS));
        }
    }
    ...
    public String getCommonVectorLabelPrefix() {
        if (commonVectorLabelPrefix == null) {
            ...
            for (int i = 0; i < units.length; i++) {
                CollectionUtils.addAll(allLabels, units[i].getMappedInputNames());
            }
