 ...
 * @version $Id: Cluster.java 2874 2009-12-11 16:03:27Z frank $
 */
...
public class Cluster {
    ...
    public void printClusterIndices(double[][] data) {
        ...
        for (int i = 0; i < indices.size() && i < MAX_INDICES_DEBUG; i++) {
            System.out.println("\tindex " + indices.elementAt(i) + " / " + getDistanceToCentroid(data[indices.elementAt(i)]));
        }
        if (indices.size() > MAX_INDICES_DEBUG) {
            System.out.println("Surpressing output of " + (indices.size() - MAX_INDICES_DEBUG) + " additional elements.");
        }
    }
