...
/**
 * @author Doris Baum
 * @version $Id: SOMComparison.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMComparison {
    ...
    public static GrowingSOM loadGSOM(String setname) {
        ...
        try {
            return new GrowingSOM(new SOMLibFormatInputReader(weightVectorFileName, unitDescriptionFileName, mapDescriptionFileName));
        } catch (Exception e) {
    ...
    public static int[][] calculateClusterAssignment(GrowingSOM gsom, int level) throws LayerAccessException, ClusteringAbortedException {
        ...
        for (int j = 0; j < gsom.getLayer().getYSize(); j++) {
            for (int i = 0; i < gsom.getLayer().getXSize(); i++) {
                if (gsom.getLayer().getUnit(i, j) != null) { // check needed for mnemonic SOMs (might not have all units != null)
                    units[i][j] = new GeneralUnitPNode(gsom.getLayer().getUnit(i, j), CommonSOMViewerStateData.getInstance(), null, null, 0, 0);
                }
            }
        }
    ...
    public int[] clusterEquivalent(int[][] assignment1, int[][] assignment2, LabelCoordinates[] coords1, LabelCoordinates[] coords2,
        ...
        if (debug) {
            ...
            for (int i = equivTable.length - 1; i >= 0; i--) {
                System.out.println(equivTable[i].cluster1 + "; " + equivTable[i].cluster2 + "; " + nf.format(equivTable[i].percentage) + ";");
            }
        }
        ...
        breakpoint: for (int i = equivTable.length - 1; i >= 0; i--) {
            if (!used1[equivTable[i].cluster1] && !used2[equivTable[i].cluster2]) {
                ...
                equivCount++;
                // Logger.getLogger("at.tuwien.ifs.somtoolbox").info(equivTable[i].cluster1 + " -> " + equivTable[i].cluster2 + " : " +
     ...
     */
    public static double[][] calculcateIntraSOMClusterDistanceMatrix(LabelCoordinates[] coords, int[][] secSOMClusterAssignment, int clusterNo,
            double[][] distances) {
    ...
    public ArrayList<Shift> calculateShifts(boolean cumulative) throws SOMToolboxException {
        ...
        if (coords1 == null || coords2 == null || dist1 == null || dist2 == null) {
            throw new SOMToolboxException("You need to call CompareSOMs.calculateMatrices before you can calculate any shifts!");
        }
        ...
        for (int i = 0; i < size; i++) {
            if ((absolute && (neighbourCount[i] >= stableCountThreshold))
                    || (!absolute && (((double) neighbourCount[i]) / oldNeighbourCount[i] >= stablePercentThreshold / 100) && (neighbourCount[i] >= minAbsoluteCount))) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (((absolute && (neighbourCount[i] >= outlierCountThreshold)) || (!absolute
                    && (((double) neighbourCount[i]) / oldNeighbourCount[i] >= outlierPercentThreshold / 100) && (neighbourCount[i] >= minAbsoluteCount)))
    ...
    private ArrayList<Shift> purgeShifts(ArrayList<Shift> allshifts) {
        ...
        String stat = String.format("(%d); Stable:   %d (%5.2f); " + "Adjacent: %d (%5.2f); " + "Outlier:  %d (%5.2f)", allS, stableS,
                100 * ((double) stableS / allS), adjS, 100 * ((double) adjS / allS), outS, 100 * ((double) outS / allS));
    ...
    public ArrayList<Shift> calculateClusterShifts(MapPNode map1, MapPNode map2) throws ClusteringAbortedException {
        ...
        int[][] assignment1 = tree1.getClusterAssignment(clusterNo, gsom1.getLayer().getXSize(), gsom1.getLayer().getYSize());
        ...
        int[][] assignment2 = tree2.getClusterAssignment(clusterNo, gsom2.getLayer().getXSize(), gsom2.getLayer().getYSize());
    ...
    private LabelCoordinates getClusterMeanPoint(int[][] assignment, int y, int x, GrowingSOM gsom) {
        ...
        try {
            ...
            if (assignment[xPos][yPos] != classNumber || (gsom.getLayer().getUnit(xPos, yPos).getNumberOfMappedInputs() == 0 && count < 3)) {
    ...
    public void calculateMatrices() {
        try {
            ...
            double maxDistance1 = Math.sqrt(gsom1.getLayer().getXSize() * gsom1.getLayer().getXSize() + gsom1.getLayer().getYSize()
                    * gsom1.getLayer().getYSize());
            double maxDistance2 = Math.sqrt(gsom2.getLayer().getXSize() * gsom2.getLayer().getXSize() + gsom2.getLayer().getYSize()
