...
/**
 * @author Martin Waitzbauer
 * @version $Id: EntropyVisualizer.java 2874 2009-12-11 16:03:27Z frank $
 */
public class EntropyVisualizer extends AbstractMatrixVisualizer implements QualityMeasureVisualizer {
    ...
    public BufferedImage createVisualization(int index, GrowingSOM gsom, int width, int height) throws SOMToolboxException {
        ...
        try {
            this.regionManager = new RegionManager(inputObjects.getClassInfo(), paintList, width, height, min_visible_class, zoom);
    ...
    /**
     * Spreads the current Region Entropy Error along the whole Palette Interval. (Entropy Error uses to be very small, and thus generates only colors
     * in a small intervall)
     * 
     * @param c1
     * @param max
     * @return
    ...
    private Color getBestFittingColor(double c1, double max) {
        int x;
        if (c1 == max) {
            x = (int) Math.round((c1 / max) * (palette.length - 1));
        } else {
            x = (int) Math.round((c1 / max) * palette.length);
        }
    ...
    /**
     * returns the maximum entropy regions value as double;
     * 
     * @return
     */
