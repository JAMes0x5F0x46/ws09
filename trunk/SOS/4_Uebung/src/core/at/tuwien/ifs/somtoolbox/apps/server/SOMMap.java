...
/**
 * @author Rudolf Mayer
 * @version $Id: SOMMap.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMMap {
    ...
    public SOMMap() throws ServletException {
        ...
        System.out.println("Application path: " + applicationPath + "; image path basics: " + imagePathBase + "; image path temp: " + imagePathTemp);
        som.setDefaultPalette(Palettes.getDefaultPalette());
        som.load(applicationPath);
    }

    public HTMLMapInformation createMapSection(HttpServletRequest request, String vis, Palette palette, int additionalVisParam, boolean showNodes,
            boolean showGrid, String fodokID, int paramMapWidth, int paramMapHeight) throws LayerAccessException {
        ...
        if (sectionMapCache.get(markedMapKey) == null) {
            ...
            try {
                ...
                sectionMapCache.put(markedMapKey, new HTMLMapInformation(TEMP_IMAGES_DIR + "/" + mapFileName, imageMap, nNearest));
        ...
        return ((HTMLMapInformation) sectionMapCache.get(markedMapKey));
    }
    ...
    public HTMLMapInformation createFullMap(HttpServletRequest request, String vis, Palette palette, int additionalVisParam, int paramMapWidth,
            int paramMapHeight, int zoom, int moveX, int moveY, Point selection, boolean showGrid, boolean showNodes, boolean showLabels,
            String fodokIDs, String fullLink, Rectangle areaSelection) throws LayerAccessException {
        initMap(paramMapWidth, paramMapHeight, palette);
        ((ThematicClassMapVisualizer) Visualizations.getVisualizationByName("ClassMap").getVis()).setZoom(cellWidth * zoom);

        String interactionMapKey = getInteractionMapKey(fodokIDs, getMapKey(vis, palette, moveX, moveY, zoom, showLabels, showGrid), selection);

        if (interactionMapCache.get(interactionMapKey) == null) {
            System.out.println("Creating map for ids: " + interactionMapKey);
            BufferedImage mapImage = getBasicMap(vis, palette, additionalVisParam, zoom, showNodes, showGrid, showLabels);
            ...
            int middleX = (int) (visibleX * zoom / 2);
            int middleY = (int) (visibleY * zoom / 2);
            ...
            if (zoom > 1) {
                ...
                System.out.println("offset calc: " + (middleX + (moveX - 1) * windowX) + "x" + (middleY + (moveY - 1) * windowY));
            ...
            if (zoom > 1) {
                ...
                // section.createGraphics().drawImage(scaledInstance, 0, 0, section.getWidth(), section.getHeight(), null);
                // mapImage = section;
                gra = mapImage.createGraphics();
            }
            String imageMap = createImageMap(zoom, offsetX, offsetY, offsetX + visibleX, offsetY + visibleY, null, fullLink);
            ...
            if (fodokIDs != null) {
                ...
                for (int i = 0; i < fodokIds.length; i++) {
                    Unit targetUnit = som.growingSOM.getLayer().getUnitForDatum(fodokIds[i]);
                    if (targetUnit != null) {
                        if (!markedUnits.contains(targetUnit)) {
                            markedUnits.add(targetUnit);
                        }
                    } else {
                        System.out.println("***** Did not find target unit for " + fodokIds[i]);
                    }
                }
                for (int i = 0; i < markedUnits.size(); i++) {
                    Unit u = (Unit) markedUnits.get(i);
                    if (zoom > 1
                        ...
                        // System.out.println("factorWidth: " + (cellWidth * zoom) + ", factorHeight: " + (cellHeight * zoom));
                        drawMarker(gra, COLOR_SEARCHRESULT, zoom, offsetX, offsetY, diameter, cellOffsetX, cellOffsetY, u.getXPos(), u.getYPos());
                        // System.out.println("filling: " + (xPos + 1) + "," + (yPos + 1) + "," + diameter + "," + diameter);
                    }
                }
            }
            ...
            if (selection != null) {
                try {
                    ...
                    if (markedUnits.contains(selectedUnit)) {
                        drawMarker(gra, Color.BLUE, zoom, offsetX, offsetY, diameter, cellOffsetX, cellOffsetY, selectedUnit.getXPos(),
                                selectedUnit.getYPos());
                    } else {
                        drawMarker(gra, Color.DARK_GRAY, zoom, offsetX, offsetY, diameter, cellOffsetX, cellOffsetY, selectedUnit.getXPos(),
                                selectedUnit.getYPos());
                    }
            ...
            try {
                ...
                interactionMapCache.put(interactionMapKey, new HTMLMapInformation(TEMP_IMAGES_DIR + "/" + mapImageFile.getName(), imageMap));
        ...
        return (HTMLMapInformation) interactionMapCache.get(interactionMapKey);
    }
    ...
    public HTMLMapInformation createFullMap(HttpServletRequest request, String vis, Palette palette, int additionalVisParam, boolean showNodes,
            boolean showGrid, boolean showLables, String fodokIDs, int paramMapWidth, int paramMapHeight, String fullLink)
            throws LayerAccessException {
        return createFullMap(request, vis, palette, additionalVisParam, paramMapWidth, paramMapHeight, 1, 0, 0, null, showGrid, showNodes,
                showLables, fodokIDs, fullLink, null);
    }
    ...
    private BufferedImage getBasicMap(String vis, Palette palette, int additionalVisParam, int zoom, boolean showNodes, boolean showGrid,
            boolean showLabels) {
        ...
        if (basicMapCache.get(basicMapKey) == null) {
            ...
            try {
                if (visualizer instanceof AbstractMatrixVisualizer) {
                    ((AbstractMatrixVisualizer) visualizer).setPalette(som.defaultPaletteIndex, som.getDefaultPalette());
                ...
                BufferedImage visImage = visualizer.getVisualization(var, som.growingSOM, mapWidth * zoom, mapHeight * zoom);
                ...
                if (showLabels) {
                    ...
                    try {
                        ...
                        gra.drawImage(labels, 0, 0, baseMapImage.getWidth(), baseMapImage.getHeight(), 0, 0, labels.getWidth(), labels.getHeight(),
                                null);
                    } catch (IOException e) {
                        System.out.println("Could not read labels image: " + e.getMessage() + ", file: " + file.getAbsolutePath());
                    }
                }
    ...
    private void drawMarker(Graphics2D gra, Color fillColor, int zoom, int offsetX, int offsetY, int diameter, int cellOffsetX, int cellOffsetY,
            int unitX, int unitY) {
    ...
    private String getBasicMapKey(String vis, Palette palette, int zoom, boolean showGrid, boolean showLabels) {
        ...
        return (mapWidth * zoom) + "x" + (mapHeight * zoom) + "_" + vis + "_" + palette.getShortName() + "_zoom-" + zoom + "_grid-" + showGrid
                + "_labels-" + showLabels;
    }
    ...
    private String getMapKey(String vis, Palette palette, int moveX, int moveY, int zoom, boolean showLabels, boolean showGrid) {
        vis = Visualizations.getVisualizationShortName(vis);
        return mapWidth + "x" + mapHeight + "_" + vis + "_" + palette.getShortName() + "_zoom-" + zoom + "_movex-" + moveX + "_movey-" + moveY
    ...
    public String createImageMap(int zoom, int offsetX, int offsetY, int extraOffsetX, int extraOffsetY, int endX, int endY, String fodokId,
            String fullLink) throws LayerAccessException {
        System.out.println("creating image map. zoom: " + zoom + ", offsetX: " + offsetX + ", offsetY: " + offsetY + ", endX: " + endX + ", endY: "
                + endY);
        ...
        for (int x = offsetX; x < endX; x++) {
            for (int y = offsetY; y < endY; y++) {
                ...
                thisUnit.append("coords=\"" + coordX + "," + coordY + "," + (coordX + cellWidth * zoom) + "," + (coordY + cellHeight * zoom) + "\" ");
    ...
    public String getClassLegend() {
        ...
        if (classInfo != null) {
            ...
            for (int i = 0; i < colors.length; i++) {
                b.append("<tr>\n");
                // b.append("<td stlye=\"background-color: rgb(" + colors[i].getRed() + "," + colors[i].getGreen() + "," + colors[i].getBlue()
                // + ")\" >&nbsp;&nbsp;&nbsp;");
                b.append("<td>");
                b.append("<span style=\"background-color: rgb(" + colors[i].getRed() + "," + colors[i].getGreen() + "," + colors[i].getBlue()
                        + ")\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;");
                b.append("</td>");
                b.append("<td>" + at.tuwien.ifs.somtoolbox.util.StringUtils.beautifyForHTML(classInfo.classNames()[i]) + "</td>");
                b.append("</tr>\n");
            }
            b.append("</table>\n");
        }
