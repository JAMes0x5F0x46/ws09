...
public class SemanticInterpreterGrid {
    ...
    /**
     * Computes the areas for the left(type==1) or right (type==2) side of the Middle part for the given Axis and divides the parts equally among the
     * granularity measures defines by the [] Gravity
     */
     ...
     * @param QMReportIntersection [0] = Name of Quality Measure, [1] = 1 = MAX, 2 = MIN, [3..length] intersectio nunits of the QM to be analyzed
     */
    public String createClassReportOnGrid(int indexReport, int clusterlevelReport, ArrayList QMReportIntersection) {
                    ...
                    if (!QMReportIntersection.get(0).equals(this.qm.UnitQualityMeasureNames.get(i))) { // that are not the current qualifier
                        ...
                        for (int j = 0; j < classUnits.size(); j++) { // for all units
                            ...
                            if ((Integer) QMReportIntersection.get(1) == 3) { // MAX-MIN REPORT
                                if (qmUnits[currUnit[0]][currUnit[1]] == 5 || qmUnits[currUnit[0]][currUnit[1]] == 4) { // we have a minimum of
                                    // another
                                    // qm there aswell
                                    tempIntersectMAX.add(this.getNode(currUnit));
                                }
        ...
        if (this.type == 1) { // HTML REPORT
            if (classeshmap.size() > 0) {
                ...
                out += (ClassesOutsideofRegion.size() + ClassesCompleteinRegion.size())
                        + " ("
                        + Math.round((((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) / this.TestRun.getNumberofClasses()) * 100)
        ...
        if (ClassesCompleteinRegion.size() > 0 || ClassesOutsideofRegion.size() > 0) {
            out += overallCompactness
                    + " ("
                    + Math.round((overallCompactness / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size())) * 100)
            ...
            out += FullyConnected
                    + " ("
                    + Math.round(FullyConnected / ((double) ClassesOutsideofRegion.size() + (double) ClassesCompleteinRegion.size()) * 100)
     ...
     * FIXME: refactor this with some generic java sorting algorithm, implementing a special {@link Comparator} first if needed
     */
        ...
        if (regions[0].size() > 0 && regions[1].size() > 0 && regions[2].size() > 0) {
            ...
            if (globMax.size() > 0) {
                ...
                for (int z = 0; z < globMax.size(); z++) {
                    if (z < globMax.size() - 1) {
                        s += getRegion((Integer) globMax.get(z) + 1) + "(" + regions[globMax.get(z)].size() + ") and ";
                    } else {
                        s += getRegion((Integer) globMax.get(z) + 1) + "(" + regions[globMax.get(z)].size() + ")";
                    }
                }
                s += ".";
            }
                ...
                if (s.charAt(s.length() - 1) == '.') {
                    ...
                    for (int v = 0; v < max.size(); v++) {
                        if (!globMax.contains(max.get(v))) {
                            if (v < max.size() - 1) {
                                s += getRegion((Integer) max.get(v) + 1) + "(" + regions[max.get(v)].size() + ") and ";
                            } else {
                                s += getRegion((Integer) max.get(v) + 1) + "(" + regions[max.get(v)].size() + ")";
                            }
                        }
                    }
                    s += ".";
                    ...
                    for (int k = 0; k < max.size(); k++) {
                        if (k < max.size() - 1) {
                            s += getRegion((Integer) max.get(k) + 1) + "(" + regions[max.get(k)].size() + ") and ";
                        } else {
                            s += getRegion((Integer) max.get(k) + 1) + "(" + regions[max.get(k)].size() + ")";
                        }
                    }
                    s += ".";
                }
     ...
     * FIXME: refactor this, make a method in {@link GrowingLayer}, based on {@link GrowingLayer#getNumberOfNotEmptyUnits()}
     */
