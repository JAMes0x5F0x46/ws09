...
/**
 * @author Rudolf Mayer
 * @version $Id: MapServlet.java 2874 2009-12-11 16:03:27Z frank $
 */
public class MapServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("\n\n Map Servlet: " + at.tuwien.ifs.somtoolbox.util.StringUtils.printMap(request.getParameterMap()));
        ...
        String basicZoomLink = params.basicLink + "&moveX=" + params.move.width + "moveY=" + params.move.height + "&zoom=";

        try {
            HTMLMapInformation mapData = map.createFullMap(request, params.vis, params.palette, params.smoothingFactor, params.mapWidth,
                    params.mapHeight, params.zoom, params.moveX, params.moveY, params.pointSelection, params.showGrid, params.showNodes,
                    params.showLabels, params.fodokIds, params.fullLink, params.areaSelection);
            ...
            for (int i = 1; i <= 3; i++) {
                if (params.zoom == i) {
                    out.println("<div valign=\"top\" class=\"mapScaleOn\" style=\"margin: 1px 0pt 3px;\" title=\"Current zoom level: " + i + "\">"
                            + i + "</div>");
                } else {
                    out.println("<a valign=\"top\" class=\"mapScaleOff\" href=\"" + basicZoomLink + i + "\" title=\"Switch to zoom level " + i
                            + "\" style=\"margin: 1px 0pt 3px;\">" + i + "</a>");
                }
            }
            ...
            if (params.allowMoveUp) {
                out.println("<a href=\"" + basicMoveYLink + "&moveY=" + (params.move.height - 1) + "\" alt=\"^^\" title=\"Navigate up\">");
            ...
            if (params.allowMoveLeft) {
                out.println("<a href=\"" + basicMoveXLink + "&moveX=" + (params.move.width - 1) + "\" alt=\"&lt;&lt;\" title=\"Navigate up\">");
            ...
            // map
            out.println("<td align=\"center\"><img border=\"0\" src=\"" + mapData.getImagePath() + "\" usemap=\"#som-map\" alt=\"\"/></td>");

            // navigate right
            out.println("<td>");
            if (params.allowMoveRight) {
                out.println("<a href=\"" + basicMoveXLink + "&moveX=" + (params.move.width + 1) + "\" alt=\"&gt;&gt;\" title=\"Navigate right\">");
            ...
            if (params.allowMoveDown) {
                out.println("<a href=\"" + basicMoveYLink + "&moveY=" + (params.move.height + 1) + "\" alt=\"vv\" title=\"Navigate down\">");
            ...
            // print the image map
            out.println((String) mapData.getImageMap());
            ...
            if (params.pointSelection != null) {
                if (SOMMap.som.growingLayer.isValidUnitLocation(params.pointSelection)) {
                    ...
                    out.println("<b>Selected unit " + params.pointSelection.x + "/" + params.pointSelection.y + "</b><br>");
                    out.println("<b>Representative terms:</b> " + SOMMap.som.growingLayer.getUnit(params.pointSelection.x, params.pointSelection.y).getUnitLabels() + "<br>");
                    out.println("<b>Assigned documents:</b><br>");
                    if (names != null) {
                        for (int i = 0; i < names.length; i++) {
                            out.println("<a target=\"_top\" href=\"" + map.documentDetailLink + "?" + map.documentDetailParamName + "=" + names[i]
                                    + "\">" + names[i] + "</a>");
                        }
                    } else {
                        out.println("None<br>");
                    ...
                    out.println("<b>Invalid unit selection " + params.pointSelection.x + "/" + params.pointSelection.x + "!<b/><br>");
                }
            }
            ...
            if (request.getParameter("expertMode") != null) {
                ...
                BackgroundImageVisualizer backgroundImageVisualizer = ServerVisualizations.getVisualizationByName(params.vis).getVis();
            ...
            if (request.getParameter("dataMode") != null) {
                ...
                for (int i = 0; i < names.length; i++) {
                    ...
                    out.println("<td style=\"font-size:xx-small; padding-left: 5px; padding-right: 5px;\"><a href=\"map?fodokIds=" + names[i]
                            + params.linkParams + "\" title=\"" + names[i] + "\">" + displayName + "</a></td>");
    ...
    private void printHiddenFormInput(HttpServletRequest request, ServletOutputStream out, String paramName) throws IOException {
