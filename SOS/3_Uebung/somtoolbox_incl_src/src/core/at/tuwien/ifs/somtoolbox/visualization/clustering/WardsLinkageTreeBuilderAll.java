...
/**
 * Class to create Clustering trees with the Ward's Linkage algorithm. This class is not compatible with mnemonic SOMs (and probably also not
 * compatible with hierarchical SOMs)
 * 
 * @author Angela Roiger
 * @version $Id: WardsLinkageTreeBuilderAll.java 2874 2009-12-11 16:03:27Z frank $
 */
public class WardsLinkageTreeBuilderAll extends AbstractWardsLinkageTreeBuilder {
    ...
    /**
     * Only use false if you suspect WardsLinkageTreeBuilderAll(true) does not return a correct clustering. The new Update function only recalculates
     * the distance values before merging two clusters. This way a lot less calculations have to be made, reducing the complexity from n^3 to n^2.
     * 
    ...
    public ClusteringTree createTree(GeneralUnitPNode[][] units) throws ClusteringAbortedException {
        ...
        HashMap<ClusterNode, ClusterNode> parents/* <ClusterNode, ClusterNode> */= null; // only used for lazyUpdate
        ...
        while (dists.size() > 0) {
            toMerge = (NodeDistance) dists.first();
            ...
            if (lazyUpdate) {
                ...
                boolean dirty = false;
                for (;;) {
                    ClusterNode p = (ClusterNode) parents.get(toMerge.n1);
                    if (p == null)
                        break;
                    toMerge.n1 = p;
                    dirty = true;
                ...
                for (;;) {
                    ClusterNode p = (ClusterNode) parents.get(toMerge.n2);
                    if (p == null)
                        break;
                    toMerge.n2 = p;
                    dirty = true;
            ...
            incrementMonitor();
            allowAborting();
            newNode = new ClusterNode(toMerge.n1, toMerge.n2, level);
    ...
    /**
     * Calculates the initial distances from each To each other unit where there is not yet a distance calculated. This results in n^2/2 distance
     * calculations and the same amount of created objects. Can take long in case of large maps. This code is only compatible with rectangular, non
     * hierarchical SOMs!
     * 
