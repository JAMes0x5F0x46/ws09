 ...
 * @author Rudolf Mayer
 * @version $Id: SOMProperties.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMProperties extends Properties {
    ...
    /* Jakob: dumpEvery */
    public SOMProperties(int xSize, int ySize, int zSize, long seed, int trainingCycles, int trainingIterations, int dumpEvery, double lernrate,
            double sigma, double tau, String metric, boolean usePCA) throws PropertiesException {
        this(xSize, ySize, zSize, seed, trainingCycles, trainingIterations, lernrate, sigma, tau, metric, usePCA);
        this.dumpEvery = dumpEvery;
    }

    /* Jakob: 3D (zSize) */
    public SOMProperties(int xSize, int ySize, int zSize, long seed, int trainingCycles, int trainingIterations, double lernrate, double sigma,
            double tau, String metric, boolean usePCA) throws PropertiesException {
        this(xSize, ySize, seed, trainingCycles, trainingIterations, lernrate, sigma, tau, metric, usePCA);
        this.zSize = zSize;
    }

    public SOMProperties(int xSize, int ySize, long seed, int numCycles, int numIterations, double lernrate, double sigma, double tau,
            String metricName, boolean usePCA) throws PropertiesException {
        this.xSize = xSize;
        this.ySize = ySize;
        this.zSize = 1;
        ...
        try {
            ...
            if (getProperty("datumToUnitMappings") != null) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    System.out.println("\n   *** Adding datum to unit mapping, datum label: " + mapping.label + "unit x, y: " + mapping.unitX + ", "
                            + mapping.unitY);
            ...
            if (selectedClassesString != null) {
                ...
                for (int i = 0; i < selectedClassesTmp.length; i++) {
                    selectedClasses.add(selectedClassesTmp[i].trim());
                }
            }
            classInfoFileName = getProperty("classInfoFileName", null);
            if ((classInfoFileName != null) && (getProperty("workingDirectory", null) != null)) {
                String tmpStr = getProperty("workingDirectory", null).concat(File.separator).concat(classInfoFileName);
    ...
    private void validatePropertyValues() throws PropertiesException {
        ...
        if (sigma <= 0) {
            ...
            // new equation, inspired by e.g. http://xmipp.cnb.csic.es/NewXmipp/Web_Site/public_html/NewXmipp/Applications/Src/SOM/Help/som.html and
            ...
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning(
                    "Sigma is missing or negative. Defaulting to " + at.tuwien.ifs.somtoolbox.util.StringUtils.format2FractionDigits.format(sigma)
                            + " for a map of size " + xSize + "x" + ySize);
        }
        if (tau == 1) {
        ...
        if (StringUtils.isBlank(growthQualityMeasureName)) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning("No growthQualityMeasureName given. Defaulting to QuantizationError.mqe.");
        ...
        if ((numIterations <= 0) && (numCycles <= 0)) {
            ...
            if (tau == 1) {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info("numIterations defines the fixed number of training iterations.");
            } else {
            ...
            if (tau == 1) {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info("numCycles defines the fixed number of training cycles (multiples of the number of data).");
            } else {
                Logger.getLogger("at.tuwien.ifs.somtoolbox").info(
    ...
    /** Return the number of iterations really trained, either using {@link #numIterations} or {@link #numCycles}, whichever value is set. */
    ...
    /**
     * Returns sigma deterimining the neighborhood radius.
     * 
     * @return sigma deterimining the neighborhood radius.
     */
    public double sigma() {
        return sigma;
    }
    ...
    public void writeToFile(String dataName, String outputDir, boolean normalised) throws IOException {
        ...
        PrintWriter writer = at.tuwien.ifs.somtoolbox.util.FileUtils.openFileForWriting("SOM Properties File", fileName, false);
