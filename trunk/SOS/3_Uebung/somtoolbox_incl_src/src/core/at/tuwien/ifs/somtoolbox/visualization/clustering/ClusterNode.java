 ...
 * @version $Id: ClusterNode.java 2874 2009-12-11 16:03:27Z frank $
 */
public class ClusterNode implements Serializable {
        ...
        if (mean == null) {
            ...
            for (int j = 0; j < weightVectorLength; j++) {
                ...
                for (int i = 0; i < unitNodes.length; i++) {
                    ...
                    sum = sum + (unitNodes[i].getUnit().getWeightVector())[j];
                }
    ...
    public ClusterNode(ClusterNode n1, ClusterNode n2, int level) {
        ...
        this.setHeight((Math.max(n1.getY() + n1.getHeight(), n2.getY() + n2.getHeight()) - Math.min(n1.getY(), n2.getY())));
     ...
     * 
     * @param factorValue A double value between 0 and 1 to determine how much the mean value should influence the label.
     * @param factorQe A double value between 0 and 1 to determine how much the qe value should influence the label.
     * @param factorNumber Currently unused. Should determine the influence of the number of input vectors of the clusters.
    ...
    private void calcLabel(double factorValue, double factorQe, double factorNumber, boolean withValue) {
        ...
        for (int i = 0; i < unitNodes.length; i++) {
            int num = unitNodes[i].getUnit().getNumberOfMappedInputs();
            if (num > 0) {
                count++;
            }
            tmpLabels = unitNodes[i].getLabels(Unit.LABELSOM);// change if other types of Labels should also be used
            if (tmpLabels != null) {
                Label tmpLabel;
                for (int j = 0; j < tmpLabels.length; j++) {
                    tmpLabel = new Label(tmpLabels[j].getName(), tmpLabels[j].getValue(), tmpLabels[j].getQe());
                    ...
                    if (allLabels.containsKey(tmpLabel.getName())) {
                        Label duplicate = (Label) allLabels.remove(tmpLabel.getName());
        ...
        for (Label l : allLabels.values()) {
            if (!withValue) {
                tmp.add(new ClusterLabel(l.getName(), l.getValue() / count, l.getQe() / count,
                        0 - ((l.getValue() / (count * maxVal)) * factorValue + (1 - (l.getQe() / maxQe)) * factorQe)));
            } else {
                tmp.add(new ClusterLabel(l.getName() + "\n" + StringUtils.format3FractionDigits.format(l.getValue() / count), l.getValue() / count, l.getQe() / count,
                        0 - ((l.getValue() * factorValue / (count * maxVal)) + ((1 - (l.getQe() / maxQe)) * factorQe))));
            }
        }

        this.labels = (ClusterLabel[]) tmp.toArray(new ClusterLabel[tmp.size()]);
    }
    ...
    public boolean containsNodes(GeneralUnitPNode node) {
    ...
    private BorderPNode makeBorder() {
        ...
        for (Rectangle2D rect : lines) {
            PPath line = PPath.createLine((float) rect.getMinX(), (float) rect.getMinY(), (float) rect.getMaxX(), (float) rect.getMaxY());
    ...
    private void xorBorderLine(ArrayList<Rectangle2D> lines, double x1, double y1, double x2, double y2) {
