 ...
 * @version $Id: SOMDescriptionLATEX.java 2874 2009-12-11 16:03:27Z frank $
 */
public class SOMDescriptionLATEX {
    ...
    /**
     * prints a list of properties describing the training process and the generated SOM this list contains besides others: \\begin{itemize} \\item
     * type and topology of SOM \\item dimensions of the som \\item different training parameters \\item neighbourhood function \\item ...
     * \\end{itemize}
     */
    ...
    public void printDataDistribution() {
        ...
        if (this.dataset.classInfoAvailable()) {
            ...
            // Create Images...
            this.testrun.createClassDistributionImage(this.imgDir + System.getProperty("file.separator"), "classDistribution.jpg", -1);
            ...
            for (int i = 0; i < this.dataset.getNumberOfClasses(); i++) {
                this.testrun.createEntropyImage(this.imgDir + System.getProperty("file.separator"), "classEntropy" + i + ".jpg", i);
                this.testrun.createClassDistributionImage(this.imgDir + System.getProperty("file.separator"), "classDistribution" + i + ".jpg", i);
            }
            ...
            this.writer.appendOutput("\n\\begin{longtable}{lcccccp{4cm}}\n"
                    ...
                    // + "\"mean unit\"&\n"
                    // + "radius&\n" +
                    + "Coordinates &\n" + "\\includegraphics[width=50pt]{" + SOMDescriptionLATEX.imgSubdir + "/run_" + this.testrun.getRunId()
                    + "_classDistribution.jpg}&\n" + "\\includegraphics[width=50pt]{" + SOMDescriptionLATEX.imgSubdir + "/run_"
                    + this.testrun.getRunId() + "_classEntropy.jpg}&\n" + "Class Entropy&\n" + "Other classes\n" + " \\\\\n");
            for (int i = 0; i < this.dataset.getNumberOfClasses(); i++) {
                ...
                this.writer.appendOutput("\\includegraphicstotab[width=50pt]{" + SOMDescriptionLATEX.imgSubdir + "/run_" + this.testrun.getRunId()
    ...
    protected void printMapLayout(boolean classInfoAvailable) {
        ...
        this.writer.appendOutput("A more sophisticated way to visualize the distributioun of classes is the SDH. "
                + this.testrun.getScientificDescription("sdh")
        ...
        this.testrun.createSDHImages(this.imgDir + System.getProperty("file.separator"), "SDH_Visualization.jpg", 30, true, -1);
        this.testrun.createSDHImages(this.imgDir + System.getProperty("file.separator"), "SDH_Visualization.jpg", 30, true, -1);
        this.testrun.createSDHImages(this.imgDir + System.getProperty("file.separator"), "SDH_Visualization.jpg", 30, true, -1);
        this.writer.appendOutput("\\begin{center}\n" + "\\begin{figure}[htbp]\n");
        int dim = this.testrun.getSOMDimensions();
        for (int i = 1; i < dim; i = i + dim / 3) {
            this.writer.appendOutput("\\begin{minipage}[hbt]{4.5cm}\n" + "\\includegraphics[width=4.5cm]{" + SOMDescriptionLATEX.imgSubdir + "/"
                    + "SDH_pics_" + this.testrun.getRunId() + "/" + "s" + i + "_" + this.testrun.getRunId() + "_SDH_Visualization.jpg}");

            this.writer.appendOutput("\\caption{SDH with k = " + i + "}" + "\\label{fig:sdh" + i + "}" + "\\end{minipage}\n" + "\\hspace{2cm}\n");
        ...
        if (this.dataset.getNumberOfSelectedInputs() > 0) {
            ...
            for (int i = 0; i < this.dataset.getNumberOfSelectedInputs(); i++) {
                ...
                this.writer.appendLatexOutput(input.getLabel()
                        ...
                        + String.format("%.5f", this.testrun.getQEForUnit(unit.getXPos(), unit.getYPos())) + "" + " and mqe="
            ...
            // +
            // "the label of a selected input item, the number in the top left corner of each cell gives the number of input items totally mapped to
            // this unit. ");
    ...
    /**
     * creates and outputs one out of two possible tables the first possible table: \\\\ the first table contains for each unit only the number of
     * input vectors mapped to it, and a pie chart image of the class distribution within this unit. (lookup == null) If no class information is
     * available, this table is not created\\\\ \\\\ The second table additionally contains the ids of input vectors selected by the user within the
     * unit they are mapped to. Therefore lookup must be a hashtable that contains for each unit (key = "x_y") a list of InputDatum objects specifying
     * the input elements mapped to this unit. If for a key there's no list, then nothing is mapped to this unit. if no class information is
     * available, the table does not contain the number of input vectors mapped to each unit
     * 
    ...
    protected void printDistributionDetailTable(Hashtable<String, Vector<InputDatum>> lookup, boolean classInformationAvailable) {
        ...
        for (int y = 0; y < Integer.parseInt(((String) this.testrun.getMapProperty(TestRunResultCollection.keyYDim))); y++) {
            ...
            for (int x = 0; x < Integer.parseInt(((String) this.testrun.getMapProperty(TestRunResultCollection.keyXDim))); x++) {
                ...
                if (classInformationAvailable) {
                    String[] matches = pieDir.list(new PrefixFilenameFilter(x + "_" + y + "_")); // lookup whether there's an image
                    if (matches != null && matches.length > 0) {
                        // image exists => insert it
                        String number = matches[0].substring((x + "_" + y + "_").length());
                        number = number.substring(0, number.length() - 4);

                        img = "\n\\includegraphicstotab[width=20pt]{" + SOMDescriptionLATEX.imgSubdir + "/pieCharts_" + this.testrun.getRunId() + "/"
                                + matches[0] + "}\n";
                        numbDiv = "" + number;
    ...
    /**
     * prints a list with different quantization errors. This includes: \\begin{itemize} \\item Map mean quantization error \\item Map mean mean
     * quantization error \\item Min/Max unit quantization error \\item Min/Max unit mean quantization error \\item Min/Max quantization error of an
     * input vector (as it is mapped to SOM) \\item Min/Max quantization error of an input vector (taken the best matching unit, as I think it could
     * happen that during the training an input vector is mapped to a unit that might be the best matching unit, but during the training process is
     * then pulled away from this input, s.t. in the end another unit would be better)
     */
    protected void printQuantizationErrorReport() {
        ...
        this.writer.appendOutput(this.testrun.getScientificDescription("QError") + "\\\\");
        // min/max quantization errors
        this.writer.appendLatexOutput("\\begin{itemize}\n");

        // map quantization error (mqe & mmqe)
        this.writer.appendLatexOutput("\\item Map quantization errors:\n\\begin{itemize}");
        this.writer.appendLatexOutput("\\item Map mean quantization error:" + String.format("%.5f", this.testrun.getMapMQE().getQE()) + "\n");
        this.writer.appendLatexOutput("\\item Map mean mean quantization error:" + String.format("%.5f", this.testrun.getMapMMQE().getQE()) + "\n");
        this.writer.appendLatexOutput("\\end{itemize}");
        ...
        this.writer.appendLatexOutput("\\item Minimal unit quantization error: " + String.format("%.5f", min.getQE()) + unitCoord + "\n");
        max = this.testrun.getMaxUnitQE();
        unitCoord = getUnitCoords(max);
        this.writer.appendLatexOutput("\\item Maximal unit quantization error: " + String.format("%.5f", max.getQE()) + unitCoord + "\n");

        // min mqe/max mqe per unit
        min = this.testrun.getMinUnitMQE();
        unitCoord = getUnitCoords(min);
        this.writer.appendLatexOutput("\\item Minimal unit mean quantization error: " + String.format("%.5f", min.getQE()) + unitCoord + "\n");
        max = this.testrun.getMaxUnitMQE();
        unitCoord = getUnitCoords(max);
        this.writer.appendLatexOutput("\\item Maximal unit mean quantization error: " + String.format("%.5f", max.getQE()) + unitCoord + "\n");
        this.writer.appendLatexOutput("\\end{itemize}");
        ...
        this.writer.appendLatexOutput("\\item Minimal (mapped) quantization error of input vector: " + String.format("%.5f", mini.getQE())
                + getInputCoords(mini) + "\n");
        maxi = this.testrun.getMaxInputMappedQE();
        this.writer.appendLatexOutput("\\item Maximal (mapped) quantization error of input vector: " + String.format("%.5f", maxi.getQE())
                + getInputCoords(maxi) + "\n");

        // min qe/max qe per input vector (best matching)
        mini = this.testrun.getMinInputWinnerQE();
        this.writer.appendLatexOutput("\\item Minimal (best matching) quantization error of input vector: " + String.format("%.5f", mini.getQE())
                + getInputCoords(mini) + "\n");
        maxi = this.testrun.getMaxInputWinnerQE();
        this.writer.appendLatexOutput("\\item Maximal (best matching) quantization error of input vector: " + String.format("%.5f", maxi.getQE())
                + getInputCoords(maxi) + "\n");
        this.writer.appendLatexOutput("\\end{itemize}");
        ...
        if (this.testrun.createQuantizationErrorImage(this.imgDir, "quantErr.jpg", 0)) {
            ...
            this.writer.appendOutput("\\begin{figure}[htbp]\n" + "\\centering" + "\\includegraphics[width=4.5cm]{" + SOMDescriptionLATEX.imgSubdir
                    + "/run_" + this.testrun.getRunId() + "_quantErr.jpg}\n" + "\\caption{Visualization of the Q-Error}" + "\\label{fig:QError}"
                    + "\\end{figure}\n");
        }
    }
    ...
    protected void printTopographicErrorReport() {
        ...
        this.writer.appendOutput(this.testrun.getScientificDescription("TError") + "\\\\");
        ...
        this.writer.appendLatexOutput("\\item Topographic error of the map: " + String.format("%.5f", this.testrun.getMapTE()) + "\n");
        this.writer.appendLatexOutput("\\end{itemize}");
        ...
        unitCoord = getUnitCoords(min);
        this.writer.appendLatexOutput("\\item Number of units without topographic error: " + this.testrun.getNumberOfTElessUnits() + "");
        this.writer.appendLatexOutput("\\item Minimal topographic error > 0 of a unit : " + String.format("%.2f", min.getQE()) + unitCoord + ""
                + "\n");
        max = this.testrun.getMaxUnitTE();
        unitCoord = getUnitCoords(max);
        this.writer.appendLatexOutput("\\item Maximal topographic error of a unit: " + String.format("%.2f", max.getQE()) + unitCoord + "\n");
        this.writer.appendLatexOutput("\\end{itemize}\n");
        ...
        if (this.testrun.createTopographicErrorImage(this.imgDir, "topErr.jpg")) {
            ...
            this.writer.appendOutput("\\begin{figure}[htbp]\n" + "\\centering" + "\\includegraphics[width=4.5cm]{" + SOMDescriptionLATEX.imgSubdir
                    + "/run_" + this.testrun.getRunId() + "_topErr.jpg}\n" + "\\caption{Visualization of the T-Error}" + "\\label{fig:TError}"
                    + "\\end{figure}\n");
    ...
    protected String getUnitCoords(UnitQEContainer value) {
        ...
        for (int i = 0; i < value.getNumUnits(); i++) {
            ...
            unitCoord += "\\lbrack" + value.getUnitCoords(i) + "\\rbrack  - " + value.getNumberOfVectorsMapped(i) + " vectors mapped";
        }
    ...
    private String getInputCoords(InputQEContainer value) {
        ...
        for (int i = 0; i < value.getNumInputs(); i++) {
            ...
            inputCoord += "\"" + value.getInputLabel(i) + "\" on unit at \\lbrack" + value.getUnitCoords(i) + "\\rbrack";
        }
    ...
    protected void printClusterInfos() {
        ...
        if (this.testrun.createUDMatrixImage(this.imgDir, "umatrix.jpg", 30, 1)) {
            ...
            this.writer.appendLatexOutput(this.testrun.getScientificDescription("uMatrix")
        ...
        // this.writer.appendLatexOutput("\\end{itemize}\n");
        // this.writer.appendLatexOutput("\\max stable is level " + nodeInfos[1] + " for "+nodeInfos[0] + " new clusters");
        ...
        for (int i = 0; i < 10 && i < stableList2.size(); i++) {
            ...
            this.writer.appendLatexOutput("\\item \\begin{itemize}\\item cluster "
                    ...
                    + " inputs in cluster\\\\"
                    + classDistInCluster((int) stableList2.get(i)[1],
                            this.testrun.getClusterNodes((int) stableList2.get(i)[1], 1).getNumberOfInputs())/*
                                                                                                              * + "\\\\<a
            ...
            this.writer.appendOutput("\\includegraphics{" + SOMDescriptionLATEX.imgSubdir + "/run_" + this.testrun.getRunId() + "_clusterLevel_"
                    + (int) stableList2.get(i)[1] + "_10.jpg}\n");
            this.writer.appendLatexOutput("\\end{itemize}\n");
        }
        this.writer.appendLatexOutput("\\end{itemize}");
    ...
    private String classDistInCluster(int level, int numbInputs) {
        ...
        if (numbClasses > 0) {
            ...
            for (int i = 0; i < classDist.length; i++) {
                ...
                out += "\\item " + this.dataset.getNameOfClass(i) + ": " + classDist[i] + " inputs ("
