...
/**
 * The graphical representation of one SOM Unit, including labels, data items and class pie charts. This class makes use of the <a
 * href="http://www.cs.umd.edu/hcil/jazz/" target="_blank">Piccolo framework</a> and is the top {@link PNode} for all unit-level visualisations. This
 * PNode has one of the four child nodes, depending on the current zoom level of the {@link SOMViewer} application -
 * {@link GeneralUnitPNode#DETAIL_LEVEL_NO}, {@link GeneralUnitPNode#DETAIL_LEVEL_LOW}, {@link GeneralUnitPNode#DETAIL_LEVEL_MEDIUM} and
 * {@link GeneralUnitPNode#DETAIL_LEVEL_HIGH}. Each of the four different PNodes represents four different levels of information details displayed.
 * Each those detail nodes has potentially other child nodes:
 * <ul>
 ...
 * @author Rudolf Mayer
 * @version $Id: GeneralUnitPNode.java 2874 2009-12-11 16:03:27Z frank $
 */
public class GeneralUnitPNode extends PNode {
    ...
    public GeneralUnitPNode(Unit u, CommonSOMViewerStateData state, SOMLibClassInformation classInfo, SOMLibDataInformation dataInfo, double width,
            double height) {
     ...
     * 
     */
    public GeneralUnitPNode(Unit u, CommonSOMViewerStateData state, SOMLibClassInformation classInfo, SOMLibDataInformation dataInfo,
            Point[][] locations, double width, double height) {
        this.u = u;
        X = u.getXPos() * width;
        Y = u.getYPos() * height;
        initPNodeProperties(width, height);
    ...
    public void reInitUnitDetails() {
        if (u.getNumberOfMappedInputs() > 0) {
            removeDetailNodes();
            for (int i = 0; i < detailNodes.length; i++) {
                detailNodes[i].removeAllChildren();
                removeAllChildren();
            }
            initDetails();
    ...
    private void removeDetailNodes() {
        for (int i = 0; i < detailNodes.length; i++) {
            if (detailNodes[i].isDescendentOf(this)) {
                removeChild(detailNodes[i]);
            }
        }
    }
    ...
    public void initClassPieCharts(Unit u, SOMLibClassInformation classInfo, double width, double height) {
        ...
        if (classInfo != null) { // class information present, generate pie chart
            ...
            for (int i = 0; i < u.getNumberOfMappedInputs(); i++) {
                int ci = classInfo.getClassIndex(u.getMappedInputName(i).trim());
                if (ci < 0) {
                    System.err.println("ERROR: Class index could not be retrieved for item " + u.getMappedInputName(i));
                } else {
                    values[ci] += 1;
                }
            }
     ...
     * 
     * @param numLabels Number of labels to be shown. If '-1' the limit is set to the number of labels in unit description file.
    ...
    private PNode initLabels(int numLabels, final int numCol, final int fontSize, final int length) {
        ...
        if (u.getLabels() != null) {
            ...
            for (int i = 0; i < numCol; i++) {
                xOffset[i] = 2 + (i * (width / (double) numCol));
            }
            double yOffset = 2;
            int columnIndex = 0;
            ...
            for (int i = 0; i < numLabels; i++) {
                ...
                // TODO: find a better way to set the colour. maybe the current palette can give the preferred label colour?
                if (state.exactUnitPlacement && getMapPNode().getCurrentVisualization() == null) { // white labels for sky vis
                ...
                label.addAttribute("tooltip", u.getLabels()[i].getName() + "\nqe: "
                        + StringUtils.format3FractionDigits.format(u.getLabels()[i].getQe()) + "\nmean: "
                        + StringUtils.format3FractionDigits.format(u.getLabels()[i].getValue()));
     ...
     * 
     * @param threshold Displays description of mapped inputs below certain threshold (count), otherwise a count is displayed.
    ...
    private PNode[] initData(int threshold, int fontSize, double yInOffset, int detailLevel, int variant) {
        ...
        if (variant == DATA_DISPLAY_VARIANT_INPUTOBJECT || variant == DATA_DISPLAY_VARIANT_INPUTOBJECTSHIFTED) { // Display stars or simply count
            if (locations != null) {
                ...
                for (int index = 0; index < nodesToDisplay; index++) {
                    ...
                    String inputDistance = StringUtils.format3FractionDigits.format(u.getMappedInputDistance(index));
                    String inputText = "";
            ...
            if (detailLevel == DETAIL_LEVEL_HIGH) {
                ...
                for (int i = 0; i < numDataCol; i++) {
                    xDataOffsets[i] = (i * (width / (double) numDataCol)) + 2;
                }
                double yOffset = yInOffset + dataCountDetail[detailLevel].getBoundsReference().getHeight() + 2;
                int numData = Math.min(dataNodes.length, u.getNumberOfMappedInputs()); // limitation by number of labels in unit description file
                int x = 0;
                for (int index = 0; index < numData; index++) {
                    ...
                    try {
                        ...
                        Logger.getLogger("at.tuwien.ifs.somtoolbox").warning("URLDecoder had problems reading the name of the datum '" + inputName + "'.");
                        inputText = inputName;
                    }
                    ...
                    if (dataInfo != null) {
                        try {
                            pText.addAttribute("tooltip", inputText + "\ndistance: "
                                    + StringUtils.format3FractionDigits.format(u.getMappedInputDistances()[index]) + "\n"
                                    + URLDecoder.decode(dataInfo.getDataLocation(inputName), "UTF-8"));
                        } catch (Exception e) {
                            Logger.getLogger("at.tuwien.ifs.somtoolbox").warning(
                                    "URLDecoder had problems reading the name of the datum '" + inputName + "'.");
    ...
    /**
     * Initializes the details node. A pie chart is generated if class info is available (except {@link #DETAIL_LEVEL_HIGH}, quality measure info is
     * added for {@link #DETAIL_LEVEL_HIGH}.
     */
    ...
    private void initDetails(int level) {
        ...
        if (u.getLabels() != null && state.labelVisibilityMode) {
            if (labelDetailNodes[level] == null) {
                labelDetailNodes[level] = initLabels(NUMBER_OF_LABELS[level], NUMBER_OF_COLUMNS[level], FONT_SIZE_LABELS[level],
                        MAX_LABEL_LENGTH[level]);
            }
            ...
            if ((qm = u.getLayer().getQualityMeasure()) != null) {
                ...
                try {
                    for (int i = 0; i < qm.getUnitQualityNames().length; i++) {
                        PText qmText = new PText(
                                qm.getUnitQualityNames()[i]
                                        + ": "
                                        + StringUtils.format3FractionDigits.format(qm.getUnitQualities(qm.getUnitQualityNames()[i])[u.getXPos()][u.getYPos()]));
                    ...
                    Logger.getLogger("at.tuwien.ifs.somtoolbox").severe(
                            e.getMessage() + " Aborting. BTW: the must be a major flaw" + "in the quality measure class that has been used.");
                    System.exit(-1);
                }
        ...
        for (int i = 0; i < DATA_DISPLAY_VARIANTS; i++) {
            if (dataDetail[level][i] == null) {
                dataDetail[level][i] = initData(state.thresholdInputPercentage[level], FONT_SIZE_DATA[level], yOffset, level, i);
            }
        }
        ...
        if (level != DETAIL_LEVEL_HIGH) {
            if (classInfo != null) { // class information present, generate pie chart
                ...
                double pHeight = height - (level == DETAIL_LEVEL_NO ? height / 5 : dataCountDetail[level].getFullBoundsReference().getHeight());
                pieChartDetailNodes[level].setBounds(xOffset, 0, pWidth, pHeight);
                if (state.classPiechartsVisible) {
    ...
    protected void paint(PPaintContext paintContext) {
        ...
        if (state.exactUnitPlacement && getMapPNode().getCurrentVisualization() == null) { // black background for sky vis
            border.setRect(X, Y, width, height);
        ...
        if (!((PCanvas) pCam.getComponent()).getClass().equals(MapOverviewPane.MapOverviewCanvas.class)) { // only for main display
            ...
            if ((currentScale != oldScale)) {
                ...
                } else if ((currentScale >= state.scaleLimits[1]) && currentScale < state.scaleLimits[2]) { // little information
    ...
    public void updateClassPieCharts() {
        if ((u.getNumberOfMappedInputs() > 0) && classInfoSelectionChanged) {
            ...
            if (!state.classPiechartsVisible) { // hiding class info is select
                ...
                double[] classValues = pieChartDetailNodes[DETAIL_LEVEL_LOW].getValues();
                int sc = 0;
    ...
    /**
     * This implementation does not check for the pie charts ({@link #pieChartDetailNodes} to be initialised and should therefore be only used if it
     * is for sure != null.
     */
    ...
    public void setClassColor(int index, Color color) {
        for (int i = 0; i < pieChartDetailNodes.length; i++) {
            if (pieChartDetailNodes[i] != null) {
                pieChartDetailNodes[i].setColor(index, color);
            }
        }
    }
    ...
    public void setClassColors(Color[] colors) {
        for (int i = 0; i < pieChartDetailNodes.length; i++) {
            if (pieChartDetailNodes[i] != null) {
                pieChartDetailNodes[i].setColors(colors);
            }
        }
    }
    ...
    public PieChartPNode getClassPieChart(int width, int height) {
        if (classInfo != null) { // class information present, generate pie chart
            ...
            for (int i = 0; i < u.getNumberOfMappedInputs(); i++) {
                int ci = classInfo.getClassIndex(u.getMappedInputNames()[i]);
                if (ci < 0) {
                    System.err.println("ERROR: Class index could not be retrieved for item " + u.getMappedInputNames()[i]);
                } else {
                    values[ci] += 1;
                }
            }
            PieChartPNode pieChartPNode = new PieChartPNode(0, 0, width, height, values);
