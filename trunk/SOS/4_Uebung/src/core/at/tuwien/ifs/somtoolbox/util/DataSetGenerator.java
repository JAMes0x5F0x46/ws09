 ...
 * @author Rudolf Mayer
 * @version $Id: DataSetGenerator.java 2874 2009-12-11 16:03:27Z frank $
 */
public class DataSetGenerator {
    ...
    public String getDataSetAsString() {
        ...
        for (int i = 0; i < allPoints.size(); i++) {
            DataPoint d = (DataPoint) allPoints.elementAt(i);
    ...
    private void makeNonNegative(Vector<DataPoint> points) {
        ...
        for (int i = 0; i < points.size(); i++) {
            DataPoint d = (DataPoint) points.elementAt(i);
    ...
    private Vector<DataPoint> generatePoints(String name, int num, double x, double y, double sigmaX, double sigmaY) {
        ...
        for (int i = 0; i < num; i++) {
            res.addElement(new DataPoint(name + "-" + (i + 1), rand.nextGaussian(x, sigmaX), rand.nextGaussian(y, sigmaY)));
        }
        return res;
    }

    public DataPoint[] getPoints() {
        return (DataPoint[]) allPoints.toArray(new DataPoint[allPoints.size()]);
    }
    ...
    public double[][][] getData() {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int j = 0; j < classPoints[i].size(); j++) {
                data[i][j][0] = ((DataPoint) classPoints[i].elementAt(j)).getX();
                data[i][j][1] = ((DataPoint) classPoints[i].elementAt(j)).getY();
            }
        }
        return data;
    }
    ...
    public double[][][] getData3D() {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int j = 0; j < classPoints[i].size(); j++) {
                data[i][j][0] = ((DataPoint) classPoints[i].elementAt(j)).getX();
                data[i][j][1] = ((DataPoint) classPoints[i].elementAt(j)).getY();
                data[i][i][2] = ((DataPoint) classPoints[i].elementAt(i)).getY() * allPoints.elementAt(i).getX() / 2;
            }
        }
        return data;
    }
