 ...
 * @author Rudolf Mayer
 * @version $Id: ReportGenWindow.java 2874 2009-12-11 16:03:27Z frank $
 */
...
public class ReportGenWindow {
    ...
    public static final String[] unitAndMmapMeasures = new String[] { "Quantization Error", "Mean Quantization Error", "Intrinsic Distance",
            "Topographic Error", "Entropy Error", "Silhouette Value", "Distortion Values" };

    public static final String[] mapMeasures = new String[] { "Spearman Coefficient", "Sammon Measure", "Metric Multiscaling", "Inversion Measure" };
    ...
    /**
     * loads the specified Input Vector file into some data structure (hopefully one that already exists) and prints a list of the vectors in the
     * selection list for marking inputs that shall be observed
     * 
    ...
    public boolean loadInputVectors() {
        ...
        if (!new File(templateVectorFilename).exists()) {
            Logger.getLogger("at.tuwien.ifs.somtoolbox.reports").warning("File does not exist: " + templateVectorFilename);
            return false;
        }
    ...
    public void generateReport(int type) {
        ...
        // check if we have the input data loaded already, and then use the alternative constructor of DatasetInformation
        DatasetInformation dataInfo = null;
        if (state.inputDataObjects.getInputData() != null) {
            dataInfo = new DatasetInformation(idList, textInputVectorsLocation.getText(), textTemplateVectorsLocation.getText(),
                    textClassFileLocation.getText(), EP, state);
        } else {
            dataInfo = new DatasetInformation(idList, textInputVectorsLocation.getText(), textTemplateVectorsLocation.getText(),
                    textClassFileLocation.getText(), EP);
        }
        this.reportGenerator.createReport(type, textOutputLocation.getText(), dataInfo, getTestrunResultCollection(dataInfo, type));
    }

    /**
     * removes the specified testrun the testrun identified with this id (that is the one at position id (starting with 1st position == 0)) is removed
     * from the list of testruns as well as from the panel.
     * 
    ...
    /**
     * Update: This is done for comfortability reasons, the user doesn't have to add the necessary files to make a report on the default Map..
     * 
     * @param dataInfo
     * @return
     */
    private TestRunResultCollection getTestrunResultCollection(DatasetInformation dataInfo, int type) {
        ...
        if (this.state == null)
            ...
            default_testrun = new TestRunResult(dataInfo, Map_location.getText(), textPropertiesLocation.getText(), getUnitPath(), getWeightPath(),
                    getDataWinnerMappingPath(), 0, type);
        else
            // initialize with already trained SOM
            default_testrun = new TestRunResult(dataInfo, Map_location.getText(), textPropertiesLocation.getText(), getUnitPath(), getWeightPath(),
                    getDataWinnerMappingPath(), 0, type, state);
        ...
        for (int i = 0; i < this.testrunResults.size(); i++) {
            ...
            collection.addTestrunResult(new TestRunResult(dataInfo, this.testrunResults.get(i).getMapFilePath(),
                    ...
                    this.testrunResults.get(i).getWeightFilePath(), this.testrunResults.get(i).getDwFilePath(), i + 1, type));
        }
    ...
    private JPanel getJContentPane() {
        ...
        panelContent.add(new JLabel("Class information file (.clsinf)"), gc.nextRow());
        ...
        inputElementSelectionScrollerContent.setLayout(new BoxLayout(inputElementSelectionScrollerContent, BoxLayout.Y_AXIS));
