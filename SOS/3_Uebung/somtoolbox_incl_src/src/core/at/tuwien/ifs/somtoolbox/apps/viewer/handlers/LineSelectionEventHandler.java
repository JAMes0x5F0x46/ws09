 ...
 * @version $Id: LineSelectionEventHandler.java 2874 2009-12-11 16:03:27Z frank $
 */
...
public class LineSelectionEventHandler extends OrderedPSelectionEventHandler {
    ...
    // width of units * blur factor = selection --> blur factor 1.0 means that every unit below the selection and +/- unit with is selected
    ...
    protected void drag(PInputEvent e) {
        ...
        // float blur = 0.4f;
        PPath pp = PPath.createLine((float) lastPoint.getX(), (float) lastPoint.getY(), (float) nextPoint.getX(), (float) nextPoint.getY());
                ...
                if (o instanceof GeneralUnitPNode) {
                    ...
                    if (this.onSelection(upn, nextPoint)) {
                        if (!this.alreadySelected(o)) {
                            super.select(upn);
                            this.selectionStatusString += " (" + upn.getUnit().getXPos() + "/" + upn.getUnit().getYPos() + ")";
    ...
    protected boolean onSelection(GeneralUnitPNode upn, Point2D selectedPoint) {
        ...
        if (selectedPoint.getX() >= nodex1 - (upn.getWidth() * BLUR_FACTOR) && selectedPoint.getX() <= nodex2 + (upn.getWidth() * BLUR_FACTOR)) {
            if (selectedPoint.getY() >= nodey1 - (upn.getWidth() * BLUR_FACTOR) && selectedPoint.getY() <= nodey2 + (upn.getHeight() * BLUR_FACTOR)) {
                return true;
            }
        }
