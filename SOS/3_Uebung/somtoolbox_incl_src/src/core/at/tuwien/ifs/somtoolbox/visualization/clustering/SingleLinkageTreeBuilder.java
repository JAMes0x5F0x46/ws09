...
/**
 * Single Linkage Clustering Algorithm. This is an adapted version only calculating the distances to thedirect neighbors. This class is not compatible
 * with mnemonic SOMs (and probably also not compatible with hierarchical SOMs)
 * 
 * @author Angela Roiger
 * @version $Id: SingleLinkageTreeBuilder.java 2874 2009-12-11 16:03:27Z frank $
 */
    ...
    private TreeSet<NodeDistance> calculateNearestDistances(GeneralUnitPNode[][] units) throws ClusteringAbortedException {
        ...
        for (int i = 0; i < xdim; i++) {
            for (int j = 0; j < ydim; j++) {
                ...
                try {
                    if (i < (xdim - 1)) {
                        dists.add(new NodeDistance(tmp[i][j], tmp[i + 1][j], l1.distance(units[i][j].getUnit().getWeightVector(),
                    ...
                    if (j < (ydim - 1)) {
                        dists.add(new NodeDistance(tmp[i][j], tmp[i][j + 1], l1.distance(units[i][j].getUnit().getWeightVector(),
                        ...
                        if (i < (xdim - 1)) {
                            dists.add(new NodeDistance(tmp[i][j], tmp[i + 1][j + 1], l1.distance(units[i][j].getUnit().getWeightVector(),
